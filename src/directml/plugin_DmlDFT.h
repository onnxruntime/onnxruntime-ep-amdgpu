...
namespace Plugin_StockhamFFT_Float16 {
#include "DmlExecutionProvider/Operators/GeneratedShaders/stockham_fp16.h"
}

namespace Plugin_BluesteinChirp_Float32 {
#include "DmlExecutionProvider/Operators/GeneratedShaders/bluestein_chirp.h"
}
...
public:
    PluginGpuDFTOperator(ID3D12Device* device, uint32_t axis = 1, bool isOnesided = true, bool isInverse = false, MLOperatorTensorDataType dataType = MLOperatorTensorDataType::Float)
     : m_device(device)
     , m_axis(axis)
    ...
    PluginGpuDFTOperator(IMLOperatorKernelCreationContext* context, int32_t version)
    {
    ...
    void PrepareBluesteinZChirp(MLOperatorTensorDataType dataType)
    {
        ...
        switch (dataType)
        {
            ...
            computePsoDesc.CS = CD3DX12_SHADER_BYTECODE(Plugin_BluesteinChirp_Float32::g_BluesteinZChirp, sizeof(Plugin_BluesteinChirp_Float32::g_BluesteinZChirp));
            break;
            ...
            case MLOperatorTensorDataType::Float16:
            {
                ...
                computePsoDesc.CS = CD3DX12_SHADER_BYTECODE(Plugin_BluesteinChirp_Float16::g_BluesteinZChirp,
                                                            sizeof(Plugin_BluesteinChirp_Float16::g_BluesteinZChirp));
            }
            break;
    ...
    void PrepareStockhamFFT(MLOperatorTensorDataType dataType)
    {
        ...
        {
            case MLOperatorTensorDataType::Float:
            computePsoDesc.CS = CD3DX12_SHADER_BYTECODE(Plugin_StockhamFFT_Float32::g_DFT, sizeof(Plugin_StockhamFFT_Float32::g_DFT));
            break;
            ...
            case MLOperatorTensorDataType::Float16:
            {
                ...
                computePsoDesc.CS = CD3DX12_SHADER_BYTECODE(Plugin_StockhamFFT_Float16::g_DFT, sizeof(Plugin_StockhamFFT_Float16::g_DFT));
            }
            break;
        ...
        )
    {
        ...
        if (PluginDFTHelpers::IsPowerOfTwo(params.DFTLength))
        {
        ...
        else
        {
            ...
            auto M = PluginDFTHelpers::NextPowerOf2((2 * dftLength) - 1);
        ...
        ID3D12GraphicsCommandList* commandList)
    {
        ...
        while (pendingElementCount > 0)
        {
            ...
            PluginDFTHelpers::GetNextDispatchSize(
                pendingElementCount,
                1,
                64,
                dispatchSizeX,
...
public:
        ...
        IMLOperatorKernel** kernel)
    {
        try
        {
            ...
            auto dftOperator = wil::MakeOrThrow<PluginGpuDFTOperator>(context, version);
    ...
    static void RegisterDFTKernel(IMLOperatorRegistry* registry, int32_t version)
    {
        ...
        auto shareInferrer = wil::MakeOrThrow<PluginDFTShapeInferrer>();
        auto factory = wil::MakeOrThrow<PluginGpuDFTOperatorFactory>();
