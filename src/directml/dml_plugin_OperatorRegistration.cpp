...
#define REG_INFO(version, operatorName, ...) \
    #operatorName, OperatorHelper::OnnxOperatorSet##version::sc_sinceVer_##operatorName, onnxruntime::kOnnxDomain,     \
        Create##operatorName, OperatorHelper::ShapeInferenceFunction<OperatorHelper::ShapeInferenceHelper_##operatorName>, Aliases(),    \
        ##__VA_ARGS__,
...
void RegisterDmlOperators(IMLOperatorRegistry* registry, const PluginDmlExecutionProviderImpl* executionProvider)
{
    ...
    for (const OperatorRegistrationInformation& information : operatorRegistrationInformationTable)
    {
        ...
        registryPrivate->RegisterOperatorKernel(
            &desc,
            factory.Get(),
            shapeInferrer.Get(),
            supportQuery.Get(),
    ...
    PluginGpuDFTOperatorFactory::RegisterDFTKernel(registry, 17);
    PluginGpuDFTOperatorFactory::RegisterDFTKernel(registry, 20);
    PluginDmlSTFTOperatorFactory::RegisterSTFTKernel(registry);
