...
HRESULT __stdcall PluginDmlExecutionProviderImpl::CopyTensors(gsl::span<IMLOperatorTensor*> dst, gsl::span<IMLOperatorTensor*> src) const noexcept
{
    try
    {
        ...
        for (uint32_t i = 0; i < dst.size(); ++i)
        {
            ...
            {
                THROW_IF_FAILED(CopyTensor(dst[i], src[i]));
                continue;
            }
            ...
            THROW_HR_IF(E_INVALIDARG, dstSizeInBytes != srcSizeInBytes); // Tensors must be the same size
            ...
            THROW_HR_IF(E_INVALIDARG, dataSizesInBytes.back() != ComputeByteSizeFromTensor(*src[i])); // Tensors must be the same size
...
Ort::Status PluginDmlExecutionProviderImpl::OnRunStart(const OrtRunOptions& run_options)
{
    if (GraphCaptureEnabled())
    {
        ...
        {
            ENFORCE(TryParseStringWithClassicLocale<int>(graphAnnotationStr, dmlGraphAnnotationId),
