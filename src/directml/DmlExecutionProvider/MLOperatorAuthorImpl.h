...
namespace InputTypeHelpers {
    inline bool IsTensorType(const onnx::TypeProto* type) {
        return type && type->value_case() == onnx::TypeProto::kTensorType;
    }
    inline bool IsTensorType(const OrtTypeInfo* type) {
        return type && type->type == ONNX_TYPE_TENSOR;
    }
    inline bool IsSequenceType(const onnx::TypeProto* type) {
        return type && type->value_case() == onnx::TypeProto::kSequenceType;
    }
    inline bool IsSequenceType(const OrtTypeInfo* type) {
        return type && type->type == ONNX_TYPE_SEQUENCE;
    }
    inline bool HasTensorShape(const onnx::TypeProto* type) {
        return type->tensor_type().has_shape();
    }
    inline bool HasTensorShape(const OrtTypeInfo* type) {
        if (!type->tensor_type_info) return false;
        return type->tensor_type_info->HasShape();
    }
    inline int GetTensorDimCount(const onnx::TypeProto* type) {
        return type->tensor_type().shape().dim_size();
    }
    inline int GetTensorDimCount(const OrtTypeInfo* type) {
        if (!type->tensor_type_info || !type->tensor_type_info->HasShape()) return 0;
        const auto* shape = type->tensor_type_info->GetShape();
        return shape ? static_cast<int>(shape->NumDimensions()) : 0;
    }
    inline bool HasDimValue(const onnx::TypeProto* type, int dim_idx) {
        return type->tensor_type().shape().dim(dim_idx).has_dim_value();
    }
    inline bool HasDimValue(const OrtTypeInfo* type, int dim_idx) {
        const auto* shape = type->tensor_type_info->GetShape();
        if (!shape || dim_idx < 0 || static_cast<size_t>(dim_idx) >= shape->NumDimensions()) return false;
        return (*shape)[dim_idx] >= 0;
    }
}

// Inline method querying whether tensor shapes are defined, during wrappers
// of shape inference callbacks. Works with both TypeProto* and OrtTypeInfo*.
template <class T>
bool InputTensorShapesDefinedOnNode(const onnxruntime::OpNodeProtoHelper<T>& nodeInfo)
{
    ...
    for (uint32_t inputIndex = 0; inputIndex < inputCount; ++inputIndex)
    {
        ...
        if (input)
        {
            if (InputTypeHelpers::IsTensorType(input))
            {
                if (!InputTypeHelpers::HasTensorShape(input))
                {
                    return false;
                }

                int dim_count = InputTypeHelpers::GetTensorDimCount(input);
                for (int input_dim = 0; input_dim < dim_count; ++input_dim)
                {
                    if (!InputTypeHelpers::HasDimValue(input, input_dim))
                    {
                        return false;
                    }
                }
            }
            else if (InputTypeHelpers::IsSequenceType(input))
            {
                return false;
            }
        }
