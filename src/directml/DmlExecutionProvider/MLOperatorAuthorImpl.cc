...
namespace Windows::AI::MachineLearning::Adapter
{
    ...
    namespace {
        // TypeProto* overload
        inline bool HasSequenceType(const onnx::TypeProto* type) { return type && type->has_sequence_type(); }

        // OrtTypeInfo* overload
        inline bool HasSequenceType(const OrtTypeInfo* type) { return type && type->type == ONNX_TYPE_SEQUENCE; }

        // Wrappers that implement TypeProto interface using OrtTypeInfo data
        class OrtTypeInfoTensorTypeWrapper {
        public:
            class Shape {
            public:
                class Dim {
                public:
                    explicit Dim(const onnxruntime::TensorShape* shape, int idx) : shape_(shape), idx_(idx) {}

                    bool has_dim_value() const {
                        // In TensorShape, negative values represent symbolic dimensions
                        // Positive or zero values are concrete dimensions
                        if (!shape_ || idx_ < 0 || static_cast<size_t>(idx_) >= shape_->NumDimensions()) {
                            return false;
                        }
                        return (*shape_)[idx_] >= 0;
                    }
                ...
                private:
                    const onnxruntime::TensorShape* shape_;
                    int idx_;
                };

                explicit Shape(const onnxruntime::TensorShape* shape) : shape_(shape) {}

                Dim dim(int idx) const { return Dim(shape_, idx); }

                int dim_size() const {
                    if (!shape_)
                        return 0;
                    return static_cast<int>(shape_->NumDimensions());
                }
            ...
            explicit OrtTypeInfoTensorTypeWrapper(const OrtTypeInfo* type_info) : shape_(nullptr) {
                if (type_info && type_info->type == ONNX_TYPE_TENSOR && type_info->tensor_type_info) {
                    const OrtTensorTypeAndShapeInfo* tensor_info = type_info->tensor_type_info.get();
                    if (tensor_info && tensor_info->HasShape()) {
                        shape_ = tensor_info->GetShape();
                    }
                }
            }

            bool has_shape() const { return shape_ != nullptr; }

            Shape shape() const { return Shape(shape_); }

        private:
            const onnxruntime::TensorShape* shape_;
        };

        class OrtTypeInfoSequenceElemWrapper {
        public:
            explicit OrtTypeInfoSequenceElemWrapper(const OrtSequenceTypeInfo* seq_info) : elem_type_info_(nullptr) {
                if (seq_info && seq_info->sequence_key_type_) {
                    elem_type_info_ = seq_info->sequence_key_type_.get();
                }
            }

            OrtTypeInfoTensorTypeWrapper tensor_type() const { return OrtTypeInfoTensorTypeWrapper(elem_type_info_); }

        private:
            const OrtTypeInfo* elem_type_info_;
        };

        class OrtTypeInfoSequenceTypeWrapper {
        public:
            explicit OrtTypeInfoSequenceTypeWrapper(const OrtTypeInfo* type_info) : seq_info_(nullptr) {
                if (type_info && type_info->type == ONNX_TYPE_SEQUENCE && type_info->sequence_type_info) {
                    seq_info_ = type_info->sequence_type_info.get();
                }
            }

            OrtTypeInfoSequenceElemWrapper elem_type() const { return OrtTypeInfoSequenceElemWrapper(seq_info_); }

        private:
            const OrtSequenceTypeInfo* seq_info_;
        };

        // TypeProto* overload
        inline const auto& GetSequenceType(const onnx::TypeProto* type) { return type->sequence_type(); }

        // OrtTypeInfo* overload - returns wrapper with real data
        inline OrtTypeInfoSequenceTypeWrapper GetSequenceType(const OrtTypeInfo* type) {
            return OrtTypeInfoSequenceTypeWrapper(type);
        }

        // has_tensor_type helpers
        inline bool HasTensorType(const onnx::TypeProto* type) { return type && type->has_tensor_type(); }
        inline bool HasTensorType(const OrtTypeInfo* type) { return type && type->type == ONNX_TYPE_TENSOR; }

        // tensor_type helpers
        inline const auto& GetTensorType(const onnx::TypeProto* type) { return type->tensor_type(); }
        inline OrtTypeInfoTensorTypeWrapper GetTensorType(const OrtTypeInfo* type) {
            return OrtTypeInfoTensorTypeWrapper(type);
        }
    } // namespace
...
#pragma warning(pop)
    ...
    // Helper template to detect if we're working with TypeProto or OrtTypeInfo
    template <typename T>
    struct is_type_proto : std::is_same<T, const ONNX_NAMESPACE::TypeProto*> {};

    template <typename T>
    struct is_ort_type_info : std::is_same<T, const OrtTypeInfo*> {};

    // ABI-safe overload that works with OrtTypeInfo instead of TypeProto
    ::MLOperatorEdgeDescription ToMLEdgeDesc(const OrtTypeInfo* type_info) {
        // Initialized to undefined class and data type
        MLOperatorEdgeDescription ret = {};
        ...
        if (type_info->type == ONNX_TYPE_TENSOR) {
            ...
            if (type_info->tensor_type_info) {
                ...
                ret.tensorDataType = ToMLTensorDataType(onnx::TensorProto_DataType(static_cast<int>(elem_type)));
            }
...
#pragma warning(pop)
    ...
    HRESULT STDMETHODCALLTYPE OpNodeInfoWrapper<NodeInfoImpl_t, Base1_t, Base2_t>::GetInputEdgeDescription(
        ...
        ORT_TRY {
            ...
            // Use auto to work with both TypeProto* and OrtTypeInfo*
            auto type = m_impl->GetInputType(inputIndex);
            ML_CHECK_BOOL(type != nullptr);
            *edgeDesc = ToMLEdgeDesc(type); // Overload resolution chooses correct version
    ...
    HRESULT STDMETHODCALLTYPE OpNodeInfoWrapper<NodeInfoImpl_t, Base1_t, Base2_t>::GetOutputEdgeDescription(
        ...
        ORT_TRY {
            ...
            memset(edgeDesc, 0, sizeof(*edgeDesc));
            // Use auto to work with both TypeProto* and OrtTypeInfo*
            auto type = m_impl->GetOutputType(outputIndex);
            ML_CHECK_BOOL(type != nullptr);
            *edgeDesc = ToMLEdgeDesc(type); // Overload resolution chooses correct version
    ...
    HRESULT STDMETHODCALLTYPE OpNodeInfoWrapper<NodeInfoImpl_t, Base1_t, Base2_t>::GetInputTensorShape(
        ...
        ORT_TRY {
            ...
            if (m_inputShapesOverride) {
                ...
                ML_CHECK_BOOL(HasTensorType(inputType));
                for (uint32_t i = 0; i < dimensionCount; ++i) {
                    ...
                    assert(GetTensorType(inputType).shape().dim(i).has_dim_value());
                    dimensions[i] = static_cast<uint32_t>(GetTensorType(inputType).shape().dim(i).dim_value());
                }
            }
    ...
    HRESULT STDMETHODCALLTYPE OpNodeInfoWrapper<NodeInfoImpl_t, Base1_t, Base2_t>::GetSequenceInputTensorShape(
        ...
        ORT_TRY {
            ...
            if (m_kernelContext) {
                ...
                const auto* inputType = m_impl->GetInputType(inputIndex);
                assert(HasSequenceType(inputType));
                ML_CHECK_BOOL(HasSequenceType(inputType));

                const auto& elemType = GetSequenceType(inputType).elem_type();
    ...
    OpNodeInfoWrapper<NodeInfoImpl_t, Base1_t, Base2_t>::IsInputValid(uint32_t inputIndex) const noexcept {
        if (IsClosed()) {
            return false;
        }
    ...
    template <class NodeInfoImpl_t, class Base1_t, class Base2_t>
    bool STDMETHODCALLTYPE
    OpNodeInfoWrapper<NodeInfoImpl_t, Base1_t, Base2_t>::IsOutputValid(uint32_t outputIndex) const noexcept {
        if (IsClosed()) {
            return false;
        }
            ...
            *dimensionCount = 0;
            ...
            if (m_inputShapesOverride) {
                *dimensionCount = gsl::narrow_cast<uint32_t>(m_inputShapesOverride->GetShape(inputIndex).size());
                ...
                // Shape inference is only done when all dimensions of all inputs have known values,
                // so the input tensors will always have shapes at this point.
                assert(GetTensorType(inputType).has_shape());

                *dimensionCount = GetTensorType(inputType).shape().dim_size();
            }
    ...
    Ort::Status AbiOpKernel::Compute(onnxruntime::OpKernelContext* context) const
    {
        auto winmlProviderCapture = m_winmlProvider;
        auto internalOpCapture = m_internalOperator;
...
#define CASE_PROTO(X, Y, Z)                                                                        \
  case ONNX_NAMESPACE::TensorProto_DataType::TensorProto_DataType_##X: {                           \
    ...
    //==============================================================================
    // Explicit Template Instantiation for ABI-Safe Support Query Context
    //==============================================================================
