...
namespace Windows::AI::MachineLearning::Adapter
{
    ...
    using KernelSupportQuery = std::function<bool(const OrtNode* node, const OrtApi& ort_api)>;
    ...
    struct InternalRegistrationInfo
    {
        ...
        // kernel creation information
        IMLOperatorKernelFactory* kernelFactory = nullptr;
        IMLOperatorShapeInferrer* shapeInferrer = nullptr;
        AttributeMap defaultAttributes;  // Store by value to fix lifetime issues
        bool requiresInputShapesAtCreation = false;
        bool requiresOutputShapesAtCreation = false;
        bool isInternalOperator = false;
    };

    // map using string keys (domain::operator_name) instead of internal KernelDef pointers
    using InternalRegistrationInfoMap = std::unordered_map<std::string, std::shared_ptr<InternalRegistrationInfo>>;
}
