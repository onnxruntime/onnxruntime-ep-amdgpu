...
public:
    ...
    void Compute(const MLOperatorKernelContext& kernelContext)
    {
        ...
        {
            auto inputTensor = kernelContext.GetInputTensor(0);
            auto* inputTensorPtr = inputTensor.GetInterface().Get();

            auto outputTensor = kernelContext.GetOutputTensor(0);
            auto* outputTensorPtr = outputTensor.GetInterface().Get();

            inputTensors = { inputTensorPtr };
            outputTensors = { outputTensorPtr };
        }
