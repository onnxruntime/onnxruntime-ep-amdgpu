...
void PluginDmlExecutionContext::CopyBufferRegion(ID3D12Resource* dstBuffer, uint64_t dstOffset,
                                                 D3D12_RESOURCE_STATES dstState,
                                        ID3D12Resource* srcBuffer, uint64_t srcOffset, D3D12_RESOURCE_STATES srcState,
                                        uint64_t byteCount) {
    assert(!m_closed);
...
void PluginDmlExecutionContext::FillBufferWithPattern(
    ID3D12Resource* dstBuffer, gsl::span<const std::byte> pattern /* Data type agnostic value, treated as raw bits */) {
    SetCommandRecorder(&m_dmlRecorder);
    m_dmlRecorder.FillBufferWithPattern(dstBuffer, pattern);
}

void PluginDmlExecutionContext::ExecuteCommandList(ID3D12GraphicsCommandList* commandList, _Outptr_ ID3D12Fence** fence,
...
void PluginDmlExecutionContext::InitializeOperator(IDMLCompiledOperator* op,
...
void PluginDmlExecutionContext::ExecuteOperator(IDMLCompiledOperator* op,
                                                const DML_BINDING_DESC& persistentResourceBinding,
...
void PluginDmlExecutionContext::AddUAVBarrier() {
    assert(!m_closed);
    SetCommandRecorder(&m_dmlRecorder);
...
void PluginDmlExecutionContext::ResourceBarrier(gsl::span<const D3D12_RESOURCE_BARRIER> barriers) {
    assert(!m_closed);
    SetCommandRecorder(&m_dmlRecorder);
...
void PluginDmlExecutionContext::GetCommandListForRecordingAndInvalidateState(ID3D12GraphicsCommandList** commandList) {
    assert(!m_closed);
    SetCommandRecorder(&m_dmlRecorder);
...
void PluginDmlExecutionContext::SetCommandRecorder(ICommandRecorder* newRecorder) {
    assert(!m_closed);
...
void PluginDmlExecutionContext::Flush() {
    assert(!m_closed);
...
void PluginDmlExecutionContext::QueueReference(IUnknown* object) {
    assert(!m_closed);
...
void PluginDmlExecutionContext::Close() {
    assert(!m_closed);
    ...
    // Keep the execution context open when requested, e.g. when used through the python API where there's a single
    // context and single command queue
    if (!m_keepOpen) {
        m_currentRecorder = nullptr;
        m_closed = true;
    }
...
GpuEvent PluginDmlExecutionContext::GetCurrentCompletionEvent() {
    assert(!m_closed);
...
void PluginDmlExecutionContext::ReleaseCompletedReferences() {
    assert(!m_closed);
    m_queue->ReleaseCompletedReferences();
}

D3D12_COMMAND_LIST_TYPE PluginDmlExecutionContext::GetCommandListTypeForQueue() const {
    assert(!m_closed);
