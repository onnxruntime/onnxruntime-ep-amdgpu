...
Ort::Status CheckInitializer(const char* name, const OrtValue* val) {
  if (name == nullptr) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Received nullptr for name");
  }

  if (val == nullptr) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Received nullptr for OrtValue");
  }

  if (!val->IsTensor()) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Received OrtValue is not a tensor. Only tensors are supported.");
  }
  if (val->Get<Tensor>().OwnsBuffer()) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Buffer containing the initializer must be owned by the user.");
  }
  return STATUS_OK;
}
...
Ort::Status SessionOptions::AddInitializer(_In_z_ const char* name, _In_ const OrtValue* val) {
  // input validation
  ...
  if (!result) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "An OrtValue for this name has already been added: ", name);
  }
...
Ort::Status SessionOptions::AddExternalInitializers(gsl::span<const std::string> names, gsl::span<const OrtValue> values) {
  ...
  for (size_t i = 0; i < init_num; ++i) {
    ...
    if (!result) {
      return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "An OrtValue for this name has already been added: ", names[i]);
    }
  }
  return STATUS_OK;
}

Ort::Status SessionOptions::AddExternalInitializersFromFilesInMemory(gsl::span<const PathString> file_names,
  ...
  for (size_t i = 0; i < num_files; ++i) {
    ...
    if (!result) {
      return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "An entry for this name has already been added: ",
