...
class FunctionKernel : public OpKernel {
 public:
  ...
  static Ort::Status Create(FuncManager& func_mgr, const OpKernelInfo& info, std::unique_ptr<OpKernel>& out) {
    ...
    if (compute->create_state_func) {
      ...
      if (ret != 0)
        return ORT_MAKE_STATUS(FAIL, "Create state function failed. Return value:", ret);
    }
