...
Ort::Status DataTransferManager::RegisterDataTransfer(std::unique_ptr<IDataTransfer> data_transfer) {
  if (nullptr == data_transfer) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "data_transfer registered is nullptr.");
  }
  datatransfers_.push_back(std::move(data_transfer));
  return STATUS_OK;
}

Ort::Status DataTransferManager::UnregisterDataTransfer(IDataTransfer* data_transfer) {
...
Ort::Status DataTransferManager::CopyTensor(const Tensor& src, Tensor& dst) const {
  if (src.Shape().Size() != dst.Shape().Size()) {
    return ORT_MAKE_STATUS(ORT_FAIL, "Tensor size mismatch");
  }
...
Ort::Status DataTransferManager::CopyTensorAsync(const Tensor& src, Tensor& dst, Stream& stream) const {
  if (src.Shape().Size() != dst.Shape().Size()) {
    return ORT_MAKE_STATUS(ORT_FAIL, "Tensor size mismatch");
  }
...
Ort::Status DataTransferManager::CopySparseTensor(const SparseTensor& src, SparseTensor& dst) const {
...
Ort::Status DataTransferManager::CopyTensors(const std::vector<IDataTransfer::SrcDstPair>& src_dst_pairs) const {
  ...
  if (first_pair.src_stream != nullptr) {
    ORT_RETURN_IF_ERROR(first_dt->CopyTensorAsync(first_pair.src.get(), first_pair.dst.get(), *(first_pair.src_stream)));
  } else {
  ...
  for (auto cur_pair = src_dst_pairs.cbegin() + 1, end_pair = src_dst_pairs.cend(); cur_pair != end_pair; ++cur_pair) {
    if (cur_pair->src_stream == nullptr) {
      ORT_RETURN_IF_ERROR(CopyTensor(cur_pair->src, cur_pair->dst));
    } else {
      ORT_RETURN_IF_ERROR(CopyTensorAsync(cur_pair->src, cur_pair->dst, *(cur_pair->src_stream)));
    }
  }
...
#if !defined(DISABLE_SPARSE_TENSORS)
Ort::Status DataTransferManager::CopySparseTensors(const std::vector<IDataTransfer::SparseSrcDstPair>& src_dst_pairs) const {
  if (src_dst_pairs.empty())
    return STATUS_OK;
