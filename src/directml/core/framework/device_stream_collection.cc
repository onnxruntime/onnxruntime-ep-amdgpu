...
class DeviceStreamCollectionImpl {
 public:
  ...
  Ort::Status CleanUp(bool sync_streams) {
    if (sync_streams) {
  ...
  Ort::Status SetStreamOverride(Stream* stream) {
    ...
    return ORT_MAKE_STATUS(INVALID_ARGUMENT, "No matching stream found to override from OrtRunOptions");
  }
...
Ort::Status DeviceStreamCollection::SetStreamOverride(Stream* stream) {
...
Ort::Status DeviceStreamCollection::CleanUp(bool sync_streams) {
