...
Ort::Status OrtArenaCfg::FromKeyValuePairs(const OrtKeyValuePairs& kvps, OrtArenaCfg& cfg) {
  cfg = OrtArenaCfg{};  // reset to default values

  const auto from_string = [](const std::string& key, const std::string& str, auto& value) -> Ort::Status {
    if (!ParseStringWithClassicLocale(str, value).IsOK()) {
      return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Failed to parse value for ", key, " from ", str);
    }
...
ORT_API_STATUS_IMPL(OrtApis::CreateMemoryInfo_V2, _In_ const char* name, _In_ enum OrtMemoryInfoDeviceType device_type,
  ...
  *out = new OrtMemoryInfo(name, type, OrtDevice{dt, mt, vendor_id, gsl::narrow<int16_t>(device_id), alignment},
