...
template <> struct fmt::formatter<onnxruntime::AllocKind> : formatter<std::string_view> {
    format_context::iterator format(onnxruntime::AllocKind alloc_kind, format_context& ctx) const {
        std::string_view name{"unknown"};
        switch (alloc_kind) {
        case onnxruntime::AllocKind::kAllocate:
            name = "Allocate";
            break;
        case onnxruntime::AllocKind::kAllocateStatically:
            name = "AllocateStatically";
            break;
        case onnxruntime::AllocKind::kPreExisting:
            name = "PreExisting";
            break;
        case onnxruntime::AllocKind::kReuse:
            name = "Reuse";
            break;
        case onnxruntime::AllocKind::kAllocateOutput:
            name = "AllocateOutput";
            break;
        case onnxruntime::AllocKind::kShare:
            name = "Share";
            break;
        case onnxruntime::AllocKind::kAllocatedExternally:
            name = "AllocatedExternally";
            break;
        case onnxruntime::AllocKind::kNotSet:
            name = "NotSet";
            break;
        }
...
std::string ComposeNestedSubgraphInfoKeyHelper(const std::string& base,
  ...
  return ::MakeString(base, graph_depth, node_index, attr_name);
}
...
class PlannerImpl {
 ...
 private:
  ...
  Ort::Status ComputePlanForInputsAndWeights() {
  ...
  Ort::Status ComputeReuseCount() {
    ...
    for (auto& stream_execution_order : stream_nodes_) {
      for (NodeIndex node_index : stream_execution_order) {
        ...
        if (pnode == nullptr) {
          return ORT_MAKE_STATUS(ORT_FAIL, "Can not find the node ", node_index);
        }
  ...
  Ort::Status ComputeValueLocation() {
    ...
    for (auto& stream_execution_order : stream_nodes_) {
      for (NodeIndex node_index : stream_execution_order) {
        ...
        if (exec_provider == nullptr) {
          return ORT_MAKE_STATUS(ORT_FAIL, "Can not find the execution provider ",
  ...
  Ort::Status GeneratePlanForWeights() {
  ...
  Ort::Status OptimizeReusePlanForMultiStream() {
  ...
  Ort::Status ComputeSingleStreamReusePlan(size_t stream_index) {
...
#ifdef ENABLE_TRAINING
  Ort::Status CalculateProgramCounter() {
    ClearUseCount();
    ORT_RETURN_IF_ERROR(ComputeReuseCount());
  ...
  Ort::Status ComputeAllocationOrder() {
    for (auto& stream : stream_nodes_) {
      ...
      for (auto& step : stream) {
        ...
        if (pnode == nullptr) return ORT_MAKE_STATUS(ORT_FAIL, "Cannot find the node ", step);
        ...
        for (const auto& node_input : input_defs) {
          ...
          if ((actual_plan.alloc_kind == AllocKind::kAllocateStatically) && (input_kind != AllocKind::kAllocateStatically))
            return ORT_MAKE_STATUS(ORT_FAIL, "AllocateInputsContiguously() requires all inputs to be initializers, or all inputs to be non-initializers.");
  ...
  Ort::Status GenerateDeallocationPlan() {
  ...
  void PartitionIntoStreams(const ExecutionProviders& /*execution_providers*/,
    ...
    if (graph_viewer_.NumberOfNodes() > 0) {
      ...
      plan_.node_stream_map_.resize(graph_viewer_.MaxNodeIndex() + 1);
  ...
  Ort::Status BuildExecutionPlan(const ExecutionProviders& execution_providers) {
...
Ort::Status PlannerImpl::CreatePlan(
#ifdef ORT_ENABLE_STREAM
    const IStreamCommandHandleRegistry& stream_handle_registry,
#endif
    const PathString& partition_config_file) {
...
Ort::Status SequentialPlanner::CreatePlan(
...
class DeviceBasedPartitioner : public IGraphPartitioner {
 public:
  DeviceBasedPartitioner(const Ort::Logger& logger,
  ...
  Ort::Status PartitionGraph(const onnxruntime::GraphViewer& graph_viewer,
...
Ort::Status DeviceBasedPartitioner::PartitionGraph(const onnxruntime::GraphViewer& graph_viewer,
                                              const ExecutionProviders& execution_providers,
                                              std::vector<InlinedVector<NodeIndex>>& stream_nodes,
...
std::unique_ptr<IGraphPartitioner> IGraphPartitioner::CreateGraphPartitioner(const Ort::Logger& logger,
