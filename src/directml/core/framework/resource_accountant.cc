...
static Ort::Status LoadNodeAllocationStats(
  ...
  while (std::getline(file, line)) {
    ...
    size_t input_sizes = std::stoull(std::string{splits[1]});
    size_t initializers_sizes = std::stoull(std::string{splits[2]});
    size_t total_dynamic_sizes = std::stoull(std::string{splits[3]});
    size_t total_temp_allocations = std::stoull(std::string{splits[4]});
...
Ort::Status NodeStatsRecorder::CreateAccountants(
  ...
  if (!resource_partitioning_settings.empty()) {
    ...
    if (splits.size() == 2) {
      if (splits[1].empty()) {
        return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Invalid resource partitioning settings");
      }
      ...
      if (!splits[0].empty()) {
        ...
        cuda_memory_limit = cuda_memory_limit * 1024;  // to bytes
      ...
      return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Invalid format for: ",
                             kOrtSessionOptionsResourceCudaPartitioningSettings,
...
std::string IResourceAccountant::MakeUniqueNodeName(const Node& node) {
  ...
  auto hash_str = [&hash](const std::string& str) {
    hash::murmur3::x86_128(str.data(), str.size(), hash[0], &hash);
  };
