...
template <> struct fmt::formatter<onnxruntime::DataTypeImpl> : formatter<std::string_view> {
    format_context::iterator format(const onnxruntime::DataTypeImpl& data_type, format_context& ctx) const {
        return formatter<std::string_view>::format(onnxruntime::DataTypeImpl::ToString(&data_type), ctx);
    }
};

template <typename T> struct fmt::formatter<onnxruntime::PrimitiveDataType<T>> : formatter<std::string_view> {
    format_context::iterator format(const onnxruntime::PrimitiveDataType<T>& data_type, format_context& ctx) const {
        return formatter<std::string_view>::format(onnxruntime::DataTypeImpl::ToString(data_type.Type()), ctx);
    }
};

template <> struct fmt::formatter<onnxruntime::PrimitiveDataTypeBase> : formatter<std::string> {
    format_context::iterator format(const onnxruntime::PrimitiveDataTypeBase& data_type, format_context& ctx) const {
        const auto data_type_stamp{fmt::format("{}_{}", data_type.GetDataType(), data_type.GetNumSubElems())};
        return formatter<std::string>::format(data_type_stamp, ctx);
    }
};
