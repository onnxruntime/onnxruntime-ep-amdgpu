...
static Ort::Status UnpackTensorWithRawDataImpl(const void* raw_data, size_t raw_data_len, size_t expected_num_elements,
                                          size_t element_size,
  ...
  if (!onnxruntime::IAllocator::CalcMemSizeForArray(expected_num_elements, element_size, &expected_size_in_bytes)) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "size overflow");
  }
...
template <typename T>
Ort::Status UnpackTensorWithRawData(const void* raw_data, size_t raw_data_len, size_t expected_num_elements,
                               /*out*/ T* p_data) {
...
#define DEFINE_4BIT_UNPACK_TENSOR_WITH_RAW_DATA_IMPL(FOUR_BIT_TYPE, CalcPairFun)                                         \
  template <>                                                                                                            \
  Ort::Status UnpackTensorWithRawData<FOUR_BIT_TYPE>(const void* raw_data, size_t raw_data_len, size_t expected_num_elements, \
    ...
    ORT_RETURN_IF_NOT(num_packed_pairs == raw_data_len, "Unexpected number of packed int4 pairs");                       \
                                                                                                                         \
    gsl::span<const FOUR_BIT_TYPE> src_span =                                                                            \
        gsl::make_span(reinterpret_cast<const FOUR_BIT_TYPE*>(raw_data), num_packed_pairs);                              \
    gsl::span<FOUR_BIT_TYPE> dst_span = gsl::make_span(p_data, num_packed_pairs);                                        \
                                                                                                                         \
...
// Read external data for tensor in unint8_t* form and return STATUS_OK if the data is read successfully.
...
Ort::Status ReadExternalDataForTensor(const ONNX_NAMESPACE::TensorProto& tensor_proto,
...
Ort::Status TensorProtoToOrtValueImpl(const Env& env, const std::filesystem::path& model_path,
                                 const ONNX_NAMESPACE::TensorProto& tensor_proto, const MemBuffer* m,
                                 AllocatorPtr alloc, OrtValue& value) {
  if (m && m->GetBuffer() == nullptr) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "MemBuffer has not been allocated.");
  }
  ...
  if (ele_type == ONNX_TENSOR_ELEMENT_DATA_TYPE_STRING && (m || !alloc)) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "string tensor requires allocator to be provided.");
  }
  ...
  if (m) {
    ...
    if (tensor->SizeInBytes() > m->GetLen()) {
      return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "The preallocated buffer is too small. Requires ",
...
Ort::Status TensorProtoWithExternalDataToTensorProto(
  ...
  if (utils::HasString(ten_proto)) {
    ...
    for (size_t i = 0, lim = gsl::narrow<size_t>(tensor_shape.Size()); i < lim; ++i) {
      // set in raw data
...
Ort::Status ValidateExternalDataPath(const std::filesystem::path& base_dir,
  ...
  if (!base_dir.empty()) {
    ...
    for (const auto& path_component : norm_location) {
      if (path_component == ORT_TSTR("..")) {
        return ORT_MAKE_STATUS(ORT_FAIL, "External data path: ", location,
...
Ort::Status GetExternalDataInfo(const ONNX_NAMESPACE::TensorProto& tensor_proto,
                           const std::filesystem::path& tensor_proto_dir,
...
static Ort::Status UnpackTensorWithExternalDataImpl(const ONNX_NAMESPACE::TensorProto& tensor,
                                               const std::filesystem::path& tensor_proto_dir,
...
template <typename T>
Ort::Status UnpackTensorWithExternalData(const ONNX_NAMESPACE::TensorProto& tensor,
...
#define DEFINE_4BIT_UNPACK_TENSOR_WITH_EXT_DATA_IMPL(FOUR_BIT_TYPE, CalcPairFun)                                    \
  template <>                                                                                                       \
  Ort::Status UnpackTensorWithExternalData<FOUR_BIT_TYPE>(const ONNX_NAMESPACE::TensorProto& tensor,                     \
                                                     const std::filesystem::path& tensor_proto_dir,                 \
    ...
    ORT_RETURN_IF_NOT(num_packed_pairs == unpacked_tensor.size(), "Unexpected number of packed int4 pairs");        \
                                                                                                                    \
    gsl::span<const FOUR_BIT_TYPE> src_span =                                                                       \
        gsl::make_span(reinterpret_cast<const FOUR_BIT_TYPE*>(unpacked_tensor.data()), num_packed_pairs);           \
    gsl::span<FOUR_BIT_TYPE> dst_span = gsl::make_span(p_data, expected_num_elements);                              \
                                                                                                                    \
    std::memcpy(dst_span.data(), src_span.data(), num_packed_pairs);                                                \
                                                                                                                    \
    return STATUS_OK;                                                                                            \
...
#define INSTANTIATE_UNPACK_EXTERNAL_TENSOR(type)                                                                 \
  template Ort::Status UnpackTensorWithExternalData(const ONNX_NAMESPACE::TensorProto&, const std::filesystem::path&, \
                                               size_t, type*);
...
Ort::Status UnpackTensorWithExternalData(const ONNX_NAMESPACE::TensorProto& /*tensor*/,
  ...
  return ORT_MAKE_STATUS(ORT_FAIL, "External data type cannot be STRING.");
}
#endif  //! defined(ORT_MINIMAL_BUILD)

// implementation of type specific unpack of data contained within the TensorProto
template <typename T>
Ort::Status UnpackTensor(const ONNX_NAMESPACE::TensorProto& tensor, const void* raw_data, size_t raw_data_len,
...
#define DEFINE_UNPACK_TENSOR_IMPL(T, Type, field_name, field_size)                                          \
  ...
  Ort::Status UnpackTensor(const ONNX_NAMESPACE::TensorProto& tensor, const void* raw_data, size_t raw_data_len, \
    ...
    if (static_cast<size_t>(tensor.field_size()) != expected_num_elements)                                  \
      return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "corrupted protobuf data: tensor shape size(",  \
...
Ort::Status UnpackTensor(const ONNX_NAMESPACE::TensorProto& tensor, const void* raw_data, size_t raw_data_len,
  ...
  for (int i = 0; i < static_cast<int>(expected_size); i++) {
    ...
    if (v < 0 || v > max_value) {
      return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "data overflow");
    }
...
#define DEFINE_INT4_UNPACK_TENSOR_IMPL(INT4_TYPE, ONNX_INT4_TYPE)                                           \
  ...
  Ort::Status UnpackTensor(const ONNX_NAMESPACE::TensorProto& tensor, const void* raw_data, size_t raw_data_len, \
    ...
    if (nullptr == p_data) {                                                                                \
      const size_t size = raw_data != nullptr ? raw_data_len : tensor.int32_data_size();                    \
      return size == 0 ? STATUS_OK : ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT);                                 \
    }                                                                                                       \
...
Ort::Status UnpackTensor(const ONNX_NAMESPACE::TensorProto& tensor, const void* raw_data, size_t raw_data_len,
  ...
  ORT_RETURN_IF_NOT(static_cast<size_t>(tensor.int32_data_size()) == expected_float4_pairs,
                    "UnpackTensor: the pre-allocated size does not match the size in proto");
...
template <typename T>
Ort::Status UnpackTensor(const ONNX_NAMESPACE::TensorProto& tensor, const std::filesystem::path& model_path,
                    /*out*/ T* p_data, size_t expected_num_elements) {
#if !defined(ORT_MINIMAL_BUILD)
...
#define INSTANTIATE_UNPACK_TENSOR(type) \
  template Ort::Status UnpackTensor(const ONNX_NAMESPACE::TensorProto&, const std::filesystem::path&, type* p_data, size_t);
...
#define CASE_PROTO_TRACE(X, Y)                                                                     \
  case ONNX_NAMESPACE::TensorProto_DataType::TensorProto_DataType_##X:                             \
    if (!IAllocator::CalcMemSizeForArrayWithAlignment<alignment>(size, sizeof(Y), out)) {          \
      return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Invalid TensorProto"); \
    }                                                                                              \
    break;
...
#define CASE_PROTO_TRACE_FLOAT4(X, Y)                                                                            \
  case ONNX_NAMESPACE::TensorProto_DataType::TensorProto_DataType_##X:                                           \
    if (!IAllocator::CalcMemSizeForArrayWithAlignment<alignment>(Y::CalcNumFloat4Pairs(size), sizeof(Y), out)) { \
      return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Invalid TensorProto");               \
    }                                                                                                            \
    break;
#endif
...
Ort::Status GetSizeInBytesFromTensorShapeAndType(const TensorShape& shape, int32_t element_type, size_t* out) {
  const auto size = gsl::narrow<size_t>(shape.Size());
  switch (element_type) {
...
#endif
    default:
      return ORT_MAKE_STATUS(ORT_NOT_IMPLEMENTED);
  }
  return STATUS_OK;
}

template <size_t alignment>
Ort::Status GetSizeInBytesFromTensorProto(const ONNX_NAMESPACE::TensorProto& tensor_proto, size_t* out) {
  TensorShape tensor_shape = GetTensorShapeFromTensorProto(tensor_proto);
...
template <size_t alignment>
Ort::Status GetSizeInBytesFromTensorTypeProto(const ONNX_NAMESPACE::TypeProto_Tensor& tensor_proto, size_t* out) {
...
template Ort::Status GetSizeInBytesFromTensorTypeProto<0>(const ONNX_NAMESPACE::TypeProto_Tensor& tensor_proto, size_t* out);
...
#if !defined(__wasm__)
static Ort::Status GetFileContent(const Env& env, const std::filesystem::path& file_path, FileOffsetType offset,
...
Ort::Status GetExtDataFromTensorProto(const Env& env,
                                 const std::filesystem::path& model_path,
  ...
  if (external_data_file_path == onnxruntime::utils::kTensorProtoMemoryAddressTag) {
    ...
    FileOffsetType end_of_read(file_offset);
    ...
    if (prepacked_info != nullptr && !prepacked_infos->empty()) {
      for (const auto& [key, blobs] : *prepacked_infos) {
        ...
        for (const auto& blob : blobs) {
          ...
          FileOffsetType end_of_blob{blob_offset};
...
Ort::Status LoadExtDataToTensorFromTensorProto(const Env& env, const std::filesystem::path& model_path,
                                          const ONNX_NAMESPACE::TensorProto& tensor_proto,
 ...
 * @return
 */
Ort::Status TensorProtoToTensor(const Env& env, const std::filesystem::path& model_path,
  ...
  if (std::any_of(tensor_shape.GetDims().begin(), tensor_shape.GetDims().end(),
    ...
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "tensor can't contain negative dims");
  }
  ...
  if (tensor_shape != tensor.Shape()) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "TensorProtoToTensor() tensor shape mismatch!");
  }
  ...
  if (source_type->Size() > tensor.DataType()->Size()) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "TensorProto type ", DataTypeImpl::ToString(source_type),
  ...
  if (nullptr != raw_data && utils::HasString(tensor_proto)) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "string tensor can not have raw data");
  }
  ...
  if (gsl::narrow<uint64_t>(tensor_size) > SIZE_MAX) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "size overflow");
  }
  ...
  switch (tensor_proto.data_type()) {
    ...
    default: {
      ...
      return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, ostr.str());
    }
  }
...
Ort::Status CreateTensorFromTensorProto(const Env& env, const std::filesystem::path& model_path,
                                           const ONNX_NAMESPACE::TensorProto& tensor_proto, Tensor& tensor) {
...
Ort::Status GetTensorProtoWithDataIfInMemory(
...
Ort::Status TensorProtoToOrtValue(const Env& env, const std::filesystem::path& model_path,
...
Ort::Status ConstantNodeProtoToTensorProto(const ONNX_NAMESPACE::NodeProto& node,
                                              const std::filesystem::path& model_path,
...
#if !defined(DISABLE_SPARSE_TENSORS)
static Ort::Status CopySparseData(const std::string& name,
  ...
  switch (indices.data_type()) {
    case ONNX_NAMESPACE::TensorProto_DataType_INT64:
      if (needs_unpack) {
        ORT_RETURN_IF_NOT(indices.raw_data().size() == (gsl::narrow<size_t>(indices_elements) * sizeof(int64_t)),
        ...
        ORT_RETURN_IF_NOT(indices.int64_data_size() == indices_elements,
                          ...
                          indices_elements);
        indices_data = gsl::make_span(indices.int64_data().data(), gsl::narrow<size_t>(indices_elements));
      }
      break;
    case ONNX_NAMESPACE::TensorProto_DataType_INT32: {
      if (needs_unpack) {
        ORT_RETURN_IF_NOT(indices.raw_data().size() == (gsl::narrow<size_t>(indices_elements) * sizeof(int32_t)),
                          "Sparse tensor: ", name, " indices raw data size does not match expected: ",
    ...
    case ONNX_NAMESPACE::TensorProto_DataType_INT16: {
      if (needs_unpack) {
        ORT_RETURN_IF_NOT(indices.raw_data().size() == (gsl::narrow<size_t>(indices_elements) * sizeof(int16_t)),
                          "Sparse tensor: ", name, " indices raw data size does not match expected: ",
    ...
    case ONNX_NAMESPACE::TensorProto_DataType_INT8: {
      if (needs_unpack) {
        ORT_RETURN_IF_NOT(indices.raw_data().size() == gsl::narrow<size_t>(indices_elements),
                          "Sparse tensor: ", name, " indices raw data size does not match expected: ",
    ...
    default:
      return ORT_MAKE_STATUS(ORT_INVALID_GRAPH,
  ...
  if (indices_rank == 1) {
    // flattened indexes
    for (size_t i = 0, lim = gsl::narrow<size_t>(nnz_elements); i < lim; ++i) {
      ...
      copier(i, gsl::narrow<size_t>(idx));
    }
    ...
    for (auto r = rank - 1; r > 0; --r) {
      multipliers[r - 1] = dense_dims[r] * multipliers[r];
    }
    ...
    status = ORT_MAKE_STATUS(ORT_INVALID_GRAPH,
...
Ort::Status SparseTensorProtoToDenseTensorProto(const ONNX_NAMESPACE::SparseTensorProto& sparse,
                                                   const std::filesystem::path& model_path,
  ...
  *dense.mutable_name() = name;
  ...
  if (indices_rank == 2 && dense_dims.size() != gsl::narrow<size_t>(indices_dims[1])) {
    return ORT_MAKE_STATUS(ORT_INVALID_GRAPH,
                           "Sparse tensor: ", name,
  ...
  if (indices_rank == 2) {
    ...
    const int64_t expected_indices_entries = nnz_elements * indices_dims[1];
  ...
  if (type != ONNX_NAMESPACE::TensorProto_DataType_STRING) {
    ...
    std::string dense_data_storage(gsl::narrow<size_t>(dense_elements) * element_size, 0);
    if (nnz_elements > 0) {
      ...
      switch (element_size) {
        ...
        default:
          return ORT_MAKE_STATUS(ORT_FAIL, "Element_size of: ", element_size, " is not supported.",
                                 " type: ", type);
      }
    ...
    status = ORT_MAKE_STATUS(ORT_FAIL, "Unsupported sparse tensor data type of ",
...
Ort::Status DenseTensorToSparseTensorProto(const ONNX_NAMESPACE::TensorProto& dense_proto,
                                              const std::filesystem::path& model_path,
  ...
  if (dense_proto.data_type() == ONNX_NAMESPACE::TensorProto_DataType_STRING) {
    return ORT_MAKE_STATUS(ORT_FAIL, "Unsupported sparse tensor data type of ",
                           ONNX_NAMESPACE::TensorProto_DataType_STRING);
  }
...
template Ort::Status GetSizeInBytesFromTensorProto<kAllocAlignment>(const ONNX_NAMESPACE::TensorProto& tensor_proto,
                                                                       size_t* out);
template Ort::Status GetSizeInBytesFromTensorProto<0>(const ONNX_NAMESPACE::TensorProto& tensor_proto, size_t* out);
...
Ort::Status UnpackInitializerData(const onnx::TensorProto& initializer,
                             const std::filesystem::path& model_path,
                             std::vector<uint8_t>& unpacked_tensor) {
  ...
  return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Unsupported type: ", initializer.data_type());
}
#undef CASE_UNPACK

Ort::Status UnpackInitializerData(const ONNX_NAMESPACE::TensorProto& initializer, std::vector<uint8_t>& unpacked_tensor) {
