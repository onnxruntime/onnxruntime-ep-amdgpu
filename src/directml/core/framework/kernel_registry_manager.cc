...
Ort::Status KernelRegistryManager::CreateKernel(const Node& node,
...
Ort::Status KernelRegistryManager::RegisterKernels(const ExecutionProviders& execution_providers) {
  for (auto& provider : execution_providers) {
    ...
    if (iter != provider_type_to_registry_.end()) {
      return ORT_MAKE_STATUS(ORT_FAIL, "found duplicated provider ", provider->Type(),
...
Ort::Status KernelRegistryManager::SearchKernelRegistry(const Node& node, const Ort::Logger& logger,
  ...
  if (ptype.empty()) {
    return ORT_MAKE_STATUS(ORT_FAIL, create_error_message("The node is not placed on any Execution Provider. "));
  }
  ...
  return ORT_MAKE_STATUS(ORT_NOT_IMPLEMENTED, create_error_message("Failed to find kernel for "));
}
