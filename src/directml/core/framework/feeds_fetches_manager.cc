...
Ort::Status FeedsFetchesInfo::MapNamesToMLValueIdxs(gsl::span<const std::string> names,
...
Ort::Status FeedsFetchesInfo::SetMLValueIdxs(const OrtValueNameIdxMap& ort_value_name_idx_map) {
  auto status = MapNamesToMLValueIdxs(feed_names, ort_value_name_idx_map, feeds_mlvalue_idxs);
  if (!status.IsOK()) {
    return ORT_MAKE_STATUS(ORT_FAIL, "Error mapping feeds: " + status.GetErrorMessage());
  }

  status = MapNamesToMLValueIdxs(output_names, ort_value_name_idx_map, fetches_mlvalue_idxs);
  if (!status.IsOK()) {
    return ORT_MAKE_STATUS(ORT_FAIL, "Error mapping output names: " + status.GetErrorMessage());
  }

  return status;
}

Ort::Status FeedsFetchesManager::Create(gsl::span<const std::string> feed_names,
...
Ort::Status FeedsFetchesManager::Create(gsl::span<const std::string_view> feed_names,
                                   gsl::span<const std::string> output_names,
                                   const OrtValueNameIdxMap& ort_value_name_idx_map,
