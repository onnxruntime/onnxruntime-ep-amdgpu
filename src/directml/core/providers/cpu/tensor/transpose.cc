...
static void DoTransposeImpl(int64_t num_axes, gsl::span<const int64_t> target_dims,
  ...
  size_t blocksize = num_elts_in_block * element_size;
  MultiIndex mindex;
  IncrementIndexAndComputeOffsetSetup(mindex, gsl::narrow<size_t>(num_axes), target_dims, stride, element_size);
...
static void DoTransposeImpl(int64_t num_axes, gsl::span<const int64_t> target_dims,
  ...
  MultiIndex mindex;
  IncrementIndexAndComputeOffsetSetup(mindex, gsl::narrow<size_t>(num_axes), target_dims, stride, 1);
...
static bool TypedDoTransposeEltWise(int64_t num_axes, gsl::span<const int64_t> target_dims, size_t num_blocks,
  ...
  if (enabled) {
    MultiIndex mindex;
    IncrementIndexAndComputeOffsetSetup(mindex, gsl::narrow<size_t>(num_axes), target_dims, stride, sizeof(T));
...
// copies source tensor to target, transposing elements.
// The stride vector indicates the transposition.
Ort::Status DoTransposeEltWise(int64_t num_axes, gsl::span<const int64_t> target_dims, size_t num_blocks,
                          const gsl::span<const size_t>& stride, const uint8_t* source, uint8_t* target,
                          size_t element_size) {
  bool enabled = false;
  switch (element_size) {
  ...
  return enabled ? STATUS_OK
                 : ORT_MAKE_STATUS(ORT_FAIL, "Transpose of element size not supported in this build. Size=",
                                   element_size);
}
...
static Ort::Status DoUntypedTranspose(const gsl::span<const size_t>& permutations, const Tensor& input, Tensor& output,
  ...
  for (size_t i = 0; i < rank; i++) {
    ...
    if (inpdim + 1 < rank)
      stride[i] = gsl::narrow<size_t>(input_shape.SizeFromDimension(inpdim + 1));
    else
      stride[i] = 1;
  }
  ...
  for (int64_t i = rank - 1; i >= 0; --i) {
    int64_t input_axis = gsl::narrow<int64_t>(permutations[gsl::narrow<size_t>(i)]);
    if (is_suffix && (input_axis == i)) {
      suffix_blocksize *= static_cast<size_t>(input_dims[gsl::narrow<size_t>(input_axis)]);
    } else {
      is_suffix = false;
      prefix_blocksize *= static_cast<size_t>(input_dims[gsl::narrow<size_t>(input_axis)]);
      ++num_axes_in_prefix;
    }
  }
  ...
  if (is_string_type) {
    ...
    if (string_enabled) {
      ...
      status = ORT_MAKE_STATUS(ORT_FAIL, "Transpose of std::string is not supported in this build.");
    }
  } else {
...
static Ort::Status TransposeImpl(const gsl::span<const size_t>& permutations, const Tensor& input, Tensor& output,
...
static Ort::Status UnpackSubByteTensor(const Tensor& src, Tensor& dst, AllocatorPtr cpu_allocator) {
...
template <typename SubByteType>
static Ort::Status DoTransposeSubByte(const gsl::span<const size_t>& permutations, const Tensor& input, Tensor& output,
                                 const TensorShape* input_shape_override, concurrency::ThreadPool* tp) {
  using UnpackedType = typename SubByteType::UnpackedType;
...
//`input_shape_override` overrides the shape of `input` for compute purposes.
Ort::Status TransposeBase::DoTranspose(const gsl::span<const size_t>& permutations, const Tensor& input, Tensor& output,
                                  const TensorShape* input_shape_override, concurrency::ThreadPool* tp) {
  auto input_type = input.DataType();
  auto output_type = output.DataType();

  if (input_type != output_type) {
    return ORT_MAKE_STATUS(ORT_FAIL, "Mismatched data types between input and output Tensors. ",
...
Ort::Status Transpose::Compute(OpKernelContext* ctx) const {
  ...
  Ort::Status status = ComputeOutputShape(X, output_dims, default_perm, p_perm);
  if (!status.IsOK())
    return status;
