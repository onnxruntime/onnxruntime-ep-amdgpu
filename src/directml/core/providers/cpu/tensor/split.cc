...
Ort::Status SplitBase::PrepareForCompute(const TensorShape& input_shape, int num_outputs, int64_t& axis, int& before_dims,
  ...
  after_dims_excluding_split = (axis + 1 == num_dimensions)
                                   ...
                                   : gsl::narrow<int>(input_shape.SizeFromDimension(SafeInt<size_t>(axis) + 1));

  if (num_outputs_ != -1) {
    if (num_outputs_ > split_dim_size) {
      return ORT_MAKE_STATUS(ORT_FAIL, "Invalid num_outputs value of ", num_outputs_,
    ...
    int32_t size = gsl::narrow<int32_t>(std::ceil(float(split_dim_size) / num_outputs));
  ...
  if (split_sizes.empty()) {
    ...
    if (split_dim_size % static_cast<size_t>(num_outputs) != 0) {
      return ORT_MAKE_STATUS(ORT_FAIL, "Input cannot be split evenly on selected axis. Input shape=", input_shape,
...
Ort::Status SplitImpl::Compute(OpKernelContext* context) const {
