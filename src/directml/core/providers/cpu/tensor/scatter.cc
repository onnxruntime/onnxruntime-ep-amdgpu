  ...
  for (int64_t index = 0; index < num_indices;) {
    ...
    for (size_t i = 0; i < num_dims; ++i) {
      if (i == size_t(axis)) {
        ...
        dst_offset += gsl::narrow<size_t>(axis_idx * dim_block_size[narrow<size_t>(i)]);
      } else {
        dst_offset += gsl::narrow<size_t>(dim_counters[narrow<size_t>(i)] * dim_block_size[narrow<size_t>(i)]);
      }
    }
...
struct ScatterDataDispatchTarget {
  Ort::Status operator()(const Tensor* data_input, const std::vector<int64_t>& indices_data, const Tensor* updates_input, int64_t axis,
...
template <typename EnabledDataTypes>
Ort::Status Scatter<EnabledDataTypes>::Compute(OpKernelContext* context) const {
  ...
  if (data_input->DataType() != updates_input->DataType()) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "data type is different from updates type");
  }
  ...
  for (size_t i = 0; i < indices_dims.size(); ++i) {
    if (indices_dims[i] != updates_dims[i]) {
      return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Indices vs updates dimensions differs at position=", i,
  ...
  if (input_dims.size() != indices_dims.size()) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Indices must have the same rank as Input. Indices rank=",
  ...
  for (size_t i = 0; i < input_dims.size(); ++i) {
    ...
    if (static_cast<int64_t>(i) != axis && input_dims[i] < indices_dims[i]) {
      return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Indices dim=", indices_dims[i], " at pos=", i,
  ...
  if (index_type == utils::ToTensorProtoElementType<int32_t>()) {
    ...
    status = ORT_MAKE_STATUS(ORT_FAIL, "Indices type is not supported.");
  }
  ...
  status = dispatcher.template InvokeRet<Ort::Status, ScatterDataDispatchTarget>(
...
Ort::Status GatherElementsGradImpl(const Tensor* indices_input, const Tensor* updates_input,
...
#define GATHER_ELEMENTS_GRAD_IMPL_SPECIALIZED(Tin, Tdata) \
  template Ort::Status GatherElementsGradImpl<Tin, Tdata>(     \
