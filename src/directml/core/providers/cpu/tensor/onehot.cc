...
Ort::Status ValidateInputs(const Tensor* depth, const Tensor* values) {
  // validation scenarios
  // depth should be scalar and > 0
  if (!depth->Shape().IsScalar()) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Invalid argument for depth; it's not a scalar.");
  }
...
Ort::Status PrepareOutputShape(const Tensor* indices, const int64_t depth_val, const int64_t axis,
  ...
  for (int64_t i = 0; i < true_axis; ++i) {
    prefix_dim_size *= indices_dims[gsl::narrow<size_t>(i)];
  }
...
Ort::Status OneHotOp<in_type, out_type, depth_type>::Compute(OpKernelContext* p_op_kernel_context) const {
  ...
  if (depth_val <= 0) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Depth is negative.");
  }
  ...
  adjusted_indices.reserve(gsl::narrow<size_t>(indices_size));
