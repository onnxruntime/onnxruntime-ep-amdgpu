...
class GatherElements final : public OpKernel {
 public:
  ...
  static Ort::Status ValidateInputShapes(const TensorShape& input_data_shape,
                                    const TensorShape& indices_shape,
                                    int64_t axis);  // axis might be different from the member axis_ based on the input being processed

 private:
  Ort::Status CoreImplString(const Tensor* input_tensor, const Tensor* indices_tensor,
                        Tensor* output_tensor, int64_t axis) const;

  Ort::Status CoreImpl(const Tensor* input_tensor, const Tensor* indices_tensor,
                  Tensor* output_tensor, int64_t axis) const;
