...
class TransposeBase {
  ...
  */
  static Ort::Status DoTranspose(const gsl::span<const size_t>& permutations, const Tensor& input, Tensor& output,
 ...
 protected:
  TransposeBase(const OpKernelInfo& info) {
    ...
    Ort::Status status = info.GetAttrs<int64_t>("perm", temp_perm);
    if (status.IsOK()) {
  ...
  Ort::Status ComputeOutputShape(const Tensor& X, TensorShapeVector& output_dims, InlinedVector<size_t>& default_perm,
