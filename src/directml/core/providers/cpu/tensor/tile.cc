...
Ort::Status TileCoreForFixedSizeTypes(const Tensor& input_tensor, Tensor& output_tensor, const int64_t* repeats, TensorAxisCounters& input_counters, const TensorPitches& output_pitches, size_t element_size) {
  ...
  while (input_counters) {
    ...
    while (input_counters.Increment()) {
      ptrdiff_t pitch = gsl::narrow<size_t>(output_pitches[input_counters.Axis()] * input_shape[input_counters.Axis()]);
...
Ort::Status TileCoreForStringType(const Tensor& input_tensor, Tensor& output_tensor, const int64_t* repeats, TensorAxisCounters& input_counters, const TensorPitches& output_pitches) {
  ...
  while (input_counters) {
    ...
    // Tile data for other axes
    while (input_counters.Increment()) {
      block_size = gsl::narrow<size_t>(output_pitches[input_counters.Axis()] * input_shape[input_counters.Axis()]);
      copy = output - block_size;
      num_repeats = repeats[input_counters.Axis()] - 1;
      for (int64_t repeat = 0; repeat < num_repeats; ++repeat) {
        output = std::copy(copy, copy + block_size, output);
...
bool IsTileMemcpy(const TensorShape& input_shape,
  ...
  for (int64_t i = static_cast<int64_t>(rank) - 1; i >= 0; --i) {
    if (repeats[i] != 1) {
      if (input_shape.SizeToDimension(gsl::narrow<size_t>(i)) == 1) {
        num_of_copies_per_batch = 1;
        for (int64_t j = 0; j <= i; ++j) {
          num_of_copies_per_batch *= gsl::narrow<size_t>(repeats[gsl::narrow<size_t>(j)]);
        }
        ...
        num_of_copies_per_batch = gsl::narrow<size_t>(repeats[gsl::narrow<size_t>(i)]);
        num_of_batch_copies = gsl::narrow<size_t>(repeats[0]);
...
Ort::Status Tile::Compute(OpKernelContext* ctx) const {
  const auto* tensor_pointer = ctx->Input<Tensor>(0);
  if (tensor_pointer == nullptr) return ORT_MAKE_STATUS(ORT_FAIL, "Input count of Tile OP mismatch, the first one is empty");
  ...
  if (tensor_pointer == nullptr) return ORT_MAKE_STATUS(ORT_FAIL, "Input count of Tile OP mismatch, the second one is empty");
  const Tensor& repeats_tensor = *tensor_pointer;
  if (repeats_tensor.Shape().NumDimensions() != 1)
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "'repeat' input tensor must be 1 dimensional");
  if (size_t(repeats_tensor.Shape().Size()) != input_rank)
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "'repeat' input tensor must have the same length as the 'input' tensor");
