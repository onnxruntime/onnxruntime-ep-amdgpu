...
class UpsampleBase {
 ...
 protected:
  ...
  [[nodiscard]] Ort::Status ScalesValidation(gsl::span<const float> scales, const UpsampleMode mode) const {
    if (!is_resize_) {
  ...
  ParseScalesData(const Tensor* scale, InlinedVector<float>& scales, int64_t rank) const {
    ...
    if (scales.empty()) {
      scales.resize(gsl::narrow<size_t>(scales_size));
    }
  ...
  void ParseRoiData(const Tensor* roi, InlinedVector<float>& roi_array) const {
    ...
    if (roi_size > 0) {
      roi_array.resize(gsl::narrow<size_t>(roi_size));
  ...
  [[nodiscard]] Ort::Status ParseSizesData(const Tensor* sizes, TensorShapeVector& output_dims,
  ...
  [[nodiscard]] Ort::Status ParseScalesDataAndAdjustOutputSize(TensorShapeVector& output_dims,
