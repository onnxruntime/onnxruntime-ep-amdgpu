...
void SetupUpsampleFilterAntiAlias(FilterParamsAntiAlias<T>& p,
  ...
  auto compute_weight_coefficients = [&alloc, roi, &get_original_coordinate, exclude_outside](
    ...
    int32_t window_size = gsl::narrow<int32_t>(ceilf(support)) * 2 + 1;
    const size_t scale_buffer_size = gsl::narrow<size_t>(window_size * output_size);
    ...
    for (int32_t i = 0; i < output_size; i++) {
      ...
      if (center - 0.5f < 0 || center - 0.5f > gsl::narrow<float>(input_size - 1)) {
...
void ComputeInterpolationAtLevel1(int64_t num_channels, int64_t input_height, int64_t input_width,
  ...
  concurrency::ThreadPool::TrySimpleParallelFor(
      tp, gsl::narrow<std::ptrdiff_t>(num_channels),
      [&](std::ptrdiff_t c) {
        ...
        if (output_width == input_width) {
          std::copy_n(Xdata_span.begin() + gsl::narrow<size_t>(x_start), gsl::narrow<size_t>(output_height * output_width),
                      Ydata_span.begin() + gsl::narrow<size_t>(y_start));
          return;
        }

        for (size_t y = 0; y < gsl::narrow<size_t>(output_height); ++y) {
          auto* Ydata_offset = Ydata + output_width * y;
          auto* bound = p_dim.bound.data();
          for (size_t x = 0; x < gsl::narrow<size_t>(output_width); ++x) {
            ...
            if constexpr (is_8bit_v<InputType>) {
              ...
              *Ydata_offset++ = gsl::narrow<int32_t>(std::round(output));
            } else {
              *Ydata_offset++ = output;
            }
          }
  ...
  if (num_channels > 2 && num_channels >= tp->DegreeOfParallelism(tp)) {
    ...
    concurrency::ThreadPool::TryParallelFor(
        ...
        [&](std::ptrdiff_t first, std::ptrdiff_t last) {
          if (output_height == input_height) {
            auto workload_in_thread = gsl::narrow<size_t>(last) - gsl::narrow<size_t>(first);
            std::copy_n(Xdata_span.begin() + gsl::narrow<size_t>(first * input_width), gsl::narrow<size_t>(workload_in_thread * output_width),
                        Ydata_span.begin() + gsl::narrow<size_t>(first * output_width));
            return;
          }
          ...
          for (auto start = first; start != last; start++) {
            ...
            const auto* y_bound = p_dim.bound.data();
            const auto* weight_coeff = p_dim.weight_coefficients.get() + p_dim.window_size * y;
            int64_t ymin = y_bound[2 * gsl::narrow<size_t>(y)];
            int64_t ymax = y_bound[2 * gsl::narrow<size_t>(y) + 1];
            auto* Ydata_offset = Ydata + output_width * y;
            for (size_t x = 0; x < gsl::narrow<size_t>(output_width); ++x) {
              AccumulateType output = is_8bit_v<InputType> ? ConstValue::mag_factor : 0;
              auto* weight_coeff_start = weight_coeff;
      ...
      [&](std::ptrdiff_t nc) {
        ...
        for (int64_t z = 0; z < output_depth && p.dim_y.out_of_bound_idx.size() > 0; ++z) {
          for (int64_t y : p.dim_y.out_of_bound_idx) {
            InputType* Ydata_offset = Ydata_base_nc + (z * output_height + y) * output_width;
            std::fill_n(Ydata_offset, gsl::narrow<size_t>(output_width), static_cast<InputType>(extrapolation_value));
          }
        }

        for (int64_t z : p.dim_z.out_of_bound_idx) {
          InputType* Ydata_offset = Ydata_base_nc + z * output_height * output_width;
          std::fill_n(Ydata_offset, gsl::narrow<size_t>(output_height * output_width), static_cast<InputType>(extrapolation_value));
        }
      });
}
  ...
  for (int64_t n = 0; n < batch_size; ++n) {
    {
      ...
      auto ydata_span = gsl::make_span(image_temp_buffer.get(), gsl::narrow<size_t>(input_height * num_channels * output_width));
