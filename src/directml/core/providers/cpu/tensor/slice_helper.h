...
inline Ort::Status PrepareForComputeHelper(const gsl::span<const int64_t>& raw_starts,
  ...
  for (size_t axis_index = 0; axis_index < axes_count; ++axis_index) {
    ...
    if (axis >= static_cast<int64_t>(dimension_count) || axis < 0)
      return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "'axes' has an axis outside of the tensor dimension count");
    auto p = unique_axes.insert(axis);
    if (!p.second)
      return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "'axes' has duplicates");
    ...
    compute_metadata.ends_[gsl::narrow<size_t>(axis)] = std::clamp(end, int64_t{0}, dim_value);

    // find output dim value for this axis
    const auto temp = compute_metadata.ends_[gsl::narrow<size_t>(axis)] - compute_metadata.starts_[gsl::narrow<size_t>(axis)];
    if (temp < 0)
...
inline Ort::Status PrepareForComputeHelper(const gsl::span<const int64_t>& raw_starts,
  ...
  for (size_t axis_index = 0; axis_index < axes_count; ++axis_index) {
    ...
    if (step == 0)
      return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "'step' value cannot be 0");
