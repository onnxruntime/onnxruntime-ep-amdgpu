...
Ort::Status ComputeMeanVarianceNormalization2D(size_t M, size_t N,
  ...
  const auto idx_M = gsl::narrow<Eigen::Index>(M), idx_N = gsl::narrow<Eigen::Index>(N);
...
Ort::Status MeanVarianceNormalization::Compute(OpKernelContext* context) const {
  ...
  const size_t M = gsl::narrow<size_t>(compute_shape.SizeToDimension(num_unspecified_axes)),
               N = gsl::narrow<size_t>(compute_shape.SizeFromDimension(num_unspecified_axes));
