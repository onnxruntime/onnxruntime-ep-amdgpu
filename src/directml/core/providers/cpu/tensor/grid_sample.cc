...
//
template <typename T>
Ort::Status GridSample<T>::Compute(OpKernelContext* context) const {
  ...
  if (data_dims == 2) {
    ...
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Only support GirdSample in 4-D or 5-D cases.");
  }
  return STATUS_OK;
}
