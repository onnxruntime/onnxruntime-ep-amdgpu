...
Ort::Status UnsqueezeBase::PrepareCompute(OpKernelContext* ctx, Prepare& p) const {
  ...
  for (int64_t axis : axes) {
    ...
    axis = HandleNegativeAxis(axis, gsl::narrow<int64_t>(output_dims.size()));
    if (axis < 0 || axis >= static_cast<int64_t>(output_dims.size()))
      return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "'axes' has an out of range axis");
    if (output_dims[gsl::narrow<size_t>(axis)] != 0)
      return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "'axes' has a duplicate axis");
    output_dims[gsl::narrow<size_t>(axis)] = 1;
  }
...
Ort::Status Unsqueeze::Compute(OpKernelContext* ctx) const {
  Prepare p;
