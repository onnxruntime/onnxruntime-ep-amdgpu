...
Ort::Status Compress::Compute(OpKernelContext* ctx) const {
  ...
  int64_t compress_input_length = has_axis_ ? input_dimensions[gsl::narrow<size_t>(axis)] : input_tensor->Shape().Size();
  ...
  if (has_axis_) {
    output_dims[gsl::narrow<size_t>(axis)] = positive_condition_count;
  } else {
  ...
  if (has_axis_) {
    ...
    int64_t axes_included_right_stride = axes_right_stride * input_dimensions[gsl::narrow<size_t>(axis)];
