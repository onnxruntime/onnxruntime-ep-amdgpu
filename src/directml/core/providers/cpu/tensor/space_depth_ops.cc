...
Ort::Status SpaceToDepth::Compute(OpKernelContext* context) const {
  const auto* tensor_pointer = context->Input<Tensor>(0);
  if (tensor_pointer == nullptr) return ORT_MAKE_STATUS(ORT_FAIL, "input count mismatch");
  ...
  if (input.IsDataType<float>()) {
    SpaceDepthOpCpuImpl<float>(input, output, permutation,
                               gsl::narrow<ptrdiff_t>(batch),
                               gsl::narrow<std::ptrdiff_t>(input_depth),
                               gsl::narrow<std::ptrdiff_t>(input_height / blocksize_),
                               gsl::narrow<std::ptrdiff_t>(blocksize_),
                               gsl::narrow<std::ptrdiff_t>(input_width / blocksize_),
                               gsl::narrow<std::ptrdiff_t>(blocksize_),
                               gsl::narrow<ptrdiff_t>(blocksize_),
                               gsl::narrow<ptrdiff_t>(blocksize_),
                               gsl::narrow<std::ptrdiff_t>(input_depth),
                               gsl::narrow<std::ptrdiff_t>(input_height / blocksize_),
                               gsl::narrow<std::ptrdiff_t>(input_width / blocksize_));
    ...
    return ORT_MAKE_STATUS(ORT_FAIL, "Unsupported input type in SpaceToDepth op: ", input.DataType());
  }

  return STATUS_OK;
}

Ort::Status DepthToSpace::Compute(OpKernelContext* context) const {
  const auto* tensor_pointer = context->Input<Tensor>(0);
  if (tensor_pointer == nullptr) return ORT_MAKE_STATUS(ORT_FAIL, "input count mismatch");
  const Tensor& input = *tensor_pointer;
  ...
  if (input.IsDataType<float>()) {
    SpaceDepthOpCpuImpl<float>(input, output, permutation,
                               gsl::narrow<std::ptrdiff_t>(batch),
                               gsl::narrow<std::ptrdiff_t>(dim1),
                               gsl::narrow<std::ptrdiff_t>(blocksize_),
                               gsl::narrow<std::ptrdiff_t>(dim3),
                               gsl::narrow<std::ptrdiff_t>(input_height),
                               gsl::narrow<std::ptrdiff_t>(input_width),
                               gsl::narrow<std::ptrdiff_t>(input_depth / blocksize_ / blocksize_),
                               gsl::narrow<std::ptrdiff_t>(input_height),
                               gsl::narrow<std::ptrdiff_t>(blocksize_),
                               gsl::narrow<std::ptrdiff_t>(input_width),
                               gsl::narrow<std::ptrdiff_t>(blocksize_));
    ...
    // user will not see this as the kernel doesn't claim support for types other than float and double
    return ORT_MAKE_STATUS(ORT_FAIL, "Unsupported input type in DepthToSpace op: ", input.DataType());
  }
