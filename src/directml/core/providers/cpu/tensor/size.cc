...
Ort::Status Size::Compute(OpKernelContext* ctx) const {
  const auto* input_tensor = ctx->Input<Tensor>(0);
  if (input_tensor == nullptr) return ORT_MAKE_STATUS(ORT_FAIL, "input count mismatch");
...
// Opset 23 added the float4e2m1 type.
