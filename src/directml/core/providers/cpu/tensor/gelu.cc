...
template <typename T>
Ort::Status Gelu<T>::Compute(OpKernelContext* context) const {
  ...
  if (approximation_algorithm_ == "tanh") {
    ...
    concurrency::ThreadPool::TryBatchParallelFor(
        ...
        [&](ptrdiff_t task_idx) {
          ...
          MlasComputeTanh(p_output, p_output, gsl::narrow<size_t>(count));
    ...
    concurrency::ThreadPool::TryBatchParallelFor(
        ...
        [&](ptrdiff_t task_idx) {
          ...
          MlasComputeErf(p_output, p_output, gsl::narrow<size_t>(count));
  ...
  return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Unsupported approximation_algorithm: ", approximation_algorithm_);
}
