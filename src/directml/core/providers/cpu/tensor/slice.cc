...
Ort::Status SliceBase::FlattenOutputDims(gsl::span<const int64_t> input_dimensions, gsl::span<const int64_t> output_dims,
...
Ort::Status SliceBase::PrepareForCompute(gsl::span<const int64_t> raw_starts, gsl::span<const int64_t> raw_ends,
...
Ort::Status SliceBase::FillVectorsFromInput(const Tensor& start_tensor,
                                       const Tensor& ends_tensor,
                                       const Tensor* axes_tensor,
                                       const Tensor* steps_tensor,
                                       TensorShapeVector& input_starts,
  ...
  const auto size = gsl::narrow<size_t>(start_tensor.Shape().Size());
...
template <typename T>
static Ort::Status SliceImpl(OpKernelContext* ctx,
...
Ort::Status SliceBase::Compute(OpKernelContext* ctx) const {
  ...
  if (input_dimensions.empty()) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Cannot slice scalars");
  }
  ...
  if (!supported) {
    status = ORT_MAKE_STATUS(ORT_FAIL, "Unsupported input data type of ", input_tensor.DataType());
  }
