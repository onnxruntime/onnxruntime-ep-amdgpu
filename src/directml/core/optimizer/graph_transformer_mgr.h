...
class GraphTransformerManager {
 public:
  ...
  Ort::Status SetSteps(unsigned steps);

  // Get the maximum number of graph transformation steps
  Ort::Status GetSteps(unsigned& steps) const;
  ...
  Ort::Status Register(std::unique_ptr<GraphTransformer> transformer, TransformerLevel level);

  // Apply all transformers registered for the given level on the given graph
  Ort::Status ApplyTransformers(Graph& graph, TransformerLevel level, const Ort::Logger& logger) const;
