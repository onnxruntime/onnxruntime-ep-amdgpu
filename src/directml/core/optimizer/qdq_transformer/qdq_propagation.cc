...
Ort::Status ValidateQDQInsertionEdges(Graph& graph, gsl::span<const ExtendedGraphEdge> insertion_edges) {
...
void LogQDQInsertion(const Ort::Logger& logger, const CodeLocation& code_location,
...
Ort::Status InsertQDQPairs(Graph& graph, gsl::span<const ExtendedGraphEdge> insertion_edges,
  ...
  LogQDQInsertion(logger, ORT_WHERE, graph, insertion_edges);
...
Ort::Status PropagateDQForward(Graph& graph, gsl::span<const NodeIndex> node_indices,
...
Ort::Status PropagateQBackward(Graph& graph, gsl::span<const NodeIndex> node_indices,
                          const InlinedHashSet<std::string_view>& compatible_eps,
                          const Ort::Logger& logger,
                          bool& modified) {
  for (auto node_index : node_indices) {
...
Ort::Status QDQPropagationTransformer::ApplyImpl(Graph& graph, bool& modified, int graph_level,
