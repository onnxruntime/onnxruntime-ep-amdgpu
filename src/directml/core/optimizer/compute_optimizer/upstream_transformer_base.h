...
class UpStreamGraphTransformerBase : public GraphTransformer {
 ...
 protected:
   ...
   */
  virtual std::optional<T1> IsSupportedForUpstream(Graph& graph, Node& node, const Ort::Logger& logger) const = 0;
  ...
  virtual bool UpStreamInternal(Graph& graph, std::deque<T1>& queue,
                                ...
                                const Ort::Logger& logger) const = 0;
 ...
 private:
   ...
   */
  bool Upstream(Graph& graph, std::deque<T1>& queue, Node& current_node, T1& info, const Ort::Logger& logger) const;
};
