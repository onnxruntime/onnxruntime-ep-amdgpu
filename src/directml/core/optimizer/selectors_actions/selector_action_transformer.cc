...
static Ort::Status MatchAndProcess(
    Graph& graph, const GraphViewer& graph_viewer, Node& node, bool& modified, const Ort::Logger& logger,
...
Ort::Status SelectorActionTransformer::ApplySelectorsAndActions(
...
static Ort::Status SetOpSinceVersionForProducedNodes(NodeIndex pre_action_max_num_nodes,
...
Ort::Status SelectorActionTransformer::ApplySavedRuntimeOptimizations(
    Graph& graph, bool& modified, int graph_level, const Ort::Logger& logger) const {
  ...
  for (const auto& record : records) {
    ...
    if (!selector_action_entry) {
      return ORT_MAKE_STATUS(ORT_FAIL, "Missing action ", record.action_id, " for transformer ", Name());
    }
...
Ort::Status SelectorActionTransformer::ApplyImpl(Graph& graph, bool& modified, int graph_level,
