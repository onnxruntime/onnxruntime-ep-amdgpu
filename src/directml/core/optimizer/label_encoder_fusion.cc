 ...
 */
bool LabelEncoderFusion::SatisfyCondition(const Graph& graph, const Node& node, const Ort::Logger& /*logger*/) const {
...
*/
template <typename T1, typename T2, typename T3>
Ort::Status LabelEncoderFusion::ApplyHelper(
    Graph& graph,
    Node& node,
    Node& next_node,
...
Ort::Status LabelEncoderFusion::Apply(Graph& graph, Node& node, RewriteRuleEffect& rule_effect, const Ort::Logger& /*logger*/) const {
