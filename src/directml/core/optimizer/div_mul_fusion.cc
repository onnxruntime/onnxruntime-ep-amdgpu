...
1 / x1 *  x2 -> x2 / x1
 */
bool DivMulFusion::SatisfyCondition(const Graph& graph, const Node& node, const Ort::Logger&) const {
...
Ort::Status DivMulFusion::Apply(Graph& graph, Node& node, RewriteRuleEffect& rule_effect, const Ort::Logger&) const {
  auto& div_node = node;
