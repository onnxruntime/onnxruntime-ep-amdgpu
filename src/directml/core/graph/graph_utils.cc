...
static bool CanUpdateImplicitInputNameInSubgraphs(const Graph& graph,
                                                  ...
                                                  const std::string& new_arg_name, const Ort::Logger& logger) {
...
Ort::Status ConvertInMemoryDataToInline(Graph& graph, const std::string& name) {
  const ONNX_NAMESPACE::TensorProto* initializer = nullptr;
...
bool CanRemoveNode(const Graph& graph, const Node& node, const Ort::Logger& logger) {
...
bool FindPath(const Node& node, bool is_input_edge, gsl::span<const EdgeEndToMatch> edges_to_match,
              std::vector<const Node::EdgeEnd*>& result, const Ort::Logger& logger) {
  result.clear();
...
bool FindPath(Graph& graph, const Node& node, bool is_input_edge, gsl::span<const EdgeEndToMatch> edges_to_match,
              std::vector<std::reference_wrapper<Node>>& result, const Ort::Logger& logger) {
  result.clear();
