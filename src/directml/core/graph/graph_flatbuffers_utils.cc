...
Ort::Status SaveInitializerOrtFormat(flatbuffers::FlatBufferBuilder& builder,
  ...
  if (has_string_data) {
    ...
    if (external_writer && unpacked_tensor.size() >= kMinimumSizeForExternalData) {
      ...
      external_data_offset = gsl::narrow<int64_t>(offset);  // offset in fb is int64_t so -1 can mark not in use
    } else {
...
Ort::Status SaveSparseInitializerOrtFormat(flatbuffers::FlatBufferBuilder& builder,
...
Ort::Status SaveAttributeOrtFormat(flatbuffers::FlatBufferBuilder& builder,
...
size_t GetSizeInBytesFromFbsTensor(const fbs::Tensor& tensor) {
  ...
  auto num_elements = std::accumulate(fbs_dims->cbegin(), fbs_dims->cend(), 1,
                                      std::multiplies<>());
...
Ort::Status LoadInitializerOrtFormat(const fbs::Tensor& fbs_tensor, TensorProto& initializer,
...
#if !defined(DISABLE_SPARSE_TENSORS)
Ort::Status LoadSparseInitializerOrtFormat(const fbs::SparseTensor& fbs_sparse_tensor,
...
Ort::Status LoadAttributeOrtFormat(const fbs::Attribute& fbs_attr,
...
Ort::Status SaveOrtTensorOrtFormat(
  ...
  if (external_data_writer) {
    ...
    int64_t external_data_offset = gsl::narrow<int64_t>(offset);
    tb.add_external_data_offset(external_data_offset);
  } else {
    tb.add_raw_data(raw_data);
  }
...
struct UnpackTensorWithType {
  Ort::Status operator()(const ONNX_NAMESPACE::TensorProto& tensor_proto, const fbs::Tensor& fbs_tensor,
    ...
    if (fbs_tensor.external_data_offset() >= 0) {
      ...
      return ORT_MAKE_STATUS(ORT_FAIL, "Invalid tensor. Expected: raw data or external data offset. Actual: ",
...
Ort::Status LoadOrtTensorOrtFormat(const fbs::Tensor& fbs_tensor, const AllocatorPtr allocator,
  ...
  return dispatcher.InvokeRet<Ort::Status, UnpackTensorWithType>(unused_tensor_proto, fbs_tensor, ort_tensor, external_data_reader);
}
