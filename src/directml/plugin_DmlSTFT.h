...
enum PluginDmlSTFTKernelInputIndex : uint32_t
{
    Signal,
    FrameStep,
    Window,
...
struct PluginDmlSTFTParameters
{
        ...
        const OperatorHelper::IShapeInformationAdapter& shapeInfo)
    {
        ...
        // input 0: signal (required; tensor)
        {
            ...
            uint32_t rank = shapeInfo.GetInputTensorDimensionCount(PluginDmlSTFTKernelInputIndex::Signal);
            ML_CHECK_VALID_ARGUMENT(rank == 2 || rank == 3, "Signal shape must be 2D or 3D.");

            auto dims = shapeInfo.GetInputTensorShape(PluginDmlSTFTKernelInputIndex::Signal);
        ...
        {
            MLOperatorTensor tensor = kernelInfo.GetConstantInputTensor(PluginDmlSTFTKernelInputIndex::FrameStep);
            this->frameStep = gsl::narrow<uint32_t>(OperatorHelper::ReadScalarTensorCastToInt64(tensor));

            ML_CHECK_VALID_ARGUMENT(this->frameStep > 0, "The frame_step must be greater than 0.");
        }

        // input 2: window (optional; tensor)
        if (kernelInfo.IsInputValid(PluginDmlSTFTKernelInputIndex::Window))
        {
            uint32_t rank = shapeInfo.GetInputTensorDimensionCount(PluginDmlSTFTKernelInputIndex::Window);
            ML_CHECK_VALID_ARGUMENT(rank == 1, "Window shape must be 1D.");

            auto shape = shapeInfo.GetInputTensorShape(PluginDmlSTFTKernelInputIndex::Window);
        ...
        if (kernelInfo.IsInputValid(PluginDmlSTFTKernelInputIndex::FrameLength))
        {
            MLOperatorTensor tensor = kernelInfo.GetConstantInputTensor(PluginDmlSTFTKernelInputIndex::FrameLength);
            uint32_t frameLength = gsl::narrow<uint32_t>(OperatorHelper::ReadScalarTensorCastToInt64(tensor));
...
namespace PluginDmlSTFTHelpers
{
...
private:
    ...
    struct
    {
        Microsoft::WRL::ComPtr<PluginGpuDFTOperator> op;
...
public:
    PluginDmlSTFTOperator(IMLOperatorKernelCreationContext* context)
    {
        ...
        PluginDmlSTFTParameters params(kernelInfo, shapeInfo);

        CompileAndInitFramingOperator(params);

        constexpr uint32_t dftAxis = 1;
        constexpr bool dftIsInverse = false;
        m_dftOperator.op = wil::MakeOrThrow<PluginGpuDFTOperator>(
            m_d3dDevice.Get(),
            dftAxis,
            params.isOnesided,
            dftIsInverse,
    ...
    void CompileAndInitFramingOperator(const PluginDmlSTFTParameters& params)
    {
            ...
            )
        {
            ...
            bufferDesc.totalTensorSizeInBytes = DMLCalcBufferTensorSize(
                bufferDesc.dataType,
                gsl::narrow<uint32_t>(bufferDesc.sizes.size()),
    ...
    STDMETHOD(Compute)(IMLOperatorKernelContext* context)
    {
        try
        {
            ...
            Microsoft::WRL::ComPtr<ID3D12Resource> outputResource = PluginDmlSTFTHelpers::GetOutputResourceFromKernelContext(context, 0);
    ...
    void DispatchFramingOperator(ID3D12GraphicsCommandList* commandList, IMLOperatorKernelContext* context, ID3D12Resource* outputResource)
    {
        ...
        Microsoft::WRL::ComPtr<ID3D12Resource> signalResource = PluginDmlSTFTHelpers::GetInputResourceFromKernelContext(context, PluginDmlSTFTKernelInputIndex::Signal);
        ...
        {
            windowResource = PluginDmlSTFTHelpers::GetInputResourceFromKernelContext(context, PluginDmlSTFTKernelInputIndex::Window);
        ...
        if (tempBufferSize > 0)
        {
            ORT_THROW_IF_FAILED(context->AllocateTemporaryData(gsl::narrow<size_t>(tempBufferSize), &tempBuffer));
...
public:
        ...
        IMLOperatorKernel** kernel)
    {
        try
        {
            auto dftOperator = wil::MakeOrThrow<PluginDmlSTFTOperator>(context);
    ...
    static void RegisterSTFTKernel(IMLOperatorRegistry* registry)
    {
        ...
        auto shareInferrer = wil::MakeOrThrow<PluginSTFTShapeInferrer>();
        auto factory = wil::MakeOrThrow<PluginDmlSTFTOperatorFactory>();
