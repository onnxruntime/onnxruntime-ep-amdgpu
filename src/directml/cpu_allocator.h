...
struct CpuAllocator : BaseAllocator {
    ...
    static void* ORT_API_CALL AllocImpl(struct OrtAllocator* /*this_*/, size_t size) { return malloc(size); }

    /// Free a block of memory previously allocated with OrtAllocator::Alloc
    static void ORT_API_CALL FreeImpl(struct OrtAllocator* /*this_*/, void* p) { return free(p); }
    ...
    static const struct OrtMemoryInfo* ORT_API_CALL InfoImpl(const struct OrtAllocator* this_) {
        const CpuAllocator& impl = *static_cast<const CpuAllocator*>(this_);
