...
namespace OperatorHelper
{
        ...
        std::vector<DimensionType>& outputShape)
    {
        ...
        // Broadcast the matrix dimensions if needed for DirectML GEMM
        // If Input1's K dimension (second-to-last) is 1 but Output expects a larger K,
        // we need to expand it to match. The TensorDesc will handle creating stride-0 for broadcasting.
        //
        // Example: A=[8,256,256], B=[8,1,64], Output=[8,256,64]
        // After batch broadcast: A=[8,256,256], B=[8,1,64]
        // We need to expand B's middle dim from 1 to 256 to match A's K dimension
        // Result: A=[8,256,256], B=[8,256,64] with B having stride-0 for middle dim
        if (inputShape1.size() >= 2 && outputShape.size() >= 2) {
            // Extract the K dimensions: A's last dim and B's second-to-last dim
            uint32_t kDimA = inputShape0[inputShape0.size() - 1];
            uint32_t kDimB = inputShape1[inputShape1.size() - 2];

            // If B's K is 1 and A's K is > 1, broadcast B's K to match A
            if (kDimB == 1 && kDimA > 1) {
                inputShape1[inputShape1.size() - 2] = kDimA;
            }
        }
    }
        ...
        )
    {
        ...
        int raw_axis = operatorAttributes.GetOptionalAttribute<int>(AttrName::Axis, -1);
        m_axis = static_cast<int>(HandleNegativeAxis(raw_axis, inputDimCount));
