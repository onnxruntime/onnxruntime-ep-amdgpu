...
template <typename T>
Ort::Status NonZero<T>::Compute(OpKernelContext* context) const {
  ...
  const Eigen::Index coordinate_size = X_shape.IsScalar() ? 1 : gsl::narrow<Eigen::Index>(X_shape.NumDimensions());
  ...
  if (X_shape.IsScalar()) {
    ...
    for (size_t i = 0, end = gsl::narrow<size_t>(X_shape.Size()); i < end; ++i) {
      const T& value = *data++;
      if (value != T{}) {
  ...
  const Eigen::Index num_non_zero_values = gsl::narrow<Eigen::Index>(non_zero_indices_buffer.size()) / coordinate_size;
