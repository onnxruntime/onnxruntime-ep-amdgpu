...
template <typename T>
Ort::Status Expand<T>::Compute(OpKernelContext* context) const {
    ...
    if (*input_shape_iter != *output_shape_iter) {
      if (1 == *output_shape_iter) {
        *output_shape_iter = *input_shape_iter;
        ...
        return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "invalid expand shape");
      }
    }
    input_shape_iter++;
    output_shape_iter++;
  ...
  std::unique_ptr<int64_t[]> input_dim_group = std::make_unique<int64_t[]>(gsl::narrow<size_t>(max_dims_size));
  std::unique_ptr<int64_t[]> output_dim_group = std::make_unique<int64_t[]>(gsl::narrow<size_t>(max_dims_size));
  std::unique_ptr<int64_t[]> expand_dim_size = std::make_unique<int64_t[]>(gsl::narrow<size_t>(max_dims_size));
  ...
  for (int64_t input_dims_iter = input_dims_size - 1,
    ...
    if ((input_dim == 1 && output_dim > 1) || output_dims_iter == 0) {
      --dim_group_start;
      input_dim_group[gsl::narrow<size_t>(dim_group_start)] = input_count;
      output_dim_group[gsl::narrow<size_t>(dim_group_start)] = output_count;
      expand_dim_size[gsl::narrow<size_t>(dim_group_start)] = output_count / input_count / last_dim_size;
      last_dim_size *= expand_dim_size[gsl::narrow<size_t>(dim_group_start)];
    }
  }

  auto distribute_count = input_dim_group[gsl::narrow<size_t>(dim_group_start)] / input_dim_group[SafeInt<size_t>(max_dims_size) - 1];
  std::vector<int64_t> output_offsets(gsl::narrow<size_t>(distribute_count), 0);
      ...
      [&](ptrdiff_t i_start, ptrdiff_t i_end) {
        for (auto i = i_start; i < i_end; i++) {
          ...
          for (auto j = dim_group_start + 1, remains = input_offset; j < max_dims_size; ++j) {
            auto current_count = remains / input_dim_group[gsl::narrow<size_t>(j)];
            output_offset += current_count * output_dim_group[gsl::narrow<size_t>(j)];
            remains = remains % input_dim_group[gsl::narrow<size_t>(j)];
          }  // for j
          memcpy(output_data + output_offset, input_data + input_offset, gsl::narrow<size_t>(copy_byte));
          output_offsets[gsl::narrow<size_t>(i)] = output_offset;
        }  // for i
      };  // distribute_fn
  ...
  if (per_thread_tasks > 4) {
    concurrency::ThreadPool::TryParallelFor(
        ...
        gsl::narrow<ptrdiff_t>(distribute_count),
        static_cast<double>(copy_byte),
        std::move(distribute_fn));
  } else {
    distribute_fn(0, gsl::narrow<ptrdiff_t>(distribute_count));
  }  // else
  ...
  for (auto i = max_dims_size - 1; i >= dim_group_start; --i) {
    auto copy_fn =
        [&](ptrdiff_t j_start, ptrdiff_t j_end) {
          for (auto j = j_start; j < j_end; j++) {
            ...
            if (output_offset % output_dim_group[gsl::narrow<size_t>(i)] == 0) {
              auto copy_len = output_dim_group[gsl::narrow<size_t>(i)] / expand_dim_size[gsl::narrow<size_t>(i)];
              ...
              auto output_end = output_from + output_dim_group[gsl::narrow<size_t>(i)];
    ...
    if (per_thread_tasks > 20) {
      concurrency::ThreadPool::TryParallelFor(
          context->GetOperatorThreadPool(),
          gsl::narrow<std::ptrdiff_t>(distribute_count),
          static_cast<double>(copy_byte),
          std::move(copy_fn));
    } else {
      copy_fn(0, gsl::narrow<std::ptrdiff_t>(distribute_count));
    }  // else
  }  // for
  return STATUS_OK;
}  // Expand::compute
