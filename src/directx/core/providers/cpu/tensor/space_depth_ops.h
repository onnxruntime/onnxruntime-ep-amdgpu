...
class SpaceDepthBase {
 protected:
  ...
  Ort::Status InputValidationsAndOutputDimsCalc(const Tensor& input,
                                           int64_t& batch,
    ...
    if (input_shape.NumDimensions() != 4) {
      return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "SpaceDepth ops require a 4-D input. Provided rank: ",
    ...
    if (is_space_to_depth) {  // SpaceToDepth op
      if ((input_height % this->blocksize_) != 0) {
        return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "SpaceToDepth requires input height to be a multiple of block_size");
      }

      if ((input_width % this->blocksize_) != 0) {
        return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "SpaceToDepth requires input width to be a multiple of block_size");
      }
