...
template <typename T>
Ort::Status IsNaN<T>::Compute(OpKernelContext* context) const {
  const auto* X_ptr = context->Input<Tensor>(0);
  if (!X_ptr) {
    return ORT_MAKE_STATUS(ORT_FAIL, "Null input ptr");
  }
  auto& X = *X_ptr;
  auto& dims = X.Shape();
...
template <>
Ort::Status IsNaN<MLFloat16>::Compute(OpKernelContext* context) const {
  const auto* X_ptr = context->Input<Tensor>(0);
  ...
  EigenMap<bool>(Y) =
      ConstEigenVectorMap<Eigen::half>(static_cast<const Eigen::half*>(static_cast<const void*>(X_data)), gsl::narrow<size_t>(shape_size))
          .array()
          .isNaN();
...
template <>
Ort::Status IsNaN<BFloat16>::Compute(OpKernelContext* context) const {
  const auto* X_ptr = context->Input<Tensor>(0);
...
#if !defined(DISABLE_FLOAT8_TYPES)
template <>
Ort::Status IsNaN<Float8E4M3FN>::Compute(OpKernelContext* context) const {
  const auto* X = context->Input<Tensor>(0);
  auto& dims = X->Shape();
  auto& Y = *context->Output(0, dims);

  auto input = ConstEigenVectorMap<uint8_t>(static_cast<const uint8_t*>(static_cast<const void*>(X->Data<Float8E4M3FN>())), gsl::narrow<size_t>(dims.Size()));
...
template <>
Ort::Status IsNaN<Float8E4M3FNUZ>::Compute(OpKernelContext* context) const {
  const auto* X = context->Input<Tensor>(0);
  ...
  // 1.0000.000
  EigenMap<bool>(Y) =
      ConstEigenVectorMap<uint8_t>(static_cast<const uint8_t*>(static_cast<const void*>(X_data)), gsl::narrow<size_t>(shape_size)).array() == 0x80;

  return STATUS_OK;
}

template <>
Ort::Status IsNaN<Float8E5M2>::Compute(OpKernelContext* context) const {
  const auto* X = context->Input<Tensor>(0);
  auto& dims = X->Shape();
  auto& Y = *context->Output(0, dims);

  auto input = ConstEigenVectorMap<uint8_t>(static_cast<const uint8_t*>(static_cast<const void*>(X->Data<Float8E5M2>())), gsl::narrow<size_t>(dims.Size()));
  auto output = EigenMap<bool>(Y);
...
template <>
Ort::Status IsNaN<Float8E5M2FNUZ>::Compute(OpKernelContext* context) const {
  const auto* X = context->Input<Tensor>(0);
  ...
  // 1.0000.000
  EigenMap<bool>(Y) = ConstEigenVectorMap<uint8_t>(static_cast<const uint8_t*>(static_cast<const void*>(X_data)), gsl::narrow<size_t>(shape_size)).array() == 0x80;
