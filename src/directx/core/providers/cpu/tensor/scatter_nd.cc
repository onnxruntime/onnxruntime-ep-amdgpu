...
Ort::Status ScatterND::ValidateShapes(
  ...
  bool is_update_shape_invalid = [&]() {
    ...
    if (input_shape.Slice(gsl::narrow<size_t>(last_indice_dimension)) != update_shape.Slice(indice_rank - 1)) {
      return true;
    }
...
template <typename TData>
Ort::Status PrepareForCompute(OpKernelContext* context, Prepare<TData>& p) {
  ...
  std::vector<int64_t> element_counts(gsl::narrow<size_t>(last_indice_dimension), 0LL);  // Number of elements for each input dimension

  TensorPitches input_strides(input_shape);
  for (int64_t i = 0; i < last_indice_dimension; ++i) {
    element_counts[gsl::narrow<size_t>(i)] = input_strides[gsl::narrow<size_t>(i)];
  }

  p.element_to_copy = input_shape.SizeFromDimension(gsl::narrow<size_t>(last_indice_dimension));
  ...
  p.element_offsets.assign(gsl::narrow<size_t>(offset_count), 0LL);
  ...
  for (int64_t i = 0; i < offset_count; ++i) {
    for (int64_t j = 0; j < last_indice_dimension; ++j) {
      ...
      if (indice >= 0) {
        if (indice >= input_shape[gsl::narrow<size_t>(j)]) {
          return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "invalid indice found, indice = ", indice);
        }
      } else {
        if (indice < -input_shape[gsl::narrow<size_t>(j)]) {
          return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "invalid indice found, indice = ", indice);
        } else {
          indice += input_shape[gsl::narrow<size_t>(j)];
        }
      }

      p.element_offsets[gsl::narrow<size_t>(i)] += indice * element_counts[gsl::narrow<size_t>(j)];
    }
  }
  return STATUS_OK;
}
...
struct ScatterNDDispatchTarget {
  Ort::Status operator()(OpKernelContext* context, concurrency::ThreadPool* tp, ScatterND::Reduction reduction) const {
    ...
    auto lambda = [&](int64_t i) {
      switch (reduction) {
        case ScatterND::Reduction::Add: {
          ...
          func(
              prepare.output_base + prepare.element_offsets[gsl::narrow<size_t>(i)],
...
Ort::Status ScatterND::Compute(OpKernelContext* context) const {
  ...
  Ort::Status status = dispatcher.template InvokeRet<Ort::Status, ScatterNDDispatchTarget>(context, tp, this->reduction_);
  return status;
}
}  // namespace onnxruntime
