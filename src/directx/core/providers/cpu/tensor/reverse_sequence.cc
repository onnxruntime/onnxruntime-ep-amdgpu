...
template <typename T>
static Ort::Status ReverseSequenceImpl(const Tensor& X, Tensor& Y, gsl::span<const int64_t> sequence_lengths,
                                  int64_t max_seq_len, int64_t batch_size, int64_t input_size, bool time_major);

Ort::Status ReverseSequenceOp::Compute(OpKernelContext* context) const {
  ...
  if (seq_len_shape.NumDimensions() != 1 || seq_len_shape[0] != batch_size) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "sequence_lens shape must be {batch_size}. Got:",
...
template <typename T>
static Ort::Status ReverseSequenceImpl(const Tensor& X,
                                  Tensor& Y,
  ...
  for (int i = 0; i < batch_size; i++) {
    ...
    if (seq_len > max_seq_len || seq_len < 0) {
      return ORT_MAKE_STATUS(ORT_FAIL, "Invalid sequence length: ", seq_len,
                             ". Value must be in range [0,", max_seq_len, "]");
    }

    for (int64_t j = 0; j < seq_len; j++) {
      gsl::span<const T> src = inputs.subspan(gsl::narrow<size_t>(input_offset(max_seq_len, batch_size, input_size, i, j)), gsl::narrow<size_t>(input_size));
      gsl::span<T> dest = inputs_reverse.subspan(
          gsl::narrow<size_t>(reversed_output_offset(max_seq_len, batch_size, input_size, i, j, seq_len)), gsl::narrow<size_t>(input_size));
    ...
    for (int64_t j = seq_len; j < max_seq_len; j++) {
      ...
      gsl::span<const T> src = inputs.subspan(gsl::narrow<size_t>(offset), gsl::narrow<size_t>(input_size));
      gsl::span<T> dest = inputs_reverse.subspan(gsl::narrow<size_t>(offset), gsl::narrow<size_t>(input_size));
