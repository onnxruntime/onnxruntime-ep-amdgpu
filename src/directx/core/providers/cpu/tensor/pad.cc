...
Ort::Status PadBase::HandleDimValueZero(const Mode& mode, const TensorShape& input_shape, const TensorShape& output_shape) {
  switch (mode) {
    case Mode::Constant: {
    ...
    default:
      return ORT_MAKE_STATUS(ORT_FAIL, "Unexpected mode of ", static_cast<int>(mode));
  }
...
static void ComputePadWithAxes(
  ...
  for (size_t i = 0; i < axes_size; ++i) {
    const size_t axis = gsl::narrow<size_t>(HandleNegativeAxis(get_axis(i), data_rank));
...
void PadBase::ComputePads(OpKernelContext& ctx, size_t data_rank, gsl::span<const int64_t> pads_data,
  ...
  if (axes_tensor) {
    ...
    ORT_ENFORCE(pads_data.size() == gsl::narrow<size_t>(2 * num_axes),
...
template <typename T>
static Ort::Status PadInputWithDimValueOfZero(OpKernelContext* ctx,
                                         const Mode& mode,
...
template <typename T>
static Ort::Status PadImpl(OpKernelContext* ctx,
                      const PadsVector& pads,
                      const PadsVector& slices,
                      const Mode& mode,
                      T value) {
  if (!utils::HasTypeWithSameSize<AllEnabledPadTypes, T>()) {
    return ORT_MAKE_STATUS(ORT_FAIL, "Input data type not supported in this build.");
  }
  ...
  const size_t inner_no_pad_size = gsl::narrow<size_t>(output_dims[inner_axis] > 0
    ...
    case Mode::Constant:
      ...
      while (input_counters) {
        output += align_skip;
        {
          ...
          const size_t inner_extent = gsl::narrow<size_t>(effective_input_extents[inner_axis]);
        ...
        while (input_counters.Increment()) {
          ptrdiff_t inner_pitch = gsl::narrow<std::ptrdiff_t>(output_pitches[input_counters.Axis()]);
    ...
    case Mode::Edge:
      ...
      while (input_counters) {
        output += align_skip;
        {
          ...
          T* axis_start = output;
          T* axis_end = axis_start + gsl::narrow<ptrdiff_t>(inner_extent);
          ...
          if (inner_no_pad_size == 1) {
            ...
            if (pads[inner_axis] > 0) {
              PadAxis(sink, axis_start - static_cast<size_t>(pre_pad), axis_start, 1, -ptrdiff_t(inner_no_pad_size), inner_no_pad_size,
                      gsl::narrow<size_t>(pads[inner_axis]));
            }
            if (pads[inner_axis + data_rank] > 0) {
              PadAxis(sink, output, output - inner_no_pad_size, 1, -ptrdiff_t(inner_no_pad_size), inner_no_pad_size,
                      gsl::narrow<size_t>(pads[inner_axis + data_rank]));
            }
          }
...
Ort::Status Pad::Compute(OpKernelContext* ctx) const {
  ...
  switch (element_size) {
    ...
    case sizeof(uint8_t):
      ...
      break;
    default:
      pad_status = ORT_MAKE_STATUS(ORT_FAIL, "Unsupported input data type of ", data_type);
      break;
  }
  return pad_status;
}
