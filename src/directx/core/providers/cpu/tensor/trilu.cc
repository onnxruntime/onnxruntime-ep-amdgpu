...
template <typename T>
static Ort::Status TriluImpl(const Tensor* X, Tensor* Y, int64_t k_val, bool up) {
  ...
  for (int64_t i = 0; i < X_num_dims - 2; ++i) {
    batch_size *= X_shape[gsl::narrow<size_t>(i)];
  }
  ...
  for (int64_t b = 0; b < batch_size; b++) {  // can be parallelized if need to
    ...
    auto input_mat = ConstEigenMatrixMapRowMajor<T>(X_batch_data, gsl::narrow<std::ptrdiff_t>(matrix_h), gsl::narrow<std::ptrdiff_t>(matrix_w));
    auto output_mat = EigenMatrixMapRowMajor<T>(Y_batch_data, gsl::narrow<std::ptrdiff_t>(matrix_h), gsl::narrow<std::ptrdiff_t>(matrix_w));
    ...
    if (up) {
      ...
      for (int64_t i = start_i; i < matrix_h; i++) {
        for (int64_t j = 0; j < i + k_val && j < matrix_w; j++) {
          output_mat(gsl::narrow<std::ptrdiff_t>(i), gsl::narrow<std::ptrdiff_t>(j)) = 0;
        }
      }
    } else {
...
Ort::Status Trilu::Compute(OpKernelContext* ctx) const {
  Ort::Status status;
  ...
  if (X_num_dims < 2) {  // this is getting capture by shape inference code as well
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Input tensor should have a rank of at least 2");
  }
