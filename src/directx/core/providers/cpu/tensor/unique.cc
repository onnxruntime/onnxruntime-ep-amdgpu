...
Ort::Status Unique::Compute(OpKernelContext* context) const {
  ...
  if (input.IsDataType<double>())
    ...
    status = ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Unsupported tensor type of ", input.DataType());
...
class Subtensor {
 public:
  ...
  Subtensor(const gsl::span<const T>& data, const TensorShape& subtensor_shape,
    ...
    int64_t columns = subtensor_shape.SizeFromDimension(gsl::narrow<size_t>(axis));
    int64_t rows = subtensor_shape.SizeToDimension(gsl::narrow<size_t>(axis));
...
static void CreateFlattenedOutput(OpKernelContext& context,
  ...
  for (int64_t i = 0, end = num_unique; i < end; ++i, ++offsets_iter) {
    ...
    Y_data[gsl::narrow<size_t>(output_idx)] = offsets_iter->first;

    if (indices_out) {
      indices_data[gsl::narrow<size_t>(output_idx)] = indices[gsl::narrow<size_t>(unsorted_idx)].front();
    }

    if (counts) {
      counts_data[gsl::narrow<size_t>(output_idx)] = indices[gsl::narrow<size_t>(unsorted_idx)].size();
    }
  }
  ...
  if (inverse_indices) {
    if (sorted) {
      ...
      unsorted_to_sorted.resize(gsl::narrow<size_t>(num_unique));
      int64_t sorted_idx = 0;
      for (const auto& offset : offsets) {
        unsorted_to_sorted[gsl::narrow<size_t>(offset.second)] = sorted_idx++;
      }

      for (size_t i = 0, end = inverse_index.size(); i < end; ++i) {
        inverse_indices_data[gsl::narrow<size_t>(i)] = unsorted_to_sorted[gsl::narrow<size_t>(inverse_index[i])];
      }
    } else {
      for (size_t i = 0, end = inverse_index.size(); i < end; ++i) {
...
static void CreateOutput(OpKernelContext& context,
  ...
  // rows and columns for the slice along axis, flattened to 2D by merging the dimensions before and after the axis
  int64_t num_cols = subtensor_shape.SizeFromDimension(gsl::narrow<size_t>(axis));
  int64_t num_rows = subtensor_shape.SizeToDimension(gsl::narrow<size_t>(axis));
  ...
  for (int64_t i = 0, end = subtensor_dims.size(); i < end; ++i) {
    if (i == axis)
      ...
      Y_dims.push_back(subtensor_dims[gsl::narrow<size_t>(i)]);
  }
  ...
  for (int64_t i = 0, end = num_unique; i < end; ++i, ++offsets_iter) {
    ...
    for (int64_t row = 0; row < num_rows; ++row) {
      ...
      if (std::is_same<T, std::string>::value) {
        std::copy(item, item + gsl::narrow<size_t>(num_cols), &Y_data[gsl::narrow<size_t>(out_offset)]);
      } else {
        std::copy_n(item, gsl::narrow<size_t>(num_cols), &Y_data[gsl::narrow<size_t>(out_offset)]);
      }

      item += gsl::narrow<size_t>(num_cols);
  ...
  if (inverse_indices) {
    if (sorted) {
      ...
      for (size_t i = 0, end = inverse_index.size(); i < end; ++i) {
        inverse_indices_data[i] = unsorted_to_sorted[gsl::narrow<size_t>(inverse_index[i])];
      }
    } else {
      for (size_t i = 0, end = inverse_index.size(); i < end; ++i) {
        inverse_indices_data[i] = inverse_index[i];
...
template <typename T>
Ort::Status Unique::ComputeImpl(OpKernelContext& context) const {
  if (!utils::HasType<EnabledUniqueDataTypes, T>()) {
    return ORT_MAKE_STATUS(ORT_FAIL, "Data type is not supported in this build.");
  }
  ...
  if (flatten_) {
    ...
    for (int64_t i = 0, end = input.Shape().Size(); i < end; ++i) {
      auto entry = offsets.find(data[gsl::narrow<size_t>(i)]);
      if (entry == offsets.end()) {
        offsets[data[gsl::narrow<size_t>(i)]] = num_unique;
        inverse_index.push_back({num_unique});
        indices.push_back({i});
        ++num_unique;
      } else {
        size_t indices_idx = gsl::narrow<size_t>(entry->second);
        indices[indices_idx].push_back(i);
        inverse_index.push_back(gsl::narrow<int64_t>(indices_idx));
      }
    }
    ...
    subtensor_dims.reserve(gsl::narrow<size_t>(input_dims));
    for (int64_t i = 0; i < input_dims; ++i) {
      subtensor_dims.push_back(i == axis ? 1 : input_shape[gsl::narrow<size_t>(i)]);
    }
    ...
    int64_t num_unique = 0;
    int64_t n_axis = input_shape[gsl::narrow<size_t>(axis)];
