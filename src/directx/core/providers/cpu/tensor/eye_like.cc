...
struct ComputeDispatchTarget {
  void operator()(const int64_t k, Tensor& output) {
    ...
    auto output_mat = EigenMatrixMapRowMajor<T>(
        ...
        gsl::narrow<size_t>(output_shape[0]),
        gsl::narrow<size_t>(output_shape[1]));
    ...
    output_mat.diagonal(gsl::narrow<ptrdiff_t>(k)).array() = static_cast<T>(1);
  }
};
}  // namespace

Ort::Status EyeLike::Compute(OpKernelContext* context) const {
  const auto& T1 = context->RequiredInput<Tensor>(0);

  const auto& input_shape = T1.Shape();
  if (input_shape.NumDimensions() != 2) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "EyeLike : Input tensor dimension is not 2");
  }
