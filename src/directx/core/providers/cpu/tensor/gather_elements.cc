...
static void core_impl(const Tensor* input_tensor, const Tensor* indices_tensor, Tensor* output_tensor, int64_t axis,
  ...
  size_t num_inner_dim = gsl::narrow<size_t>(CalculateInnerDimCount(indices_shape));
  size_t inner_dim_size = gsl::narrow<size_t>(indices_shape[SafeInt<size_t>(input_rank) - 1]);
  const Tin* indices_data = indices_tensor->Data<Tin>();

  const TensorPitches input_shape_pitches(*input_tensor);
  int64_t axis_pitch = input_shape_pitches[gsl::narrow<size_t>(axis)];
  int64_t axis_size = input_tensor->Shape()[gsl::narrow<size_t>(axis)];
  ...
  auto MainLoop = [&](auto* output_data, auto* input_data) {
    auto BatchWork = [&](size_t inner_dim) {
      ORT_TRY {
        ...
        auto input = input_data + CalculateOffset(inner_dim, input_shape_pitches, gsl::narrow<size_t>(axis), indices_shape);
...
Ort::Status GatherElements::ValidateInputShapes(const TensorShape& input_data_shape,
                                           const TensorShape& indices_shape,
                                           int64_t axis) {
  ...
  for (int64_t i = 0; i < indices_rank; ++i) {
    ...
    if (i != axis) {
      if (indices_shape[gsl::narrow<size_t>(i)] < 0 || indices_shape[gsl::narrow<size_t>(i)] > input_data_shape[gsl::narrow<size_t>(i)])
        return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT,
                               "GatherElements op: 'indices' shape should have values within bounds of 'data' shape. "
                               "Invalid value in indices shape is: ",
                               indices_shape[gsl::narrow<size_t>(i)]);
    }
  }
...
Ort::Status GatherElements::Compute(OpKernelContext* context) const {
