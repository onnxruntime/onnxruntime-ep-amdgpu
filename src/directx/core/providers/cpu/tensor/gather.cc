...
Ort::Status GatherBase::PrepareForCompute(OpKernelContext* context, Prepare& p) const {
...
Ort::Status GatherCopyData(const Tensor* indices_tensor, const uint8_t* src_base, uint8_t* dst_base, bool is_string_type,
...
Ort::Status Gather::Compute(OpKernelContext* context) const {
  Prepare p;
  ...
  return ORT_MAKE_STATUS(ORT_NOT_IMPLEMENTED, "Gather Tind type not supported in this build.");
}
