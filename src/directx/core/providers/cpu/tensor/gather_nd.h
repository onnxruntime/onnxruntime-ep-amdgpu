...
class GatherNDBase {
 public:
  ...
  Ort::Status PrepareForCompute(const TensorShape& input_shape, const Tensor* indices_tensor,
...
class GatherND final : public OpKernel, protected GatherNDBase {
 ...
 private:
  Ort::Status GatherNumber(const Prepare& p, concurrency::ThreadPool* tp) const;
  Ort::Status GatherString(const Prepare& p, concurrency::ThreadPool* tp) const;
};
