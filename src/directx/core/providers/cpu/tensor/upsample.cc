...
static std::vector<int64_t> UpsampleNearestSetupRank1InputMapping(
  ...
  std::vector<int64_t> input_mapping(gsl::narrow<size_t>(length_resized));
...
UpsampleNearestSetupInputMappings(int64_t n_dim,
  ...
  for (int64_t axis = 0; axis < n_dim; ++axis) {
    ...
    input_mapping.resize(gsl::narrow<size_t>(output_shape[narrow<size_t>(axis)]));
...
template <typename T>
static Ort::Status UpsampleNearestImpl(const T* input,
                                  T* output,
                                  const TensorShape& input_shape,
                                  const TensorShape& output_shape,
                                  gsl::span<const float> scales,
  ...
  std::vector<int64_t> output_dim_counter(gsl::narrow<size_t>(n_dim));
...
static Ort::Status ValidateUpsampleInput(const void* input, const void* output,
...
template <typename T>
static Ort::Status UpsampleNearest(const T* input,
                              T* output,
                              const TensorShape& input_shape,
                              const TensorShape& output_shape,
                              gsl::span<const float> scales,
...
static Ort::Status UpsampleLinearImpl(const std::function<void(size_t, size_t, float)>& apply,
                                 const TensorShape& input_shape,
                                 const TensorShape& output_shape,
...
template <typename T>
static Ort::Status UpsampleLinear(const T* input,
                             T* output,
                             const TensorShape& input_shape,
                             const TensorShape& output_shape,
                             const vector<float>& scales,
...
template <typename T>
Ort::Status Upsample<T>::BaseCompute(OpKernelContext* context,
                                gsl::span<const float> roi,
                                gsl::span<const float> scales,
  ...
  switch (mode_) {
    ...
    case UpsampleMode::LINEAR: {
      ...
      if (dims.size() == 2 || dims.size() == 4) {
        ...
        return ORT_MAKE_STATUS(ORT_FAIL, (is_resize_ ? "Resize" : "Upsample"),
    ...
    default:
      return ORT_MAKE_STATUS(ORT_FAIL, is_resize_ ? "Resize: unexpected mode" : "Upsample: unexpected mode");
  }
}

template <typename T>
Ort::Status Upsample<T>::Compute(OpKernelContext* context) const {
  const auto* X = context->Input<Tensor>(0);
