...
Ort::Status ConcatBase::PrepareForCompute(OpKernelContext* ctx,
  ...
  for (size_t index = 0; index < input_count; ++index) {
    ...
    if (num_elements > 0) {
      ...
      reference_tensor_index = gsl::narrow<int>(index);
  ...
  p.axis = static_cast<uint64_t>(HandleNegativeAxis(axis_, gsl::narrow<int64_t>(!is_stack_
                                                                                            ? reference_rank
                                                                                            : reference_rank + 1)));
  ...
  if (!is_stack_) {  // 'Concat' mode
    ...
    for (size_t index = 0; index < input_count; index++) {
      concat_axis_size += gsl::narrow<size_t>(input_tensors[index]->Shape()[gsl::narrow<size_t>(p.axis)]);
    }

    output_dims[gsl::narrow<size_t>(p.axis)] = gsl::narrow<int64_t>(concat_axis_size);
  } else {  // 'Stack' mode
...
Ort::Status ConcatBase::ComputeImpl(Prepare& p, OpKernelContext* ctx) const {
  ...
  for (int input_index = 0; input_index < input_count; input_index++) {
    ...
    auto status = DispatchStridedCopy<EnabledDataTypes>(ctx->GetOperatorThreadPool(),
                                                        *p.output_tensor,
                                                        gsl::narrow<ptrdiff_t>(initial_output_offset),
    ...
    if (is_stack_) {
      initial_output_offset += output_strides_full[gsl::narrow<size_t>(p.axis)];
    } else {
      initial_output_offset += prep.tensor->Shape()[gsl::narrow<size_t>(p.axis)] * output_strides_for_copy[gsl::narrow<size_t>(p.axis)];
    }
  }
...
Ort::Status Concat::Compute(OpKernelContext* ctx) const {
