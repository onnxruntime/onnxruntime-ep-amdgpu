...
Ort::Status GatherNDBase::PrepareForCompute(const TensorShape& input_shape, const Tensor* indices_tensor,
                                       const int64_t bytes_per_value, Prepare& p, concurrency::ThreadPool* tp) const {
  const auto& indices_shape = indices_tensor->Shape();
  if (indices_shape.NumDimensions() == 0) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "indices tensor must has rank larger than 0");
  }
  ...
  std::vector<int64_t> sizes_from_slice_dims(gsl::narrow<size_t>(num_slice_dims));
  for (int64_t i = 0; i < num_slice_dims; ++i) {
    sizes_from_slice_dims[gsl::narrow<size_t>(i)] = input_shape.SizeFromDimension(SafeInt<size_t>(batch_dims_) + i + 1);
  }
  ...
  p.slice_offsets.assign(gsl::narrow<size_t>(num_slices), 0LL);

  // Compute the element_offset
  auto lambda = [&](int64_t slice_idx) {
    const size_t batch_idx = gsl::narrow<size_t>(slice_idx / num_slices_per_batch);
    ...
    for (int64_t dim_idx = 0; dim_idx < num_slice_dims; ++dim_idx) {
      ...
      relative_slice_offset += SafeInt<size_t>(index) * sizes_from_slice_dims[gsl::narrow<size_t>(dim_idx)];
    }

    p.slice_offsets[gsl::narrow<size_t>(slice_idx)] = static_cast<uint64_t>(input_base_offset) + relative_slice_offset;
  };

  concurrency::ThreadPool::TryParallelFor(
      tp, gsl::narrow<size_t>(num_slices), static_cast<double>(num_slice_dims),
  ...
  return err_index == 0 ? STATUS_OK
                        : ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "invalid index found, index = ", err_index);
}

template Ort::Status GatherNDBase::PrepareForCompute<int32_t>(const TensorShape&,
                                                         const Tensor*,
                                                         const int64_t,
                                                         Prepare&,
                                                         concurrency::ThreadPool*) const;
template Ort::Status GatherNDBase::PrepareForCompute<int64_t>(const TensorShape&,
                                                         const Tensor*,
                                                         const int64_t,
                                                         Prepare&,
                                                         concurrency::ThreadPool*) const;

Ort::Status GatherND::Compute(OpKernelContext* context) const {
  ...
  shape.insert(shape.end(), input_shape.GetDims().begin() + gsl::narrow<std::ptrdiff_t>(last_indices_dimension),
  ...
  if (indices_tensor->IsDataType<int32_t>()) {
    ...
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "indices tensor data type not supported");
  }

  return nullptr == p.input_str_base ? GatherNumber(p, tp) : GatherString(p, tp);
}

Ort::Status GatherND::GatherNumber(const Prepare& p, concurrency::ThreadPool* tp) const {
  auto lambda = [&](int64_t slice_idx) {
    memcpy(p.output_base + slice_idx * p.bytes_per_slice, p.input_base + p.slice_offsets[gsl::narrow<size_t>(slice_idx)] * p.element_bytes,
           gsl::narrow<size_t>(p.bytes_per_slice));
  };
  concurrency::ThreadPool::TryParallelFor(
...
Ort::Status GatherND::GatherString(const Prepare& p, concurrency::ThreadPool* tp) const {
  auto lambda = [&](int64_t slice_idx) {
    ...
    for (int64_t j = 0; j < static_cast<int64_t>(p.element_count_per_slice); ++j) {
      p.output_str_base[slice_base_offset + j] = p.input_str_base[p.slice_offsets[gsl::narrow<size_t>(slice_idx)] + j];
    }
  };
  concurrency::ThreadPool::TryParallelFor(
