...
struct DeviceHelpers {
  using ZeroData = std::function<Ort::Status(void* data, size_t size_in_bytes)>;
  using Transpose = std::function<Ort::Status(const gsl::span<const size_t>& permutations,
...
class Scan : public controlflow::IControlFlowKernel {
 public:
  ...
  Ort::Status Compute(OpKernelContext* ctx) const override;

  Ort::Status SetupSubgraphExecutionInfo(const SessionState& session_state,
 ...
 private:
  ...
  TensorShapeVector input_directions_;
  TensorShapeVector output_directions_;
  TensorShapeVector input_axes_;
  TensorShapeVector output_axes_;
