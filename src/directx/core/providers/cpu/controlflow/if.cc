...
Ort::Status If::SetupSubgraphExecutionInfo(const SessionState& session_state,
...
Ort::Status If::Compute(OpKernelContext* ctx) const {
...
Ort::Status IfImpl::AllocateOutputTensors() {
    ...
    if (graph_output_type->has_tensor_type()
      ...
      if (graph_output_shape) {
        ...
        if (output_shape.Size() < 0) {
          ...
          if (!tensor)
            return ORT_MAKE_STATUS(ORT_FAIL, "Failed to create output tensor for ", graph_output->Name());
...
Ort::Status IfImpl::Execute(const FeedsFetchesManager& ffm) {
  Ort::Status status =STATUS_OK;
  ...
  for (int i = 0; i < info_.num_outputs; ++i) {
    ...
    if (outputs_[i].first == AllocationType::Delayed) {
      ...
      fetch_allocators[i] = [this, i, &fetches](const TensorShape& shape, const OrtDevice& location,
        ...
        if (!tensor)
          return ORT_MAKE_STATUS(ORT_FAIL, "Failed to create output tensor for If output ", i);
