...
template <typename T>
Ort::Status Add<T>::Compute(OpKernelContext* context) const {
...
template <>
Ort::Status Add<MLFloat16>::Compute(OpKernelContext* context) const {
  ProcessBroadcastSpanFuncs funcs{
      [](BroadcastHelper& per_iter_bh) {
...
template <typename T>
Ort::Status Sub<T>::Compute(OpKernelContext* context) const {
  ProcessBroadcastSpanFuncs funcs{
      [](BroadcastHelper& per_iter_bh) {
        per_iter_bh.OutputEigen<T>() = per_iter_bh.ScalarInput0<T>() - per_iter_bh.EigenInput1<T>().array();
      },
...
template <>
Ort::Status Sub<MLFloat16>::Compute(OpKernelContext* context) const {
  ProcessBroadcastSpanFuncs funcs{
      [](BroadcastHelper& per_iter_bh) {
        per_iter_bh.OutputEigen<Eigen::half>() = per_iter_bh.ScalarInput0<Eigen::half>() - per_iter_bh.EigenInput1<Eigen::half>().array();
      },
...
template <typename T>
Ort::Status Mul<T>::Compute(OpKernelContext* context) const {
  ProcessBroadcastSpanFuncs funcs{
      [](BroadcastHelper& per_iter_bh) {
        per_iter_bh.OutputEigen<T>() = per_iter_bh.ScalarInput0<T>() * per_iter_bh.EigenInput1<T>().array();
      },
...
template <>
Ort::Status Mul<MLFloat16>::Compute(OpKernelContext* context) const {
  ProcessBroadcastSpanFuncs funcs{
      [](BroadcastHelper& per_iter_bh) {
        per_iter_bh.OutputEigen<Eigen::half>() = per_iter_bh.ScalarInput0<Eigen::half>() * per_iter_bh.EigenInput1<Eigen::half>().array();
      },
...
template <typename T>
Ort::Status Div<T>::Compute(OpKernelContext* context) const {
  ProcessBroadcastSpanFuncs funcs{
      [](BroadcastHelper& per_iter_bh) {
        per_iter_bh.OutputEigen<T>() = per_iter_bh.ScalarInput0<T>() / per_iter_bh.EigenInput1<T>().array();
      },
...
template <>
Ort::Status Div<MLFloat16>::Compute(OpKernelContext* context) const {
  ProcessBroadcastSpanFuncs funcs{
      [](BroadcastHelper& per_iter_bh) {
        per_iter_bh.OutputEigen<Eigen::half>() = per_iter_bh.ScalarInput0<Eigen::half>() / per_iter_bh.EigenInput1<Eigen::half>().array();
      },
...
template <typename B>
Ort::Status DispatchOnBase(OpKernelContext& context, const Tensor& Y) {
  ...
  switch (Y.GetElementType()) {
    ...
    case on::TensorProto_DataType_DOUBLE:
      ...
      break;
    default:
      s = ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Unsupported Y type: ",
...
Pow::Compute(OpKernelContext* context) const {
  ...
  switch (X.GetElementType()) {
    ...
    case on::TensorProto_DataType_DOUBLE:
      ...
      break;
    default:
      s = ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Unsupported X type: ",
...
template <typename T>
Ort::Status Sum_6<T>::Compute(OpKernelContext* ctx) const {
...
template <typename T>
Ort::Status Sum_8<T>::Compute(OpKernelContext* context) const {
...
template <>
Ort::Status Min_6<float>::Compute(OpKernelContext* ctx) const {
...
struct Min_8::ComputeImpl {
  Ort::Status operator()(const Min_8& inst, OpKernelContext* context) const {
    const auto typed_allocator = [](const TensorAllocator& tensor_allocator, const TensorShape& shape) {
      return tensor_allocator.Allocate<T>(shape);
    };
...
static Ort::Status MinMaxMLFloat16(const OpKernel& inst, OpKernelContext* context) {
  const auto typed_allocator = [](const TensorAllocator& tensor_allocator, const TensorShape& shape) {
...
Ort::Status Min_8::Compute(OpKernelContext* context) const {
  ...
  switch (dt_type) {
    ...
    default:
      utils::MLTypeCallDispatcher<float, double, int8_t, int32_t, uint32_t,
                                  int64_t, uint8_t, uint64_t>
          t_disp(dt_type);
      return t_disp.InvokeRet<Ort::Status, ComputeImpl>(*this, context);
  }
}

template <>
Ort::Status Max_6<float>::Compute(OpKernelContext* ctx) const {
  auto inputCount = Node().InputArgCount().front();
  ORT_ENFORCE(inputCount >= 1, "Must have 1 or more inputs");
  auto& data_0 = *ctx->Input<Tensor>(0);
  auto& shape = data_0.Shape();
...
struct Max_8::ComputeImpl {
  Ort::Status operator()(const Max_8& inst, OpKernelContext* context) const {
    const auto typed_allocator = [](const TensorAllocator& tensor_allocator, const TensorShape& shape) {
      return tensor_allocator.Allocate<T>(shape);
    };
...
Ort::Status Max_8::Compute(OpKernelContext* context) const {
  auto dt_type = context->Input<Tensor>(0)->GetElementType();
...
Ort::Status Not::Compute(OpKernelContext* context) const {
...
Ort::Status And::Compute(OpKernelContext* context) const {
...
Ort::Status Or::Compute(OpKernelContext* context) const {
...
Ort::Status Xor::Compute(OpKernelContext* context) const {
...
template <typename T>
Ort::Status Equal<T>::Compute(OpKernelContext* context) const {
  ProcessBroadcastSpanFuncs funcs{
      [](BroadcastHelper& per_iter_bh) {
...
template <typename T>
Ort::Status Less<T>::Compute(OpKernelContext* context) const {
  ProcessBroadcastSpanFuncs funcs{
      [](BroadcastHelper& per_iter_bh) {
...
template <typename T>
Ort::Status Greater<T>::Compute(OpKernelContext* context) const {
  ProcessBroadcastSpanFuncs funcs{
      [](BroadcastHelper& per_iter_bh) {
        per_iter_bh.OutputEigen<bool>() = per_iter_bh.EigenInput1<T>().array() < per_iter_bh.ScalarInput0<T>();
      },
...
template <typename T>
Ort::Status LessOrEqual<T>::Compute(OpKernelContext* context) const {
  ProcessBroadcastSpanFuncs funcs{
      [](BroadcastHelper& per_iter_bh) {
        per_iter_bh.OutputEigen<bool>() = per_iter_bh.EigenInput1<T>().array() >= per_iter_bh.ScalarInput0<T>();
      },
...
template <typename T>
Ort::Status GreaterOrEqual<T>::Compute(OpKernelContext* context) const {
  ProcessBroadcastSpanFuncs funcs{
      [](BroadcastHelper& per_iter_bh) {
        per_iter_bh.OutputEigen<bool>() = per_iter_bh.EigenInput1<T>().array() <= per_iter_bh.ScalarInput0<T>();
      },
...
template <>
Ort::Status Mean_6<float>::Compute(OpKernelContext* ctx) const {
  auto inputCount = Node().InputArgCount().front();
  ORT_ENFORCE(inputCount >= 1, "Must have 1 or more inputs");
  auto& data_0 = *ctx->Input<Tensor>(0);
  auto& shape = data_0.Shape();
...
template <>
Ort::Status Mean_8<float>::Compute(OpKernelContext* context) const {
  const auto typed_allocator = [](const TensorAllocator& tensor_allocator, const TensorShape& shape) {
...
template <typename T>
Ort::Status BitShift<T>::Compute(OpKernelContext* context) const {
  ProcessBroadcastSpanFuncs funcs{
      [](BroadcastHelper& per_iter_bh) {
...
template <typename T>
Ort::Status BitwiseAnd<T>::Compute(OpKernelContext* context) const {
  ProcessBroadcastSpanFuncs funcs{
      [](BroadcastHelper& per_iter_bh) {
        const T X = per_iter_bh.ScalarInput0<T>();
...
template <typename T>
Ort::Status BitwiseNot<T>::Compute(OpKernelContext* context) const {
  auto& input = *context->Input<Tensor>(0);
  auto& output = *context->Output(0, input.Shape());
...
template <typename T>
Ort::Status BitwiseOr<T>::Compute(OpKernelContext* context) const {
  ProcessBroadcastSpanFuncs funcs{
      [](BroadcastHelper& per_iter_bh) {
        const T X = per_iter_bh.ScalarInput0<T>();
        auto Y = per_iter_bh.SpanInput1<T>();
...
template <typename T>
Ort::Status BitwiseXor<T>::Compute(OpKernelContext* context) const {
  ProcessBroadcastSpanFuncs funcs{
      [](BroadcastHelper& per_iter_bh) {
        const T X = per_iter_bh.ScalarInput0<T>();
        auto Y = per_iter_bh.SpanInput1<T>();
...
template <>
Ort::Status PRelu<float>::Compute(OpKernelContext* context) const {
  ProcessBroadcastSpanFuncs funcs{
      [](BroadcastHelper& per_iter_bh) {
...
template <typename T>
Ort::Status Expand_8<T>::Compute(OpKernelContext* context) const {
  ProcessBroadcastSpanFuncs funcs{
      [](BroadcastHelper& per_iter_bh) {
...
template <>
Ort::Status Erf<float>::Compute(OpKernelContext* context) const {
  ...
  const std::ptrdiff_t elem_count = gsl::narrow<std::ptrdiff_t>(X->Shape().Size());
...
template <>
Ort::Status Erf<MLFloat16>::Compute(OpKernelContext* context) const {
  const auto* X = context->Input<Tensor>(0);
  const auto& x_shape = X->Shape();
  auto* Y = context->Output(0, x_shape);
  ...
  const auto narrow_task_count = gsl::narrow<std::ptrdiff_t>(task_count);
  ...
  concurrency::ThreadPool::TryBatchParallelFor(
      ...
      [&](ptrdiff_t task_idx) {
        ...
        const auto narrow_count = gsl::narrow<std::ptrdiff_t>(count);
...
class Mod final : public OpKernel {
 public:
  Mod(const OpKernelInfo& info) : OpKernel(info) {
    int64_t fmod = 0;
    Ort::Status s = info.GetAttr<int64_t>("fmod", &fmod);
    if (s.IsOK()) {
...
Ort::Status Mod::Compute(OpKernelContext* context) const {
  const auto& X = *context->Input<Tensor>(0);
