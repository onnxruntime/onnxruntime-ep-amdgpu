...
Ort::Status SequenceLength::Compute(OpKernelContext* context) const {
...
Ort::Status SequenceAt::Compute(OpKernelContext* context) const {
  const auto* X = context->Input<TensorSeq>(0);
  ...
  const Tensor& indexed_tensor = X->Get(gsl::narrow<size_t>(input_seq_idx));
...
Ort::Status SequenceEmpty::Compute(OpKernelContext* context) const {
...
Ort::Status SequenceInsert::Compute(OpKernelContext* context) const {
  ...
  Y->Reserve(num_tensors_input_seq + 1);
...
Ort::Status SequenceErase::Compute(OpKernelContext* context) const {
  const auto* S = context->Input<TensorSeq>(0);
  const auto* I = context->Input<Tensor>(1);
...
Ort::Status SequenceConstruct::Compute(OpKernelContext* context) const {
...
Ort::Status SplitToSequence::Compute(OpKernelContext* context) const {
...
Ort::Status SplitToSequence::PrepareForCompute(const TensorShape& input_shape, int64_t split_scalar, bool is_split_input_scalar,
  ...
  const int64_t split_dim_size = input_dims[gsl::narrow<size_t>(axis)];

  before_dims = gsl::narrow<int>(input_shape.SizeToDimension(gsl::narrow<size_t>(axis)));
  after_dims_including_split_axis = gsl::narrow<int>(input_shape.SizeFromDimension(gsl::narrow<size_t>(axis)));
  ...
  if (is_split_input_scalar) {
    ...
    split_sizes.resize(gsl::narrow<size_t>(num_outputs));
    std::fill(split_sizes.begin(), split_sizes.begin() + gsl::narrow<size_t>(num_even_splits), split_scalar);
    std::fill(split_sizes.begin() + gsl::narrow<size_t>(num_even_splits), split_sizes.end(), num_remaining_splits);
  } else {
...
static void GetSplitSizesInput(const Tensor& tensor, InlinedVector<int64_t>& split_sizes) {
  ...
  split_sizes.reserve(gsl::narrow<size_t>(num_elems));
  if (tensor.IsDataType<int32_t>()) {
...
Ort::Status SplitToSequence::ComputeImpl(OpKernelContext& context, const Tensor& input,
  ...
  for (int i = 0; i < num_outputs; ++i) {
    ...
    output_dimensions[gsl::narrow<size_t>(axis)] = split_size;
    ...
    if (is_string_type) {
      ...
      if (lda == N) {
        ...
        const auto lda_bytes_inc = lda * element_size;
        const auto ldb_bytes_inc = ldb * element_size;
    ...
    if (use_keep_dims && keepdims_ == 0) {
      ...
      for (int64_t idx = 0, end = static_cast<int64_t>(output_dimensions.size()); idx < end; ++idx) {
        if (idx != axis) {
          new_dims.push_back(output_dimensions[gsl::narrow<size_t>(idx)]);
        }
      }
