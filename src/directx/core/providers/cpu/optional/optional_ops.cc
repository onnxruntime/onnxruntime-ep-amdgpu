...
static Ort::Status PropagateInputOrtValueToFirstOutput(const OrtValue* input_ort_value,
                                                  OpKernelContext* ctx,
                                                  const DataTransferManager& data_transfer_mgr) {
...
Ort::Status Optional::Compute(OpKernelContext* ctx) const {
...
Ort::Status OptionalHasElement::Compute(OpKernelContext* ctx) const {
  const auto* input_ort_value = ctx->GetInputOrtValue(0);
...
Ort::Status OptionalGetElement::Compute(OpKernelContext* ctx) const {
  const auto* input_ort_value = ctx->GetInputOrtValue(0);
