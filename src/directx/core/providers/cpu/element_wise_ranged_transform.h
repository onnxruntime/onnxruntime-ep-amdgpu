...
inline Ort::Status GetFloatParam(const std::string& name, const onnxruntime::NodeAttributes& attributes,
                                    float& out) {
  ...
  if (attr->second.type() != ONNX_NAMESPACE::AttributeProto::AttributeType::AttributeProto_AttributeType_FLOAT) {
    return ORT_MAKE_STATUS(ORT_FAIL, "Attribute name and type don't match for '", name, "'");
  }
  out = attr->second.f();
  return STATUS_OK;
}
...
struct ElementWiseRangedTransform {
  ...
  static Ort::Status Create(const std::string& activation_type, const onnxruntime::NodeAttributes& attributes,
...
#define ORT_GET_FLOAT_ATTR_AND_RETURN(X)                           \
  float X;                                                         \
  Ort::Status Init(const onnxruntime::NodeAttributes& attributes) {     \
