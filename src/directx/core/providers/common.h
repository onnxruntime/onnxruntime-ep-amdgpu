...
constexpr int64_t ComputeOutputShape(const int64_t in_dim,
  ...
  int64_t dkernel_value = in_dim + pad_head + pad_tail - dkernel;
  return static_cast<int64_t>(static_cast<double>(dkernel_value) / static_cast<double>(stride) + 1.);
}
...
inline Ort::Status ComputePad(const int64_t in_dim,
  ...
  switch (pad_type) {
    ...
    case AutoPadType::SAME_LOWER: {
      if (1 != dilation)
        return Ort::Status{"Dilation not supported for "
                           "AutoPadType::SAME_UPPER or "
                           "AutoPadType::SAME_LOWER.", ORT_INVALID_ARGUMENT};
      ...
      int64_t actual_out_size = ComputeOutputShape(in_dim, stride, kernel, /*dilation*/ 1,
    ...
    default:
      return Ort::Status{"ComputePad: pad type not supported.", ORT_INVALID_ARGUMENT};
  }
...
/// </summary>
inline Ort::Status ComputeBroadcastOutputShape(const std::string& node_name,
  ...
  for (size_t i = 0; i < out_rank; ++i) {
    ...
    if (lhs_dim != out_dim && lhs_dim != 1)
      return ORT_MAKE_STATUS(ORT_FAIL, node_name, ": left operand cannot broadcast on dim ", lhs_rank - 1 - i,
                             " LeftShape: ", lhs_shape, ", RightShape: ", rhs_shape);
    if (rhs_dim != out_dim && rhs_dim != 1)
      return ORT_MAKE_STATUS(ORT_FAIL, node_name, ": right operand cannot broadcast on dim ", rhs_rank - 1 - i,
                             " LeftShape: ", lhs_shape, ", RightShape: ", rhs_shape);
