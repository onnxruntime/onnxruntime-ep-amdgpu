  ...
  do {                                                                    \
    ::onnxruntime::PrintFinalMessage(                                     \
        ::onnxruntime::OnnxRuntimeException(                              \
            ORT_WHERE, ::MakeString(__VA_ARGS__))            \
            .what());                                                     \
    abort();                                                              \
  } while (false)
...
#define ORT_NOT_IMPLEMENTED(...)                                                       \
  do {                                                                                 \
    ::onnxruntime::PrintFinalMessage(                                                  \
        ::onnxruntime::NotImplementedException(::MakeString(__VA_ARGS__)) \
            .what());                                                                  \
    abort();                                                                           \
  } while (false)
...
#define ORT_ENFORCE(condition, ...)                                                   \
  do {                                                                                \
    if (!(condition)) {                                                               \
      ::onnxruntime::PrintFinalMessage(                                               \
          ::onnxruntime::OnnxRuntimeException(ORT_WHERE, #condition,       \
...
#define ORT_THROW_EX(ex, ...)                                                                      \
  do {                                                                                             \
    ::onnxruntime::PrintFinalMessage(                                                              \
        ::MakeString(#ex, "(", ::MakeString(__VA_ARGS__), ")").c_str()); \
    abort();                                                                                       \
  } while (false)
...
#define ORT_THROW_WITH_CATEGORY_AND_CODE(category, code, ...)                       \
  do {                                                                              \
    ::onnxruntime::PrintFinalMessage(                                               \
        ::onnxruntime::OnnxRuntimeException(ORT_WHERE,                   \
                                            ::MakeString(__VA_ARGS__), \
...
// DO NOT use a printf format string, as that will not work as you expect.
#define ORT_THROW(...) \
  throw ::onnxruntime::OnnxRuntimeException(ORT_WHERE, ::MakeString(__VA_ARGS__))

// Just in order to mark things as not implemented. Do not use in final code.
#define ORT_NOT_IMPLEMENTED(...) \
  throw ::onnxruntime::NotImplementedException(::MakeString(__VA_ARGS__))
...
#define ORT_ENFORCE(condition, ...)                                                      \
  do {                                                                                   \
    if (!(condition)) {                                                                  \
      throw ::onnxruntime::OnnxRuntimeException(ORT_WHERE, #condition,                   \
                                                ::MakeString(__VA_ARGS__));              \
    }                                                                                    \
  } while (false)
...
#define ORT_THROW_FROM_STATUS(status)                                                                         \
  throw ::onnxruntime::OnnxRuntimeException{ORT_WHERE, status.GetErrorMessage()}

#define ORT_THROW_WITH_ERROR_CODE(code, ...)                                                                  \
  throw ::onnxruntime::OnnxRuntimeException(ORT_WHERE,                                                        \
                                            ::MakeString(__VA_ARGS__),                                        \
                                            code)
...
#define ORT_RETURN_IF(condition, ...)                                                                          \
  do {                                                                                                         \
    if (condition) {                                                                                           \
      return ::Ort::Status{::MakeString(ORT_WHERE.ToString(), " ", __VA_ARGS__).c_str(), ORT_FAIL};            \
    }                                                                                                          \
  } while (false)
...
#define ORT_RETURN_IF_ERROR_SESSIONID(expr, session_id)                                                       \
  do {                                                                                                        \
    if (const Ort::Status& _status{expr}; !_status.IsOK()) {                                                  \
      return ORT_MAKE_STATUS(_status.GetErrorCode(), _status.GetErrorMessage());                              \
    }                                                                                                         \
  } while (0)
