...
class Env {
 public:
   ...
   */
  virtual Ort::Status GetFileLength(_In_z_ const ORTCHAR_T* file_path, size_t& length) const = 0;
  virtual Ort::Status GetFileLength(int fd, /*out*/ size_t& file_size) const = 0;
   ...
   */
  virtual Ort::Status ReadFileIntoBuffer(_In_z_ const ORTCHAR_T* file_path, FileOffsetType offset, size_t length,
   ...
   */
  virtual Ort::Status MapFileIntoMemory(_In_z_ const ORTCHAR_T* file_path, FileOffsetType offset, size_t length,
  ...
  virtual Ort::Status CreateFolder(const std::wstring& path) const = 0;
  // Mainly for use with protobuf library
  virtual Ort::Status FileOpenRd(const std::wstring& path, /*out*/ int& fd) const = 0;
  // Mainly for use with protobuf library
  virtual Ort::Status FileOpenWr(const std::wstring& path, /*out*/ int& fd) const = 0;
#endif
  /// \brief Returns true if the directory exists.
  virtual bool FolderExists(const std::string& path) const = 0;
  virtual bool FileExists(const std::string& path) const = 0;
  /// \brief Recursively creates the directory, if it doesn't exist.
  virtual Ort::Status CreateFolder(const std::string& path) const = 0;
  // Recursively deletes the directory and its contents.
  // Note: This function is not thread safe!
  virtual Ort::Status DeleteFolder(const PathString& path) const = 0;
  // Mainly for use with protobuf library
  virtual Ort::Status FileOpenRd(const std::string& path, /*out*/ int& fd) const = 0;
  // Mainly for use with protobuf library
  virtual Ort::Status FileOpenWr(const std::string& path, /*out*/ int& fd) const = 0;
  // Mainly for use with protobuf library
  virtual Ort::Status FileClose(int fd) const = 0;

  /** Gets the canonical form of a file path (symlinks resolved). */
  virtual Ort::Status GetCanonicalPath(
  ...
  // function.
  virtual Ort::Status LoadDynamicLibrary(const PathString& library_filename, bool global_symbols, void** handle) const = 0;

  virtual Ort::Status UnloadDynamicLibrary(void* handle) const = 0;
  ...
  virtual Ort::Status GetSymbolFromLibrary(void* handle, const std::string& symbol_name, void** symbol) const = 0;
