...
class WindowsThread : public EnvThread {
 ...
 public:
  WindowsThread(const ORTCHAR_T* name_prefix, int index,
    ...
    if (gsl::narrow<size_t>(index) < thread_options.affinities.size()) {
...
Ort::Status WindowsEnv::GetFileLength(_In_z_ const ORTCHAR_T* file_path, size_t& length) const {
  wil::unique_hfile file_handle{
      CreateFile2(file_path, FILE_READ_ATTRIBUTES, FILE_SHARE_READ, OPEN_EXISTING, NULL)};
  if (file_handle.get() == INVALID_HANDLE_VALUE) {
    const auto error_code = GetLastError();
    return ORT_MAKE_STATUS(ORT_FAIL, "open file ", ToUTF8String(Basename(file_path)), " fail, errcode = ", error_code, " - ", std::system_category().message(error_code));
  }
  LARGE_INTEGER filesize;
  if (!GetFileSizeEx(file_handle.get(), &filesize)) {
    const auto error_code = GetLastError();
    return ORT_MAKE_STATUS(ORT_FAIL, "GetFileSizeEx ", ToUTF8String(Basename(file_path)), " fail, errcode = ", error_code, " - ", std::system_category().message(error_code));
  }
  if (static_cast<ULONGLONG>(filesize.QuadPart) > std::numeric_limits<size_t>::max()) {
    return ORT_MAKE_STATUS(ORT_FAIL, "GetFileLength: File is too large");
  }
  length = static_cast<size_t>(filesize.QuadPart);
  return STATUS_OK;
}

Ort::Status WindowsEnv::GetFileLength(int fd, /*out*/ size_t& file_size) const {
    if (fd < 0) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Invalid fd was supplied: ", fd);
  }

  struct _stat buf;
  int rc = _fstat(fd, &buf);
  if (rc < 0) {
    return ORT_MAKE_STATUS(ORT_FAIL, "_fstat failed: ", errno);
  }

  if (buf.st_size < 0) {
    return ORT_MAKE_STATUS(ORT_FAIL, "Received negative size from stat call");
  }

  if (static_cast<unsigned long long>(buf.st_size) > std::numeric_limits<size_t>::max()) {
    return ORT_MAKE_STATUS(ORT_FAIL, "File is too large.");
  }
...
Ort::Status WindowsEnv::ReadFileIntoBuffer(_In_z_ const ORTCHAR_T* const file_path, const FileOffsetType offset, const size_t length,
  ...
  if (offset > 0) {
    ...
    if (!SetFilePointerEx(file_handle.get(), current_position, &current_position, FILE_BEGIN)) {
      const auto error_code = GetLastError();
      return ORT_MAKE_STATUS(ORT_FAIL, "SetFilePointerEx ", ToUTF8String(Basename(file_path)), " fail, errcode = ", error_code, " - ", std::system_category().message(error_code));
    }
  }
  ...
  while (total_bytes_read < length) {
    ...
    if (!ReadFile(file_handle.get(), buffer.data() + total_bytes_read, bytes_to_read, &bytes_read, nullptr)) {
      const auto error_code = GetLastError();
      return ORT_MAKE_STATUS(ORT_FAIL, "ReadFile ", ToUTF8String(Basename(file_path)), " fail, errcode = ", error_code, " - ", std::system_category().message(error_code));
    }

    if (bytes_read != bytes_to_read) {
      return ORT_MAKE_STATUS(ORT_FAIL, "ReadFile ", ToUTF8String(Basename(file_path)), " fail: unexpected end");
    }
...
Ort::Status WindowsEnv::MapFileIntoMemory(_In_z_ const ORTCHAR_T* file_path,
  ...
  const SIZE_T mapped_length = offset_to_granularity + length;
...
Ort::Status WindowsEnv::CreateFolder(const std::wstring& path) const {
  size_t pos = 0;
  do {
    ...
    if (CreateDirectoryW(directory.c_str(), NULL) == 0) {
      return ORT_MAKE_STATUS(ORT_FAIL, "CreateDirectory() failed: ", errno);
    }
  } while (pos != std::string::npos);
  return STATUS_OK;
}

Ort::Status WindowsEnv::CreateFolder(const std::string& path) const {
  size_t pos = 0;
  do {
...
Ort::Status WindowsEnv::DeleteFolder(const PathString& path) const {
  Ort::Status final_status{};
  LoopDir(
      path,
      [this, &path, &final_status](
        ...
        if (file_type == OrtFileType::TYPE_DIR) {
          ...
          if (!delete_dir_status.IsOK()) {
            final_status = ORT_MAKE_STATUS(delete_dir_status.GetErrorCode(), delete_dir_status.GetErrorMessage());
          }
        } else {  // not directory
          if (!DeleteFileW(child_path.c_str())) {
            const auto error_code = GetLastError();
            final_status = ORT_MAKE_STATUS(ORT_FAIL,
...
Ort::Status WindowsEnv::FileOpenRd(const std::wstring& path, /*out*/ int& fd) const {
  _wsopen_s(&fd, path.c_str(), _O_RDONLY | _O_SEQUENTIAL | _O_BINARY, _SH_DENYWR, _S_IREAD | _S_IWRITE);
  if (0 > fd) {
    return ORT_MAKE_STATUS(ORT_FAIL, "_wsopen_s() failed: ", errno);
  }
  return STATUS_OK;
}

Ort::Status WindowsEnv::FileOpenWr(const std::wstring& path, /*out*/ int& fd) const {
...
Ort::Status WindowsEnv::FileOpenRd(const std::string& path, /*out*/ int& fd) const {
  _sopen_s(&fd, path.c_str(), _O_RDONLY | _O_SEQUENTIAL | _O_BINARY, _SH_DENYWR, _S_IREAD | _S_IWRITE);
  if (0 > fd) {
      return ORT_MAKE_STATUS(ORT_FAIL, "_sopen_s() failed: ", errno);
  }
  return STATUS_OK;
}

Ort::Status WindowsEnv::FileOpenWr(const std::string& path, /*out*/ int& fd) const {
...
Ort::Status WindowsEnv::FileClose(int fd) const {
  int ret = _close(fd);
  if (0 != ret) {
      return ORT_MAKE_STATUS(ORT_FAIL, "_close() failed: ", errno);
  }
  return STATUS_OK;
}

Ort::Status WindowsEnv::GetCanonicalPath(
  ...
  if (file_handle.get() == INVALID_HANDLE_VALUE) {
    const auto error_code = GetLastError();
    return ORT_MAKE_STATUS(ORT_FAIL, "open file ", ToUTF8String(Basename(path)), " fail, errcode = ",
...
Ort::Status WindowsEnv::LoadDynamicLibrary(const PathString& wlibrary_filename, bool /*global_symbols*/, void** handle) const {
...
#endif
  if (!*handle) {
    return ORT_MAKE_STATUS(ORT_FAIL, "LoadDynamicLibrary failed");
  }
  return STATUS_OK;
}

Ort::Status WindowsEnv::UnloadDynamicLibrary(void* handle) const {
  if (::FreeLibrary(reinterpret_cast<HMODULE>(handle)) == 0) {
    const auto error_code = GetLastError();
    return ORT_MAKE_STATUS(ORT_FAIL, "FreeLibrary failed with error ", error_code, " - ", std::system_category().message(error_code));
  }
  return STATUS_OK;
}
...
Ort::Status WindowsEnv::GetSymbolFromLibrary(void* handle, const std::string& symbol_name, void** symbol) const {
  ...
  if (!*symbol) {
    ...
    status = ORT_MAKE_STATUS(ORT_FAIL, ToUTF8String(errmsg));
  }
