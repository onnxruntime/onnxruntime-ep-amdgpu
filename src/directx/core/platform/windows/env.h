...
#pragma warning(pop)
  ...
  Ort::Status GetFileLength(_In_z_ const ORTCHAR_T* file_path, size_t& length) const override;
  Ort::Status GetFileLength(int fd, /*out*/ size_t& file_size) const override;
  Ort::Status ReadFileIntoBuffer(_In_z_ const ORTCHAR_T* const file_path, const FileOffsetType offset, const size_t length,
                            const gsl::span<char> buffer) const override;
  Ort::Status MapFileIntoMemory(_In_z_ const ORTCHAR_T* file_path,
  ...
  Ort::Status CreateFolder(const std::wstring& path) const override;
  Ort::Status CreateFolder(const std::string& path) const override;
  Ort::Status DeleteFolder(const PathString& path) const override;
  Ort::Status FileOpenRd(const std::wstring& path, /*out*/ int& fd) const override;
  Ort::Status FileOpenWr(const std::wstring& path, /*out*/ int& fd) const override;
  Ort::Status FileOpenRd(const std::string& path, /*out*/ int& fd) const override;
  Ort::Status FileOpenWr(const std::string& path, /*out*/ int& fd) const override;
  Ort::Status FileClose(int fd) const override;
  Ort::Status GetCanonicalPath(const PathString& path, PathString& canonical_path) const override;
  PathString GetRuntimePath() const override;
  Ort::Status LoadDynamicLibrary(const PathString& library_filename, bool /*global_symbols*/, void** handle) const override;
  Ort::Status UnloadDynamicLibrary(void* handle) const override;
  Ort::Status GetSymbolFromLibrary(void* handle, const std::string& symbol_name, void** symbol) const override;
