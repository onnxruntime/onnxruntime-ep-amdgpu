...
Ort::Status Tensor::CalculateTensorStorageSize(MLDataType elt_type, const TensorShape& shape, size_t alignment,
  ...
  if (num_elems > 0) {
    if (static_cast<uint64_t>(num_elems) > std::numeric_limits<size_t>::max()) {
      return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Tensor shape is too large");
    }
    if (!IAllocator::CalcMemSizeForArrayWithAlignment(static_cast<size_t>(num_elems), elt_type->Size(), alignment,
                                                      &storage_size)) {
      return ORT_MAKE_STATUS(ORT_FAIL, "Calculation for Tensor storage size overflowed");
    }
  } else {
    storage_size = 0;
  }
...
size_t Tensor::SizeInBytes() const {
  size_t ret = 0;
  if (!IAllocator::CalcMemSizeForArray(NumStorageElements(), dtype_->Size(), &ret)) {
