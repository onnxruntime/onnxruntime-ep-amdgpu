...
Ort::Status ExecuteKernel(StreamExecutionContext& ctx,
                                  NodeIndex idx,
                                  size_t stream_idx,
  ...
  if (p_kernel->IsAsync()) {
    ...
    ORT_TRY {
      ...
      if (node_stats_recorder != nullptr) {
        ...
        if (kernel_ctx.GetAllocatorStats(temp_stats)) {
          node_stats.total_temp_allocations = gsl::narrow<size_t>(temp_stats.total_allocated_bytes);
        }
    ...
    ORT_CATCH(const std::exception& ex) {
      ORT_HANDLE_EXCEPTION([&]() {
        status = ORT_MAKE_STATUS(ORT_RUNTIME_EXCEPTION, ex.what());
      });
    }
  }
  if (!status.IsOK()) {
    ...
    ss << "Non-zero status code returned while running " << node.OpType() << " node. Name:'" << node.Name()
       << "' Ort::Status Message: " << status.GetErrorMessage();
    ...
    return ORT_MAKE_STATUS(status.GetErrorCode(), msg_string);
  }
...
Ort::Status ExecuteThePlan(const SessionState& session_state, gsl::span<const int> feed_mlvalue_idxs,
...
#ifdef ENABLE_TRAINING
Ort::Status PartialExecuteThePlan(const SessionState& session_state, gsl::span<const int> feed_mlvalue_idxs,
