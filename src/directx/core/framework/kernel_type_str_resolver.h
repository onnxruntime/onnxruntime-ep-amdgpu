...
class IKernelTypeStrResolver {
 public:
   ...
   */
  virtual Ort::Status ResolveKernelTypeStr(const Node& node, std::string_view kernel_type_str,
...
class KernelTypeStrResolver final : public IKernelTypeStrResolver {
 public:
  Ort::Status ResolveKernelTypeStr(const Node& node, std::string_view kernel_type_str,
   ...
   */
  Ort::Status RegisterOpSchema(const ONNX_NAMESPACE::OpSchema& op_schema, bool* registered = nullptr);

  /**
   * Registers kernel type string matching info from an op schema from a node.
   * @param node The node to register.
   */
  Ort::Status RegisterNodeOpSchema(const Node& node);

  /**
   * Registers kernel type string matching info from op schemas from nodes in a graph.
   * @param graph The graph to register.
   */
  Ort::Status RegisterGraphNodeOpSchemas(const Graph& graph);
   ...
   */
  Ort::Status LoadFromOrtFormat(const fbs::KernelTypeStrResolver& fbs_kernel_type_str_resolver);
