...
class OpNodeProtoHelper {
  ...
  */
  template <typename T>
  Ort::Status GetAttr(const std::string& name, T* value) const;
  ...
  */
  ...
  template <typename T>
  Ort::Status GetAttrsAsSpan(const std::string& name, gsl::span<const T>& values) const;

  Ort::Status GetAttrs(const std::string& name, TensorShapeVector& out) const;
  ...
  */
  template <typename T>
  Ort::Status GetAttrs(const std::string& name, std::vector<T>& values) const;

  template <typename T>
  Ort::Status GetAttrs(const std::string& name, gsl::span<T> values) const;

  Ort::Status GetAttrsStringRefs(const std::string& name,
  ...
  [[nodiscard]] uint32_t GetOutputCount() const {
    ...
    [[nodiscard]] auto GetInputType(size_t index) const {
      return impl_->getInputType(index);
    }

    [[nodiscard]] auto GetOutputType(size_t index) const {
...
// ABI-safe version that works with OrtNodePlugin* and returns OrtTypeInfo* instead of TypeProto*
class AbiSafeProtoHelperNodeContext {
public:
    explicit AbiSafeProtoHelperNodeContext(const OrtNodePlugin* node);
    // ABI-safe construction from opaque OrtNode* via C API
    AbiSafeProtoHelperNodeContext(const OrtNode* node, const OrtApi& ort_api);
    AbiSafeProtoHelperNodeContext() = delete;

    const ONNX_NAMESPACE::AttributeProto* getAttribute(const std::string& name) const;
    size_t getNumInputs() const;
    const OrtTypeInfo* getInputType(size_t index) const;
    size_t getNumOutputs() const;
    const OrtTypeInfo* getOutputType(size_t index) const;

private:
    const OrtNodePlugin* plugin_node_ = nullptr;  // Non-null when constructed from OrtNodePlugin*
    const OrtApi* api_ = nullptr;
    std::vector<const OrtValueInfo*> inputs_;
    std::vector<const OrtValueInfo*> outputs_;
    // Plugin-side AttributeProto copies built via C API (ABI-safe)
    std::unordered_map<std::string, std::unique_ptr<ONNX_NAMESPACE::AttributeProto>> attributes_;
