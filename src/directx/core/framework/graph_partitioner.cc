...
static Ort::Status GetCapabilityForEP(const GetCapabilityForEPParams& params, const Ort::Logger& logger) {
...
static Ort::Status GetCapabilityForEPForAotInlining(const GraphViewer& graph_viewer,
...
static Ort::Status PartitionOnnxFormatModelImpl(Graph& graph, FuncManager& func_mgr,
                                           ...
                                           const Ort::Logger& logger, IResourceAccountant* resource_accountant,
                                           const GraphOptimizerRegistry& graph_optimizer_registry,
    ...
    if (sub_graph_available_for_assignment && !capability->nodes_to_optimize.empty()) {
      for (auto& optimization_cc : capability->nodes_to_optimize) {
        if (optimization_cc->optimization_func) {
          ...
          if (status != STATUS_OK) {
            return ORT_MAKE_STATUS(ORT_FAIL, type, "The optimization function failed to finish.");
          }
  ...
  if (!nodes_to_compile.empty()) {
    ...
    if (fusion_style == IExecutionProvider::FusionStyle::Function) {
      ...
      return ORT_MAKE_STATUS(ORT_FAIL, type, "The Function Style fusion is deprecated.");
    } else {
      ...
      for (size_t j = 0, end = nodes_to_compile.size(); j < end; j++) {
        ...
        if (disable_model_compile && graph_viewer_has_non_compiled_node(*graph_viewer)) {
          return ORT_MAKE_STATUS(ORT_MODEL_REQUIRES_COMPILATION, "User disabled EP compilation but EP '",
      ...
      if (node_compute_funcs.size() != nodes_to_compile.size()) {
        return ORT_MAKE_STATUS(ORT_FAIL, type, " did not return correct number of compiled functions");
      }
      ...
      for (size_t j = 0, end = nodes_to_compile.size(); j < end; j++) {
        ...
        ORT_RETURN_IF_ERROR(fused_kernel_registry.Register(
            builder,
            [](FuncManager& func_mgr, const OpKernelInfo& info, std::unique_ptr<OpKernel>& out) -> Ort::Status {
...
static Ort::Status InlineNodes(Graph& graph, bool& modified_graph) {
...
static Ort::Status InlineFunctionsAOTImpl(const ExecutionProviders& execution_providers,
                                     const KernelRegistryManager& kernel_registry_mgr,
                                     Graph& graph,
                                     const GraphOptimizerRegistry& graph_optimizer_registry,
                                     const Ort::Logger& logger,
  ...
  for (const auto& ep : execution_providers) {
    ...
    if (check_load_cancellation_fn()) {
      return ORT_MAKE_STATUS(ORT_MODEL_LOAD_CANCELED, "AOT inlining is canceled due to user request.");
    }
...
static Ort::Status GetValidatedEpContextPath(const std::filesystem::path& ep_context_path,
  ...
  if (!ep_context_path.empty()) {
    ...
    if (!(context_cache_path.has_filename() && context_cache_path.extension() != "")) {
      return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "context_file_path should not point to a folder.");
    }
    ...
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Both ep_context_path and model_path are empty.");
  }

  if (std::filesystem::exists(context_cache_path) && error_if_output_file_exists) {
    return ORT_MAKE_STATUS(ORT_FAIL, "Failed to generate EP context model since the file '",
...
// TODO: Move function to ep_context_utils.h/cc
static Ort::Status CreateEpContextModel(const ExecutionProviders& execution_providers,
                                   const Graph& graph,
  ...
  for (const auto& node : graph.Nodes()) {
    ...
    if (ep_context_node.first) {
      ...
      return ORT_MAKE_STATUS(ORT_INVALID_GRAPH, "Encountered an invalid node while compiling a model. ",
  ...
  if (output_buffer_holder != nullptr) {
    ...
    *output_buffer_holder->buffer_ptr = buffer.release();
    ...
    int fd = 0;
    Ort::Status status = Env::Default().FileOpenWr(valid_output_model_path, fd);
    ...
    ORT_TRY {
      ...
      if (!serialize_result) {
        status = ORT_MAKE_STATUS(ORT_INVALID_PROTOBUF,
...
static Ort::Status PartitionOnnxFormatModel(const PartitionParams& partition_params, GraphPartitioner::Mode mode,
                                       const ExecutionProviders& execution_providers,
                                       KernelRegistryManager& kernel_registry_manager,
                                       const std::optional<ResourceAccountantMap>& acc_map,
                                       const GraphOptimizerRegistry& graph_optimizer_registry,
                                       const Ort::Logger& logger, bool disable_model_compile) {
...
static Ort::Status PartitionOrtFormatModelImpl(const PartitionParams& partition_params,
                                          KernelRegistryManager& kernel_registry_mgr,
                                          IExecutionProvider& current_ep,
                                          const GraphOptimizerRegistry& graph_optimizer_registry,
                                          const Ort::Logger& logger) {
  ...
  for (const auto& capability : capabilities) {
    ...
    if (!metadef) {
      ...
      return ORT_MAKE_STATUS(ORT_FAIL, "Compiling capabilities is not supported in this build.");
#endif  // !defined(ORT_MINIMAL_BUILD) || defined(ORT_EXTENDED_MINIMAL_BUILD)
    }
  }
  ...
  for (const auto& compilation_entry : compilation_entries) {
    ...
    if (partition_params.check_load_cancellation_fn()) {
      return ORT_MAKE_STATUS(ORT_MODEL_LOAD_CANCELED, "Graph partitioning is canceled due to user request.");
    }
...
static Ort::Status PartitionOrtFormatModel(const PartitionParams& partition_params,
                                      const ExecutionProviders& execution_providers,
                                      KernelRegistryManager& kernel_registry_manager,
                                      const GraphOptimizerRegistry& graph_optimizer_registry,
                                      const Ort::Logger& logger) {
...
Ort::Status GraphPartitioner::InlineFunctionsAOT(Model& model,
                                            const ExecutionProviders& execution_providers,
...
Ort::Status GraphPartitioner::Partition(Graph& graph, FuncManager& func_mgr,
  ...
  //    preference.
  if (providers_.Empty()) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "No provider specified.");
  }
  ...
  if (mode == Mode::kNormal || mode == Mode::kAssignOnly) {
    ...
    return ORT_MAKE_STATUS(ORT_FAIL, "ONNX models are not supported in this build.");
#endif  //! defined(ORT_MINIMAL_BUILD)
  } else {
