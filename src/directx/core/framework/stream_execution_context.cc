...
const Ort::Logger& StreamExecutionContext ::GetLogger() const { return *logger_; }

ExecutionFrame& StreamExecutionContext ::GetExecutionFrame() { return frame_; }

const Ort::Status& StreamExecutionContext ::TaskStatus() const {
  return task_status_;
}
...
void StreamExecutionContext ::SetStatus(Ort::Status& status) {
  ...
  if (task_status_.IsOK() && !status.IsOK())
    task_status_ = ORT_MAKE_STATUS(status.GetErrorCode(), status.GetErrorMessage());
}

StreamExecutionContext::~StreamExecutionContext() = default;
...
void RunSince(size_t stream_idx, StreamExecutionContext& ctx, SessionScope& session_scope, const bool& terminate_flag, size_t since) {
  ...
  while (since < end) {
    ...
    if (terminate_flag) {
      Ort::Status status_made = ORT_MAKE_STATUS(ORT_FAIL, "Exiting due to terminate flag being set to true.");
