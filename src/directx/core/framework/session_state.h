...
class SessionState {
 public:
   ...
   * as sparse tensor.
   */
  Ort::Status AddInitializedTensor(int ort_value_index, const OrtValue& ort_value, bool constant, bool sparse);
  ...
  /**
    ...
    */
  Ort::Status GetInitializedTensors(
  ...
  Falls back to returning Ort::LoggingManager::DefaultLogger if SetLogger has not been called.
  */
  const Ort::Logger& Logger() const noexcept { return logger_; }
  ...
  */
  Ort::Status UpdateMemoryPatternGroupCache(gsl::span<const OrtValue> tensor_inputs,
  ...
  */
  ...
  Ort::Status AddInputNameToNodeInfoMapping(const std::string& input_name, const NodeInfo& node_info);
  Ort::Status GetInputNodeInfo(const std::string& input_name, InlinedVector<NodeInfo>& node_info_vec) const;
  ...
  Ort::Status GetOutputNodeInfo(const std::string& output_name, InlinedVector<NodeInfo>& node_info_vec) const;
  ...
  Ort::Status FinalizeSessionState(const std::basic_string<PATH_CHAR_TYPE>& graph_loc,
 ...
 private:
  ...
  Ort::Status CreateKernels(const KernelRegistryManager& custom_registry_manager);
   ...
   */
  Ort::Status PrepackConstantInitializedTensors(InlinedHashMap<std::string, size_t>& constant_initializers_use_count,
                                           const std::unordered_map<std::string, const OrtValue*>& initializers_to_share_map);

  SessionState* GetMutableSubgraphSessionState(onnxruntime::NodeIndex index, const std::string& attribute_name);

  Ort::Status CreateSubgraphSessionState();

  void AddSubgraphSessionState(onnxruntime::NodeIndex index, const std::string& attribute_name,
                               std::unique_ptr<SessionState> session_state);

  Ort::Status PopulateKernelCreateInfo(const KernelRegistryManager& kernel_registry_manager,
                                  bool saving_ort_format);

  Ort::Status FinalizeSessionStateImpl(const std::basic_string<PATH_CHAR_TYPE>& graph_loc,
                                  const KernelRegistryManager& kernel_registry_manager,
...
#ifdef ENABLE_TRAINING
  Ort::Status GeneratePatternGroupCache(
