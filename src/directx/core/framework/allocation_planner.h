...
class IGraphPartitioner {
 public:
  ...
  static std::unique_ptr<IGraphPartitioner> CreateGraphPartitioner(const Ort::Logger& logger,
                                                                   const PathString& config_file);
  virtual Ort::Status PartitionGraph(const onnxruntime::GraphViewer& graph_viewer,
 ...
 protected:
  IGraphPartitioner(const Ort::Logger& logger,
