...
Ort::Status ExternalDataLoaderManager::RegisterExternalDataLoader(std::unique_ptr<IExternalDataLoader> external_data_loader) {
  if (nullptr == external_data_loader) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "external_data_loader registered is nullptr.");
  }
