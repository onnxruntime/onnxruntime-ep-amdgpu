...
class OrtValueNameIdxMap {
 public:
  ...
  Ort::Status GetIdx(std::string_view name, int& idx) const {
    idx = -1;

    auto it = map_.find(name);
    if (it == map_.end()) {
      return ORT_MAKE_STATUS(ORT_FAIL, "Could not find OrtValue with name '", name, "'");
    }
  ...
  Ort::Status GetName(int idx, std::string& name) const {
    auto it = idx_name_map_.find(idx);
    if (it == idx_name_map_.end()) {
      return ORT_MAKE_STATUS(ORT_FAIL, "Could not find OrtValue with idx '", idx, "'");
    }
