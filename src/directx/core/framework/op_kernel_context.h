...
class OpKernelContext {
  ...
  */
   ...
   */
  [[nodiscard]] virtual Ort::Status GetTempSpaceAllocator(AllocatorPtr* output) const;

  /**
   Return the allocator associated with the CPU EP with memtype of OrtMemTypeDefault.
   @remarks Use SafeInt when calculating the size of memory to allocate using AllocatorPtr->Alloc.
   */
  [[nodiscard]] Ort::Status GetTempSpaceCPUAllocator(AllocatorPtr* output) const;
 ...
 protected:
  OpKernelContext(concurrency::ThreadPool* threadpool, const Ort::Logger& logger, Stream* stream);
