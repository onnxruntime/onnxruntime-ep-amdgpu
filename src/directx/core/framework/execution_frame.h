...
class IExecutionFrame {
 ...
 public:
  ...
  Ort::Status SetOutputMLValue(int index, const OrtValue& ort_value);
#endif
  ...
  void UpdateFetches(gsl::span<const int> fetch_mlvalue_idxs, gsl::span<const OrtValue> fetches,
                     ...
                     const std::unordered_map<int, OrtValue>& initializers);
  Ort::Status GetOutputs(gsl::span<const int> fetch_mlvalue_idxs, std::vector<OrtValue>& fetches);
  ...
  Ort::Status GetOrCreateNodeOutputMLValue(const int index, int output_arg_index, const TensorShape* shape,
   ...
   */
  Ort::Status GetOutputs(std::vector<OrtValue>& fetches);

  AllocatorPtr GetAllocator(const OrtDevice& info) const;

  Ort::Status ReleaseMLValue(int ort_value_idx);
 ...
 protected:
  ...
  virtual Ort::Status ReleaseMLValueImpl(int ort_value_idx);
 ...
 private:
  ...
  virtual Ort::Status CreateNodeOutputMLValueImpl(OrtValue& ort_value, int ort_value_idx, const TensorShape* shape) = 0;

  virtual Ort::Status CopyTensor(const Tensor& src, Tensor& dest) const = 0;
...
class ExecutionFrame final : public IExecutionFrame {
 public:
  ...
  Ort::Status AllocateMLValueTensorSelfOwnBuffer(OrtValue& ort_value, int ort_value_index, MLDataType element_type,
                                            const OrtDevice& location, const TensorShape& shape);

  Ort::Status AllocateMLValueTensorPreAllocateBuffer(OrtValue& ort_value, int ort_value_index_reuse, MLDataType element_type,
                                                     const OrtDevice& location, const TensorShape& shape,
                                                     bool is_strided_tensor = false);

  // thread-safe
  Ort::Status GeneratePatterns(MemoryPatternGroup& out);
 ...
 private:
  ...
  Ort::Status ReleaseMLValueImpl(int ort_value_idx) override;
  Ort::Status CreateNodeOutputMLValueImpl(OrtValue& ort_value, int ort_value_idx, const TensorShape* shape) override;
  void VerifyOutputSizes(int output_index, const Node& node, const TensorShape& output_shape) override;
  Ort::Status CopyTensor(const Tensor& src, Tensor& dest) const override;
  const DataTransferManager& GetDataTransferManager() const override;

  Ort::Status AllocateReusedOrtValueIfNotAllocatedHelper(int reuse_mlvalue_index, const TensorShape* shape);

  Ort::Status AllocateAsPerAllocationPlan(OrtValue& ort_value, int ort_value_index, const TensorShape* shape);

  Ort::Status AllocateMLValueTensorSelfOwnBufferHelper(OrtValue& ort_value, int ort_value_index, MLDataType element_type,
                                                  const OrtDevice& location, const TensorShape& shape);

  Ort::Status AllocateTensorWithPreAllocateBufferHelper(OrtValue& ort_value, void* pBuffer, MLDataType element_type,
                                                   const OrtDevice& location, const TensorShape& shape);
