...
class KernelRegistry {
 public:
  ...
  Ort::Status Register(KernelDefBuilder& kernel_def_builder, const KernelCreateFn& kernel_creator);

  Ort::Status Register(KernelCreateInfo&& create_info);

  // TODO(edgchen1) for TryFindKernel(), consider using `out` != nullptr as indicator of whether kernel was found and
  // Ort::Status as an indication of failure
  ...
  Ort::Status TryFindKernel(const Node& node, ProviderType exec_provider,
   ...
   * @return
   */
  Ort::Status TryFindKernel(ProviderType exec_provider,
  ...
  static bool HasImplementationOf(const KernelRegistry& r, const Node& node,
    ...
    Ort::Status st = r.TryFindKernel(node, exec_provider, kernel_type_str_resolver, logger, &info);
    return st.IsOK();
  }
 ...
 private:
  ...
  Ort::Status TryFindKernelImpl(const Node& node, ProviderType exec_provider,
                                const IKernelTypeStrResolver* kernel_type_str_resolver,
                                const TypeConstraintMap* type_constraints,
                                const Ort::Logger& logger,
                                const KernelCreateInfo** out) const;
