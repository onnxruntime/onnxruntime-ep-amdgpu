...
static Ort::Status AllocateHelper(const AllocatorPtr& allocator,
                                     Stream* target_stream,
                                     const OrtValue& source_mlvalue,
                                     OrtValue& target_mlvalue) {
  if (!allocator) {
    return ORT_MAKE_STATUS(ORT_FAIL, "invalid allocator.");
  }
  ...
  if (source_mlvalue.IsTensor()) {
    ...
    if (target_stream && allocator->IsStreamAware()) {
      ...
      if (p_data == nullptr && len > 0) {
        return ORT_MAKE_STATUS(ORT_FAIL, "Allocation failed.");
      }
    }
    ...
    return ORT_MAKE_STATUS(ORT_FAIL, "Unsupported OrtValue type.");
  }
  return STATUS_OK;
}
...
static Ort::Status BatchOrCopyMLValue(const SessionState& session_state,
...
#endif
{
  ...
  if (source_mlvalue.IsTensor()) {
    ...
    if (copy_tensor_pairs != nullptr) {
      ...
      if (stream != nullptr) {
          ORT_RETURN_IF_ERROR(session_state.GetDataTransferMgr().CopyTensorAsync(source_tensor, *p_output_tensor, *stream));
      } else {
    ...
    return ORT_MAKE_STATUS(ORT_FAIL, "Unsupported OrtValue type to copy between device.");
  }
...
static const OrtDevice& FindDeviceForValue(const OrtValueNameIdxMap& map,
  ...
  int idx = -1;
  ORT_THROW_IF_ERROR(map.GetIdx(name, idx));
  return plan.GetLocation(idx);
}
...
static Ort::Status CalculateStaticCopyInfoForFeed(const SessionState& session_state,
                                                     const std::string& input_name,
                                                     MLValueCopyInfo& copy_info) {
  InlinedVector<SessionState::NodeInfo> node_info_vec;
#ifdef ENABLE_TRAINING
  if (session_state.GetInputNodeInfo(input_name, node_info_vec) ==STATUS_OK) {
#else
...
static Ort::Status CalculateStaticCopyInfoForFeeds(const SessionState& session_state,
...
static Ort::Status CalculateStaticCopyInfoForFetches(const SessionState& session_state,
...
Ort::Status InitializeFeedFetchCopyInfo(const SessionState& session_state,
...
static Ort::Status CopyInputsAcrossDevices(const SessionState& session_state,
...
Ort::Status CopyOneInputAcrossDevices(const SessionState& session_state, const std::string& input_name,
...
static Ort::Status CopyOutputsAcrossDevices(const SessionState& session_state,
...
Ort::Status ExecuteGraph(const SessionState& session_state,
                            FeedsFetchesManager& feeds_fetches_manager,
                            gsl::span<const OrtValue> feeds, std::vector<OrtValue>& fetches,
                            ExecutionMode execution_mode, const bool& terminate_flag,
                            const Ort::Logger& logger,
...
#ifdef ENABLE_TRAINING
Ort::Status ExecutePartialGraphImpl(const SessionState& session_state, FeedsFetchesManager& feeds_fetches_manager,
                                       std::vector<OrtValue>& feeds, std::vector<OrtValue>& fetches,
                                       const Ort::Logger& logger, PartialGraphExecutionState& state,
...
Ort::Status ExecutePartialGraph(const SessionState& session_state, FeedsFetchesManager& feeds_fetches_manager,
                                   std::vector<OrtValue>& feeds, std::vector<OrtValue>& fetches,
                                   const Ort::Logger& logger, PartialGraphExecutionState& state,
                                   const OrtValueCachePtr& cache, const bool& terminate_flag,
                                   int32_t partial_graph_index,
...
Ort::Status ExecuteSubgraph(const SessionState& session_state, const FeedsFetchesManager& feeds_fetches_manager,
                               gsl::span<const OrtValue> feeds, std::vector<OrtValue>& fetches,
                               const std::unordered_map<size_t, IExecutor::CustomAllocator>& fetch_allocators,
                               ExecutionMode execution_mode, const bool& terminate_flag, const Ort::Logger& logger,
                               Stream* parent_stream,
...
Ort::Status VerifyInputTensorsAllocatedContiguously(OpKernelContext* context) {
