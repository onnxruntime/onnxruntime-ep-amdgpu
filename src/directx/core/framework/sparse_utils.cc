...
Ort::Status DenseTensorToSparseCsr(const DataTransferManager& data_manager, const Tensor& src,
  ...
  if (is_string) {
    ...
    switch (element_size) {
      ...
      default:
        return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Unsupported element size: ", element_size);
    }
  }
...
Ort::Status SparseCsrToDenseTensor(const DataTransferManager& data_manager, const SparseTensor& src,
                              const AllocatorPtr& cpu_allocator, const AllocatorPtr& dst_allocator,
                              Tensor& dst) {
  const auto& src_dims = src.DenseShape().GetDims();
  if (src_dims.size() != 2) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Support 2-D matrices only");
  }

  if (!(src.Format() == SparseFormat::kCsrc)) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Input must be of CSR format");
  }
...
Ort::Status SparseCooToDenseTensor(const DataTransferManager& data_manager, const SparseTensor& src,
  ...
  if (!(src.Format() == SparseFormat::kCoo)) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Input must be of COO format");
  }
...
Ort::Status DenseTensorToSparseCoo(const DataTransferManager& data_manager, const Tensor& src,
