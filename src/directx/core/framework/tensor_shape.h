...
template <> struct fmt::formatter<onnxruntime::TensorShape> : formatter<std::string> {
    format_context::iterator format(onnxruntime::TensorShape shape, format_context& ctx) const {
        return formatter<std::string>::format(shape.ToString(), ctx);
    }
};
