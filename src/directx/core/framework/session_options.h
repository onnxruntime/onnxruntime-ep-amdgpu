...
struct SessionOptions {
  ...
  Ort::Status AddInitializer(_In_z_ const char* name, _In_ const OrtValue* val);
  ...
  Ort::Status AddExternalInitializers(gsl::span<const std::string> names, gsl::span<const OrtValue> values);
  InlinedHashMap<PathString, std::pair<char*, size_t>> external_initializer_files_mmap;
  Ort::Status AddExternalInitializersFromFilesInMemory(gsl::span<const PathString> file_names,
...
inline std::ostream& operator<<(std::ostream& os, const SessionOptions& session_options) {
  os << "Session Options { "
     ...
     << " optimized_model_filepath:" << std::filesystem::path{session_options.optimized_model_filepath}
     ...
     << " profile_file_prefix:" << std::filesystem::path{session_options.profile_file_prefix}
