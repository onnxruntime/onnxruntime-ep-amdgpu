...
std::optional<std::string> ConfigOptions::GetConfigEntry(const std::string& config_key) const noexcept {
...
Ort::Status ConfigOptions::AddConfigEntry(const char* config_key, const char* config_value) noexcept {
  std::string key = config_key;
  if (key.empty() || key.length() > kMaxKeyLength)
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Config key is empty or longer than maximum length ",
                           kMaxKeyLength);
