...
OpKernelContext::OpKernelContext(_Inout_ IExecutionFrame* frame, _In_ const OpKernel* kernel,
                                 _In_ Stream* stream,
                                 _In_opt_ concurrency::ThreadPool* threadpool, _In_ const Ort::Logger& logger)
...
OrtValue* OpKernelContext::OutputMLValue(int index, const TensorShape& shape) {
  ...
  Ort::Status status = execution_frame_->GetOrCreateNodeOutputMLValue(index, GetOutputArgIndex(index), &shape, p_ml_value, kernel_->Node());
...
Ort::Status OpKernelContext::GetTempSpaceAllocator(AllocatorPtr* output) const {
  *output = GetAllocator(kernel_->GetDevice(OrtMemTypeDefault));
  if (!*output)
    return ORT_MAKE_STATUS(ORT_FAIL, "TempSpace allocator not found");
  return STATUS_OK;
}

Ort::Status OpKernelContext::GetTempSpaceCPUAllocator(AllocatorPtr* output) const {
  ...
  *output = GetAllocator(OrtDevice());
  if (!*output)
    return ORT_MAKE_STATUS(ORT_FAIL, "CPU allocator not found");
  return STATUS_OK;
}
...
#ifdef ENABLE_ATEN
Ort::Status OpKernelContext::SetOutputMLValue(int index, const OrtValue& ort_value) {
  if (index < 0 || index >= OutputCount()) {
