...
class OpKernel {
 public:
  ...
  [[nodiscard]] virtual Ort::Status Compute(_Inout_ OpKernelContext* context) const = 0;
  ...
  [[nodiscard]] virtual Ort::Status ComputeAsync(_Inout_ OpKernelContext*, DoneCallback) const {
  ...
  //   Ort::Status PrePack(const Tensor& tensor, int input_idx, /*out*/ bool& is_packed,
  ...
  virtual Ort::Status GetRemovableAttributes(InlinedVector<std::string>& removable_attributes) const {
  ...
  // Ort::Status UseSharedPrePackedBuffers(std::vector<BufferUniquePtr>& prepacked_buffers,
  //                                 int input_idx,
  ...
  virtual Ort::Status UseSharedPrePackedBuffers(std::vector<BufferUniquePtr>& /*prepacked_buffers*/,
                                           int /*input_idx*/,
                                           /*out*/ bool& used_shared_buffers) {
  ...
  /// <returns></returns>
  virtual Ort::Status UseSharedPrePackedBuffers_V2(std::vector<BufferUniquePtr>& prepacked_buffers,
...
class FuncManager;
using KernelCreateFn = std::function<Ort::Status(FuncManager& func_mgr, const OpKernelInfo& info, std::unique_ptr<OpKernel>& out)>;
using KernelCreatePtrFn = std::add_pointer<Ort::Status(FuncManager& func_mgr, const OpKernelInfo& info, std::unique_ptr<OpKernel>& out)>::type;
    ...
    return KernelCreateInfo(                                                              \
        ...
        static_cast<KernelCreatePtrFn>(                                                   \
            [](FuncManager&,                                                              \
               ...
               std::unique_ptr<OpKernel>& out) -> Ort::Status {                                \
  ...
  BuildKernelCreateInfo<ONNX_OPERATOR_VERSIONED_KERNEL_CLASS_NAME(provider, domain, startver, endver, name)>() {                   \
    return KernelCreateInfo(                                                                                                       \
        builder.SetName(#name)                                                                                                     \
            ...
            .Provider(provider)                                                                                                    \
            .Build(),                                                                                                              \
        static_cast<KernelCreatePtrFn>([](FuncManager&, const OpKernelInfo& info, std::unique_ptr<OpKernel>& out) -> Ort::Status { out = std::make_unique<__VA_ARGS__>(info); return STATUS_OK; })); \
  }
  ...
  BuildKernelCreateInfo<ONNX_OPERATOR_THREE_TYPED_KERNEL_CLASS_NAME(provider, domain, ver, type1, type2, type3, name)>() {         \
    return KernelCreateInfo(                                                                                                       \
        builder.SetName(#name)                                                                                                     \
            ...
            .Provider(provider)                                                                                                    \
            .Build(),                                                                                                              \
        static_cast<KernelCreatePtrFn>([](FuncManager&, const OpKernelInfo& info, std::unique_ptr<OpKernel>& out) -> Ort::Status {  \
          out = std::make_unique<__VA_ARGS__>(info); return STATUS_OK; })); \
  }
