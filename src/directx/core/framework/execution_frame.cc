...
#ifdef ENABLE_ATEN
Ort::Status IExecutionFrame::SetOutputMLValue(int index, const OrtValue& ort_value) {
  ...
  if (ort_value_idx == NodeIndexInfo::kInvalidEntry || static_cast<size_t>(ort_value_idx) >= all_values_size_) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "invalid index ", ort_value_idx);
  }
...
Ort::Status IExecutionFrame::GetOutputs(gsl::span<const int> fetch_mlvalue_idxs, std::vector<OrtValue>& fetches) {
  ...
  if (fetches.empty()) {
    ...
    if (fetches.size() != num_fetches) {
      return ORT_MAKE_STATUS(ORT_FAIL, "Fetches vector passed to GetOutputs contains ", fetches.size(),
...
Ort::Status IExecutionFrame::GetOrCreateNodeOutputMLValue(const int output_index, int output_arg_index,
...
Ort::Status IExecutionFrame::ReleaseMLValue(int ort_value_idx) { return ReleaseMLValueImpl(ort_value_idx); }
...
Ort::Status IExecutionFrame::ReleaseMLValueImpl(int ort_value_idx) {
  if (ort_value_idx == NodeIndexInfo::kInvalidEntry || static_cast<size_t>(ort_value_idx) >= all_values_size_) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "invalid index ", ort_value_idx);
  }
...
Ort::Status IExecutionFrame::GetOutputs(std::vector<OrtValue>& fetches) {
  auto num_fetches = fetch_mlvalue_idxs_.size();
...
Ort::Status ExecutionFrame::CopyTensor(const Tensor& src, Tensor& dest) const {
...
Ort::Status ExecutionFrame::AllocateMLValueTensorSelfOwnBuffer(OrtValue& ort_value, int ort_value_index,
...
Ort::Status ExecutionFrame::AllocateMLValueTensorSelfOwnBufferHelper(OrtValue& ort_value, int ort_value_index,
                                                                MLDataType element_type,
                                                                const OrtDevice& location,
                                                                const TensorShape& shape) {
  if (ort_value_index == NodeIndexInfo::kInvalidEntry) {
    return ORT_MAKE_STATUS(ORT_FAIL, "Trying to allocate memory for unused optional inputs/outputs");
  }
...
Ort::Status ExecutionFrame::AllocateMLValueTensorPreAllocateBuffer(OrtValue& ort_value, int ort_value_index_reuse,
                                                              MLDataType element_type, const OrtDevice& location,
                                                              const TensorShape& shape,
  ...
  if (!is_strided_tensor) {
    ...
    if (buffer_num_elements != required_num_elements) {
      ...
      if (buffer_num_elements >= required_num_elements) {
        ...
        return ORT_MAKE_STATUS(ORT_FAIL, message);
      }
    }
  }
...
Ort::Status ExecutionFrame::AllocateTensorWithPreAllocateBufferHelper(OrtValue& ort_value, void* pBuffer,
                                                                 MLDataType element_type,
                                                                 const OrtDevice& location,
                                                                 const TensorShape& shape) {
...
static Ort::Status AllocateTraditionalMLValue(OrtValue& ort_value, const NonTensorTypeBase& type) {
  auto creator = type.GetCreateFunc();
  ort_value.Init(creator(), &type, type.GetDeleteFunc());
  return STATUS_OK;
}

static Ort::Status AllocateTensorSequence(OrtValue& ort_value) {
...
#if !defined(DISABLE_SPARSE_TENSORS)
static Ort::Status AllocateSparseTensor(OrtValue& mlvalue, const DataTypeImpl& ml_type, AllocatorPtr allocator,
                                   const TensorShape& shape,
                                   const SessionState& /*session_state*/) {
...
Ort::Status ExecutionFrame::AllocateReusedOrtValueIfNotAllocatedHelper(int reuse_mlvalue_index, const TensorShape* shape) {
...
// This method is not thread safe!
Ort::Status ExecutionFrame::AllocateAsPerAllocationPlan(OrtValue& ort_value, int ort_value_index, const TensorShape* shape) {
  ...
  if (ml_type->IsTensorType()
    ...
    switch (alloc_kind) {
      ...
      default: {
        ...
        return ORT_MAKE_STATUS(ORT_FAIL, ostr.str());
      }
    }
...
// This method is not thread safe!
// Return S_OK and nullptr if index map to a value that is an unused optional input/output
Ort::Status ExecutionFrame::CreateNodeOutputMLValueImpl(OrtValue& ort_value, int ort_value_idx, const TensorShape* shape) {
#ifdef ENABLE_TRAINING
  try {
    ...
    ReleaseAllMLValues();
    return ORT_MAKE_STATUS(ORT_FAIL, e.what());
  }
#else
...
Ort::Status ExecutionFrame::ReleaseMLValueImpl(int ort_value_idx) {
...
void ExecutionFrame::TraceAllocate(int ort_value_idx, size_t size) {
  if (planner_.has_value()) {
    ...
    (void)planner_->TraceAllocation(ort_value_idx, size);
  }
}
  ...
  if (planner_.has_value() && !IsOutput(ort_value_idx)) {
    ...
    if (ml_type->IsTensorType()) {
      ...
      if (!utils::IsDataTypeString(ml_data_type)) {
        (void)planner_->TraceFree(ort_value_idx);
      }
    }
  }
}
...
Ort::Status ExecutionFrame::GeneratePatterns(MemoryPatternGroup& out) {
  if (!planner_.has_value()) {
    return ORT_MAKE_STATUS(ORT_FAIL, "Memory pattern planner is not enabled on this execution framework.");
  }
