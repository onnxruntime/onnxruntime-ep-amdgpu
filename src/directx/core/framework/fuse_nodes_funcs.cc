...
Ort::Status FuncManager::AddFuncInfo(const std::string& name, const std::string& dll_path) {
  auto it = fused_funcs_->find(name);
  if (it != fused_funcs_->end())
    return ORT_MAKE_STATUS(ORT_FAIL, "func info for node: " + name + " already exist.");
  (*fused_funcs_)[name] = {dll_path, NodeComputeInfo()};
  return STATUS_OK;
}

Ort::Status FuncManager::AddFuncInfo(const std::string& name, NodeComputeInfo&& compute_info) {
  auto it = fused_funcs_->find(name);
  if (it != fused_funcs_->end())
    return ORT_MAKE_STATUS(ORT_FAIL, "func info for node: " + name + " already exist.");

  if (!compute_info.compute_func || !compute_info.create_state_func || !compute_info.release_state_func)
    return ORT_MAKE_STATUS(ORT_FAIL, "Can't use func with null ptr");

  (*fused_funcs_)[name] = {"", std::move(compute_info)};
  return STATUS_OK;
}

Ort::Status FuncManager::GetFuncs(const std::string& name, const NodeComputeInfo*& compute_info) {
  auto it = fused_funcs_->find(name);
  if (it == fused_funcs_->end())
    return ORT_MAKE_STATUS(ORT_FAIL, "func info for node: " + name + " not found.");
