...
Ort::Status KernelTypeStrResolver::ResolveKernelTypeStr(const Node& node, std::string_view kernel_type_str,
...
Ort::Status KernelTypeStrResolver::RegisterOpSchema(const ONNX_NAMESPACE::OpSchema& op_schema, bool* registered_out) {
  ...
  auto process_formal_params = [&](ArgType arg_type) -> Ort::Status {
...
Ort::Status KernelTypeStrResolver::RegisterNodeOpSchema(const Node& node) {
  ORT_RETURN_IF(node.Op() == nullptr, "Op schema must be available.");
  return RegisterOpSchema(*node.Op());
}

Ort::Status KernelTypeStrResolver::RegisterGraphNodeOpSchemas(const Graph& graph) {
...
Ort::Status KernelTypeStrResolver::SaveToOrtFormat(
...
static std::string LoadFromOrtFormatImpl(const fbs::KernelTypeStrResolver& fbs_kernel_type_str_resolver,
  ...
  for (const auto* fbs_op_kernel_type_str_args_entry : *fbs_op_kernel_type_str_args) {
    ...
    if (!load_op_id_status.IsOK()) {
      return load_op_id_status.GetErrorMessage();
    }
...
Ort::Status KernelTypeStrResolver::LoadFromOrtFormat(const fbs::KernelTypeStrResolver& fbs_kernel_type_str_resolver) {
  const auto error_message = LoadFromOrtFormatImpl(fbs_kernel_type_str_resolver, op_kernel_type_str_map_);
  return error_message.empty() ? STATUS_OK
                               : ORT_MAKE_STATUS(ORT_FAIL, error_message, " ",
...
#if !defined(ORT_MINIMAL_BUILD)
Ort::Status OpSchemaKernelTypeStrResolver::ResolveKernelTypeStr(
