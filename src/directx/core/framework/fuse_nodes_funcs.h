...
class FuncManager {
 public:
  ...
  Ort::Status AddFuncInfo(const std::string& name, const std::string& dll_path);

  Ort::Status AddFuncInfo(const std::string& name, NodeComputeInfo&& compute_info);

  // Do not call AddFuncInfo after this function is called.
  Ort::Status GetFuncs(const std::string& name, const NodeComputeInfo*& compute_info);
