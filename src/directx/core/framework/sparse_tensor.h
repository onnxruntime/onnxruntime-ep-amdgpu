...
class SparseTensor final {
 public:
  ...
  Ort::Status UseCooIndices(gsl::span<int64_t> indices);
  ...
  /// <returns></returns>
  Ort::Status MakeCooData(const IDataTransfer& data_transfer, const OrtMemoryInfo& data_location,
  ...
  /// <returns>Status</returns>
  Ort::Status MakeCooStrings(size_t string_count, const char* const* strings, gsl::span<const int64_t> indices);
  ...
  /// <returns></returns>
  Ort::Status UseCsrIndices(gsl::span<int64_t> inner_index, gsl::span<int64_t> outer_index);
  ...
  /// <returns></returns>
  Ort::Status MakeCsrData(const IDataTransfer& data_transfer,
  ...
  /// <param name="outer_index">outer index to be copied. Use empty span for fully sparse tensors.</param>
  /// <returns></returns>
  Ort::Status MakeCsrStrings(size_t string_count, const char* const* strings,
                             gsl::span<const int64_t> inner_index,
                             gsl::span<const int64_t> outer_index);
  ...
  /// <returns></returns>
  Ort::Status UseBlockSparseIndices(const TensorShape& indices_shape, int32_t* indices_data);
  ...
  /// <returns></returns>
  Ort::Status MakeBlockSparseData(const IDataTransfer& data_transfer,
                                  const OrtMemoryInfo& data_location,
  ...
  /// <returns></returns>
  Ort::Status MakeBlockSparseStrings(const TensorShape& values_shape, const char* const* strings,
                                     const TensorShape& indices_shape, const int32_t* indices_data);
  ...
  /// <returns></returns>
  Ort::Status Copy(const DataTransferManager& data_transfer_manager, SparseTensor& dst_tensor) const;
  ...
  /// <param name="dst_tensor"></param>
  /// <returns></returns>
  Ort::Status Copy(const IDataTransfer& data_transfer, SparseTensor& dst_tensor) const;

 private:
  Ort::Status AllocateBuffer(int64_t buffer_size, size_t num_values);
  void ReleaseBuffer();
  void* IndicesStart(int64_t values_bytes);
  const void* IndicesStart(int64_t values_bytes) const;
  Ort::Status ValidateBlockSparseShapes(const TensorShape& values_shape, const TensorShape& index_shape) const;
  ...
  Ort::Status ValidateCsrIndices(size_t values_count, size_t inner_size, size_t outer_size) const;
...
template <> struct fmt::formatter<onnxruntime::SparseFormat> : formatter<std::string_view> {
    format_context::iterator format(const onnxruntime::SparseFormat& format, format_context& ctx) const {
        std::string_view name{"unknown"};
        switch (format) {
        case onnxruntime::SparseFormat::kUndefined:
            name = "Undefined";
            break;
        ...
        case onnxruntime::SparseFormat::kCsrc:
            name = "Csrc";
            break;
        case onnxruntime::SparseFormat::kBlockSparse:
            name = "BlockSparse";
            break;
        }
