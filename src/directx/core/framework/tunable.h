...
struct HasIsSupportedMethod<
    T, Arg, std::enable_if_t<std::is_same_v<decltype(std::declval<T>().IsSupported(std::declval<Arg>())), Ort::Status>>> {
  constexpr static bool value = true;
};

// A type erased Callable wrapper. We could have used std::function<Ort::Status<const ParamsT*>> here. However, std::function
...
//
// Then an Op is Ort::Status(const ParamsT*), that is, a callable accepts a const ParamsT* and returns a Status.
...
class Op {
 public:
  ...
  Op(Op&&) = default;
  Ort::Status operator()(const ParamsT* param) { return (*callable_)(param); }
  Ort::Status IsSupported(const ParamsT* param) { return (*callable_).IsSupported(param); }

 private:
  struct ICallable {
    virtual ~ICallable() = default;
    virtual Ort::Status operator()(const ParamsT*) = 0;
    virtual Ort::Status IsSupported(const ParamsT*) = 0;
  };
  ...
  struct CallableImpl : ICallable {
    ...
    Ort::Status operator()(const ParamsT* param) override { return c_(param); }

    Ort::Status IsSupported(const ParamsT* param) override {
...
// NOTE: onnxruntime's Ort::Status currently does not have a StatusCode::UNSUPPORTED. Currently, we do not want to extend the
...
class TunableOp {
 public:
  ...
  Ort::Status operator()(const ParamsT* params) {
    int id = -1;
 ...
 private:
  ...
  // processed by onnxruntime. We return Ort::Status to avoid the construction of op and params signature string.
  static Ort::Status IsSupported(Op<ParamsT>& op, const ParamsT* params) {
    Ort::Status status = op.IsSupported(params);
    if (status.Category() == Ort::StatusCategory::NONE && status.Code() == Ort::StatusCode::INVALID_ARGUMENT) {
      return status;
    }
  ...
  int FindFastestImpl(const ParamsT* params, const std::vector<Op<ParamsT>>& candidates) {
    ...
    for (size_t i = 0; i < candidates.size(); i++) {
      ...
      if (!status.IsOK()) {
        ...
        LOGS_DEFAULT(VERBOSE) << "│  reason: " << status.GetErrorMessage();
        continue;
      }
