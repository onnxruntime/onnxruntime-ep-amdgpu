...
Ort::Status SessionState::PopulateKernelCreateInfo(const KernelRegistryManager& kernel_registry_manager,
...
Ort::Status SessionState::CreateKernels(const KernelRegistryManager& kernel_registry_manager) {
...
void SessionState::PruneRemovableAttributes() {
  ...
  for (size_t i = 0; i < session_kernels_.size(); ++i) {
    ...
    (void)node->PruneRemovableAttributes(removable_attributes);
  }
}
...
Ort::Status SessionState::AddInitializedTensor(int ort_value_index, const OrtValue& ort_value,
                                          bool constant, bool sparse) {
  auto p = initialized_tensors_.insert({ort_value_index, ort_value});
  if (!p.second)
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "duplicated ort_value index:", ort_value_index,
...
Ort::Status SessionState::GetInitializedTensors(
  ...
  for (const auto& weight_name : interested_weights) {
    ...
    if (!status.IsOK()) {
      ORT_RETURN_IF_NOT(
          ...
          "Failed to get OrtValue index from name: ", status.GetErrorMessage());
      continue;
    }
...
static Ort::Status KernelUseSharedPrePackedBuffers(OpKernel& kernel, int input_idx,
  ...
  if (!used_shared_buffers)
    return ORT_MAKE_STATUS(ORT_FAIL, "The kernel corresponding to the node ", node_name,
...
Ort::Status SessionState::PrepackConstantInitializedTensors(
  ...
  auto prepacked_constant_weights = [this, &constant_initializers_use_count, &initializers_to_share_map](
                                        bool should_cache_prepacked_weights_for_shared_initializers) -> Ort::Status {
    for (auto& node : GetGraphViewer().Nodes()) {
      if (sess_options_.IsLoadCancellationFlagSet()) {
        return ORT_MAKE_STATUS(ORT_MODEL_LOAD_CANCELED,
...
#ifdef ENABLE_TRAINING
namespace {
Ort::Status ResolveDimParams(const GraphViewer& graph,
  ...
  for (const auto* input : graph.GetInputs()) {
    ...
    if (!shape || shape->dim_size() != static_cast<int>(it->second.NumDimensions())) {
      return ORT_MAKE_STATUS(ORT_FAIL, "Graph input " + input->Name() +
  ...
  for (auto& dim : arg->Shape()->dim()) {
    if (dim.has_dim_param()) {
      ...
      if (it == symbolic_dimensions.end()) {
        return ORT_MAKE_STATUS(ORT_FAIL, "Unknown symbolic dimension, " + dim.dim_param() + ", found in memory pattern compute.");
      }
      safe_size *= it->second;
...
Ort::Status SessionState::GeneratePatternGroupCache(gsl::span<const OrtValue> tensor_inputs,
  ...
  for (auto ml_value_idx : exe_plan->activation_allocation_order) {
    ...
    if (exe_plan->allocation_plan[ml_value_idx].alloc_kind == AllocKind::kAllocate &&
      ...
      if (size == 0) {
        ...
        return ORT_MAKE_STATUS(ORT_FAIL, "Unknown shape found in memory pattern compute, node name is : " + node_name);
      }

      if (!IAllocator::CalcMemSizeForArrayWithAlignment<kAllocAlignment>(size, ml_data_type->Size(), &size)) {
        return ORT_MAKE_STATUS(ORT_FAIL, "Size overflow");
      }
  ...
  if (!mem_planner.GeneratePatterns(output).IsOK()) {
    return ORT_MAKE_STATUS(ORT_FAIL, "Generate Memory Pattern failed");
  }
  return STATUS_OK;
}
...
Ort::Status SessionState::UpdateMemoryPatternGroupCache(gsl::span<const OrtValue> tensor_inputs,
...
Ort::Status SessionState::AddInputNameToNodeInfoMapping(const std::string& input_name, const NodeInfo& node_info) {
...
Ort::Status SessionState::GetInputNodeInfo(const std::string& input_name,
  ...
  if (entry == input_names_to_nodeinfo_mapping_.cend()) {
    return ORT_MAKE_STATUS(ORT_FAIL, "Failed to find input name in the mapping: " + input_name);
  }
...
Ort::Status SessionState::GetOutputNodeInfo(const std::string& output_name,
                                               InlinedVector<NodeInfo>& node_info_vec) const {
  auto entry = output_names_to_nodeinfo_mapping_.find(output_name);
  if (entry == output_names_to_nodeinfo_mapping_.cend()) {
    return ORT_MAKE_STATUS(ORT_FAIL, "Failed to find output name in the mapping: " + output_name);
  }
...
Ort::Status SessionState::CreateSubgraphSessionState() {
  for (auto& node : graph_.Nodes()) {
    for (auto& entry : node.GetAttributeNameToMutableSubgraphMap()) {
      ...
      auto subgraph_session_state =
          std::make_unique<SessionState>(*subgraph, execution_providers_,
                                         ...
                                         external_data_loader_mgr_, logger_, sess_options_,
...
static Ort::Status VerifyEachNodeIsAssignedToAnEpImpl(const Graph& graph, bool is_verbose,
...
static Ort::Status VerifyEachNodeIsAssignedToAnEp(const Graph& graph, const Ort::Logger& logger,
...
Ort::Status SessionState::FinalizeSessionState(const std::basic_string<PATH_CHAR_TYPE>& graph_location,
...
static Ort::Status Index(const OrtValueNameIdxMap& ort_value_name_idx_map,
...
static Ort::Status OuterScopeNodeArgLocationAccumulator(const SequentialExecutionPlan& plan,
...
Ort::Status SessionState::FinalizeSessionStateImpl(const std::basic_string<PATH_CHAR_TYPE>& graph_location,
                                              const KernelRegistryManager& kernel_registry_manager,
  ...
  ORT_RETURN_IF_ERROR(session_state_utils::SaveInitializedTensors(
      ...
      [this, remove_initializers](const std::string& name, int idx, const OrtValue& value,
                                  bool constant, bool sparse) -> Ort::Status {
