...
Ort::Status KernelRegistry::TryFindKernelImpl(const Node& node,
  ...
  if (!verify_kernel_def_error_strs.empty()) {
    ...
    return ORT_MAKE_STATUS(ORT_FAIL, oss.str());
  }

  return ORT_MAKE_STATUS(ORT_FAIL, "Kernel not found");
}

Ort::Status KernelRegistry::TryFindKernel(const Node& node, ProviderType exec_provider,
                                     const IKernelTypeStrResolver& kernel_type_str_resolver,
                                     const Ort::Logger& logger,
                                     const KernelCreateInfo** out) const {
...
Ort::Status KernelRegistry::TryFindKernel(ProviderType exec_provider,
...
Ort::Status KernelRegistry::Register(KernelDefBuilder& kernel_builder,
                                const KernelCreateFn& kernel_creator) {
  return Register(KernelCreateInfo(kernel_builder.Build(), kernel_creator));
}

Ort::Status KernelRegistry::Register(KernelCreateInfo&& create_info) {
  if (!create_info.kernel_def) {
    return ORT_MAKE_STATUS(ORT_FAIL, "kernel def can't be NULL");
  }
