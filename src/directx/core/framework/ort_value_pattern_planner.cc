...
Ort::Status OrtValuePatternPlanner::TraceAllocation(int ort_value_idx,
...
Ort::Status OrtValuePatternPlanner::TraceAllocation(int ort_value_idx, size_t size) {
  const auto& location = execution_planner_.GetLocation(ort_value_idx);
  auto it = planner_map_.find(location);
  if (it == planner_map_.end()) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT);
...
Ort::Status OrtValuePatternPlanner::TraceFree(int ort_value_index) {
...
Ort::Status OrtValuePatternPlanner::GeneratePatterns(MemoryPatternGroup& out) {
