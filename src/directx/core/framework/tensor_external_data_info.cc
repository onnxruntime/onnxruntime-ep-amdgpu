...
Ort::Status ExternalDataInfo::Create(const RepeatedPtrField<StringStringEntryProto>& input,
  ...
  for (int i = 0; i != input_size; ++i) {
    ...
    if (!stringmap.has_key())
      return ORT_MAKE_STATUS(ORT_FAIL, "model format error! Need a key for the external data info");
    if (!stringmap.has_value())
      return ORT_MAKE_STATUS(ORT_FAIL, "model format error! Need a value for the external data info");
    ...
    if (stringmap.key() == "location" && !stringmap.value().empty()) {
      ...
      return ORT_MAKE_STATUS(ORT_FAIL, "model format error!");
    }
  }

  if (external_data_info->rel_path_.empty()) {
    return ORT_MAKE_STATUS(ORT_FAIL, "model format error! Missing 'location'");
  }
...
std::ostream& ExternalDataInfo::WritePrepackedToFileAndAddToProto(
  ...
  for (const auto& key : blob_keys) {
    ...
    for (size_t i = 0, size = prepacked_weights->buffers_.size(); i < size; ++i) {
      ...
      external_offset = external_offset + size_in_bytes;
    }
