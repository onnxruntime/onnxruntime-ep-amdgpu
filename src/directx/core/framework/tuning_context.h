...
class ITuningContext {
 public:
  ...
  virtual Ort::Status LoadTuningResults(const TuningResults& tr);
...
class TuningResultsValidator {
 public:
  ...
  using ValidateFunc = std::function<Ort::Status(const std::string&)>;
  ...
  Ort::Status ValidateAll(const std::unordered_map<std::string, std::string>& to_validate) const;

 protected:
  void RegisterValidator(const std::string& key, const GetFunc& gf, const ValidateFunc& vf);

  virtual std::string GetOrtVersion() const;
  virtual Ort::Status ValidateOrtVersion(const std::string& value) const;

  virtual std::string GetOrtGitCommit() const;
  virtual Ort::Status ValidateOrtGitCommit(const std::string& value) const;

  virtual std::string GetOrtBuildConfig() const;
  virtual Ort::Status ValidateOrtBuildConfig(const std::string& value) const;

 public:
  static constexpr std::array mandatory_keys{"ORT_VERSION", "ORT_GIT_COMMIT", "ORT_BUILD_CONFIG"};
