...
static Ort::Status AllocateBufferUsingDeviceAllocatorFromShapeAndType(const TensorShape& tensor_shape,
 ...
 * @return Ort::Status indicating success or failure of the deserialization process.
 ...
 */
static Ort::Status DeserializeTensorProto(const Env& env, const std::basic_string<PATH_CHAR_TYPE>& proto_path,
  ...
  if (utils::HasExternalData(tensor_proto)) {
    ...
    if (external_data_loader) {
      ...
      if (utils::HasString(tensor_proto)) {
        return ORT_MAKE_STATUS(ORT_FAIL, "string tensor is not supported for copying between allocators");
      }
...
Ort::Status AllocateTensor(const onnxruntime::MemBuffer* memory_buffer,
                              Tensor& tensor,
  ...
  if (memory_buffer != nullptr) {
    ...
    if (memory_buffer->GetLen() < tensor.SizeInBytes()) {
      return ORT_MAKE_STATUS(ORT_FAIL, "Internal error. The preallocated buffer is too small. Requires ",
...
Ort::Status AllocateTensorOnDeviceOrMemory(
    bool use_device_allocator_for_initializers,
    onnxruntime::TensorShape& tensor_shape,
    const onnxruntime::DataTypeImpl* const& type,
    const onnxruntime::AllocatorPtr& alloc,
...
Ort::Status CopyTensorFromCPUToDevice(
    const onnxruntime::DataTransferManager& data_transfer_mgr,
    const Tensor& deserialized_tensor,
    Tensor&& tensor,
    OrtValue& ort_value) {
  Ort::Status copy_status = data_transfer_mgr.CopyTensor(deserialized_tensor, tensor);
  if (!copy_status.IsOK()) {
    if (copy_status.GetErrorMessage().empty()) {
      ...
      return ORT_MAKE_STATUS(copy_status.GetErrorCode(),
...
Ort::Status SaveInitializedTensors(
  ...
  for (const auto& entry : id_to_initialized_tensor) {
    ...
    if (user_supplied_initializer_ids.find(entry.first) != user_supplied_initializer_ids.end()) {
      ...
      if (OrtValue ort_value_from_graph;
        ...
        Ort::Status st = DeserializeTensorProto(env, graph_loc, tensor_proto,
                                           ...
                                           use_device_allocator_for_initializers);
        if (!st.IsOK()) {
          return ORT_MAKE_STATUS(st.GetErrorCode(), "Deserialize tensor ", name, " failed.", st.GetErrorMessage());
        }
      }
    }
...
Ort::Status SaveInputOutputNamesToNodeMapping(const onnxruntime::GraphViewer& graph,
