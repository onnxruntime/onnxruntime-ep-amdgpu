...
Ort::Status DispatchStridedCopy(concurrency::ThreadPool* thread_pool,
                           Tensor& dst,
                           std::ptrdiff_t dst_offset,
                           const TensorShapeVector& dst_strides,
                           const TensorShape& copy_shape,
  ...
  return !supported ? ORT_MAKE_STATUS(ORT_FAIL, "Unsupported input data type of ", *src.DataType())
                    :STATUS_OK;
}
