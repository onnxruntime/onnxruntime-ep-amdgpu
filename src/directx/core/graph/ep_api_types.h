...
struct EpValueInfo : public OrtValueInfo {
 public:
  ...
  Ort::Status GetProducerInfo(OrtValueInfo::ProducerInfo& producer_info) const override;
  ...
  Ort::Status GetConsumerInfos(std::vector<OrtValueInfo::ConsumerInfo>& consumer_infos) const override;

  // Gets the number of ConsumerInfo instances that will be returned by GetConsumerInfos.
  Ort::Status GetNumConsumerInfos(size_t& num_consumers) const override;
  ...
  Ort::Status GetInitializerValue(const OrtValue*& value) const override;
  ...
  Ort::Status GetExternalInitializerInfo(std::unique_ptr<onnxruntime::ExternalDataInfo>& ext_info) const override;

  // Check if this value is a required graph input.
  Ort::Status IsRequiredGraphInput(bool& is_required_graph_input) const override;

  // Check if this value is an optional graph input.
  Ort::Status IsOptionalGraphInput(bool& is_optional_graph_input) const override;

  // Check if this value is a graph output.
  Ort::Status IsGraphOutput(bool& is_graph_output) const override;

  // Check if this value is a constant initializer.
  Ort::Status IsConstantInitializer(bool& is_const_initializer) const override;

  // Check if this value is defined in an outer scope (i.e., an outer graph).
  Ort::Status IsFromOuterScope(bool& is_outer_scope) const override;
...
/// </summary>
struct EpNode : public OrtNodePlugin {
 private:
 ...
 public:
  ...
  /// <returns>A Ort::Status indicating success or an error.</returns>
  static Ort::Status Create(const Node& node, const EpGraph* ep_graph,
                            std::unordered_map<std::string, std::unique_ptr<EpValueInfo>>& value_infos,
                            /*out*/ std::unique_ptr<EpNode>& result);

  // Defines ToExternal() and ToInternal() functions to convert between OrtNodePlugin and EpNode.
  DEFINE_ORT_GRAPH_IR_TO_EXTERNAL_INTERNAL_FUNCS(OrtNodePlugin, EpNode, OrtGraphIrApi::kEpApi)
  ...
  Ort::Status GetSinceVersion(int& since_version) const override;

  // Get the number of node inputs.
  size_t GetNumInputs() const override;

  // Gets the node's inputs.
  Ort::Status GetInputs(gsl::span<const OrtValueInfo*> inputs) const override;

  // Get the number of node outputs.
  size_t GetNumOutputs() const override;

  // Gets the node's outputs.
  Ort::Status GetOutputs(gsl::span<const OrtValueInfo*> outputs) const override;

  // Gets the number of implicit inputs.
  Ort::Status GetNumImplicitInputs(size_t& num_implicit_inputs) const override;

  // Gets the node's implicit inputs.
  Ort::Status GetImplicitInputs(gsl::span<const OrtValueInfo*> inputs) const override;

  // Get the number of node attributes.
  size_t GetNumAttributes() const override;

  // Gets the node's attributes.
  Ort::Status GetAttributes(gsl::span<const OrtOpAttr*> attrs) const override;

  // Gets the number of subgraphs contained by this node.
  Ort::Status GetNumSubgraphs(size_t& num_subgraphs) const override;

  // Gets the subgraphs contained by this node.
  Ort::Status GetSubgraphs(gsl::span<const OrtGraph*> subgraphs,
  ...
  Ort::Status GetGraph(const OrtGraph*& parent_graph) const override;
...
struct EpGraph : public OrtGraph {
 ...
 public:
  ...
  /// <returns></returns>
  static Ort::Status Create(const GraphViewer& graph_viewer,
  ...
  /// <param name="result"></param>
  /// <returns></returns>
  static Ort::Status Create(std::unique_ptr<GraphViewer> graph_viewer,
                       std::unique_ptr<IndexedSubGraph> indexed_sub_graph,
                       /*out*/ std::unique_ptr<EpGraph>& result);
  ...
  Ort::Status GetNumOperatorSets(size_t& num_operator_sets) const override;

  // Gets the operator sets that the graph's model uses. An operator set is uniquely identified by a
  // (domain, opset version) pair.
  Ort::Status GetOperatorSets(gsl::span<const char*> domains,
  ...
  Ort::Status GetInitializers(gsl::span<const OrtValueInfo*> initializers) const override;
  ...
  Ort::Status GetNodes(gsl::span<const OrtNodePlugin*> nodes) const override;

  // Gets the graph's parent node or nullptr if this is not a nested subgraph.
  Ort::Status GetParentNode(const OrtNodePlugin*& parent_node) const override;
  ...
  // within this graph.
  Ort::Status GetInitializerValue(std::string_view name, const OrtValue*& value) const;
 ...
 private:
  ...
  /// <returns></returns>
  static Ort::Status CreateImpl(std::unique_ptr<EpGraph> ep_graph, const GraphViewer& graph_viewer,
