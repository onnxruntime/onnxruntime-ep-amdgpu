...
/// </summary>
using ExternalDataWriter = std::function<Ort::Status(int32_t data_type, gsl::span<const uint8_t> bytes, uint64_t& offset)>;
...
using ExternalDataReader = std::function<Ort::Status(uint64_t offset, gsl::span<uint8_t> output_buffer)>;
...
Ort::Status SaveInitializerOrtFormat(
...
#if !defined(DISABLE_SPARSE_TENSORS)
Ort::Status SaveSparseInitializerOrtFormat(
...
Ort::Status SaveAttributeOrtFormat(
...
Ort::Status LoadInitializerOrtFormat(const fbs::Tensor& fbs_tensor,
...
/// @return Ort::Status indicating success or providing error information
