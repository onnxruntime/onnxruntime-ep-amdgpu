...
static Ort::Status MergeShapeInfo(const std::string& output_name,
                             const TypeProto& source, TypeProto& target,
                             bool strict, const Ort::Logger& logger) {
  if (!(utils::HasTensorType(source) && utils::HasTensorType(target))
    ...
    return ORT_MAKE_STATUS(ORT_FAIL, ss.str());
  }
  ...
  ORT_CATCH(const ONNX_NAMESPACE::InferenceError& ex) {
    ...
    if (!strict) {
      ...
      ORT_HANDLE_EXCEPTION([&]() {
        status = ORT_MAKE_STATUS(ORT_FAIL, "Output:", output_name, " ", ex.what());
      });
    }
  }
...
Ort::Status NodeArg::OverrideTypesHelper(const ONNX_NAMESPACE::TypeProto& input_type,
  ...
  if (input_tensor_elem_type != current_tensor_elem_type) {
    if (override_types) {
      ...
      return ORT_MAKE_STATUS(ORT_FAIL, "Tensor element type mismatch. ",
...
Ort::Status NodeArg::UpdateTypeAndShape(const ONNX_NAMESPACE::TypeProto& input_type, bool strict,
                                           bool override_types, const Ort::Logger& logger) {
  if (!utils::HasType(node_arg_info_)) {
    SetType(input_type);
    return STATUS_OK;
  }
  ...
  if (current_type_case != input_type_case)
    return ORT_MAKE_STATUS(ORT_FAIL, "Type mismatch. Current=",
  ...
  switch (input_type_case) {
    ...
    case TypeProto::kOptionalType: {
      ...
      if (is_input_type_optional_tensor_type != is_current_type_optional_tensor_type) {
        return ORT_MAKE_STATUS(ORT_FAIL, "Optional Type mismatch. Expected: ",
...
Ort::Status NodeArg::UpdateTypeAndShape(const NodeArg& node_arg, bool strict, bool override_types,
...
Ort::Status Node::SaveToOrtFormat(flatbuffers::FlatBufferBuilder& builder,
  ...
  if (func_body_ != nullptr && node_type_ != Type::Primitive) {
    return ORT_MAKE_STATUS(ORT_FAIL, "Serialization of fused function body is not currently supported, ",
  ...
  // Checks an attribute was not removed.
  if (!can_be_saved_) {
    return ORT_MAKE_STATUS(ORT_FAIL, "Removable attributes were removed before the node is saved.");
  }
...
Ort::Status Node::LoadFromOrtFormat(const onnxruntime::fbs::Node& fbs_node, Graph& graph,
                               const OrtFormatLoadOptions& load_options,
                               const Ort::Logger& logger, std::unique_ptr<Node>& node) {
  node = std::make_unique<Node>(fbs_node.index(), graph);
  return node->LoadFromOrtFormat(fbs_node, load_options, logger);
}

Ort::Status Node::LoadFromOrtFormat(const onnxruntime::fbs::Node& fbs_node,
                               const OrtFormatLoadOptions& load_options,
                               const Ort::Logger& logger) {
  auto LoadNodeArgsFromOrtFormat =
      [&](const flatbuffers::Vector<flatbuffers::Offset<flatbuffers::String>>* fbs_node_arg_names,
          std::vector<NodeArg*>& node_args,
          bool check_parent_graph = false) -> Ort::Status {
...
Ort::Status Node::LoadEdgesFromOrtFormat(const onnxruntime::fbs::NodeEdge& fbs_node_edges,
                                    const Graph& graph) {
  ...
  auto add_edges = [&graph](const flatbuffers::Vector<const onnxruntime::fbs::EdgeEnd*>* fbs_edges,
                            EdgeSet& edge_set, const std::string& dst_name) -> Ort::Status {
    if (fbs_edges) {
...
#if !defined(ORT_MINIMAL_BUILD)
Ort::Status Node::UpdateInputArgCount() {
...
// Ort::Status Graph::LoadGraph(const GraphProto& graph_proto,
...
Graph::Graph(const Model& owning_model,
  ...
  if (owning_model_.IsLoadCancellationFlagSet()) {
    ORT_THROW_WITH_ERROR_CODE(ORT_MODEL_LOAD_CANCELED, "Graph loading canceled due to user request.");
  }
  ...
  if (graph_proto_->sparse_initializer_size() > 0) {
    for (const auto& sparse_tensor : graph_proto_->sparse_initializer()) {
      ...
      ORT_ENFORCE(status.IsOK(), status.GetErrorMessage());
      auto p = sparse_tensor_names_.emplace(tensor->name());
...
Ort::Status Graph::VerifyNoDuplicateName() {
...
Ort::Status Graph::BuildConnections(std::unordered_set<std::string>& outer_scope_node_args_consumed,
        ...
        if (resolve_context_.output_args.end() != output_arg_iter) {
          ...
          if (parent_graph_ != nullptr) {
            ...
            if (check &&
              ...
              return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Invalid model. Node input '", input_arg_name,
...
Ort::Status Graph::PerformTopologicalSortAndCheckIsAcyclic() {
...
using SubgraphInferencingFunc =
    std::function<Ort::Status(const Node&, Graph&, const std::vector<const TypeProto*>&, std::vector<const TypeProto*>&,
...
class GraphInferencerImpl : public ONNX_NAMESPACE::GraphInferencer {
 public:
  ...
  std::vector<const TypeProto*> doInferencing(const std::vector<const TypeProto*>& input_types,
    ...
    if (status !=STATUS_OK) {
      fail_type_inference("Graph attribute inferencing failed: ", status.GetErrorMessage());
    }
...
Ort::Status Graph::SaveShapeValuesFromDataPropagation(const Node& node,
                                                 NodeArg& output_def,
  ...
  auto get_initialized_input_values_func = [&](const std::string& input_name, TensorShapeVector& input_values)
    ...
    if (initializer) {
      ...
      size_t element_cnt = gsl::narrow<size_t>(tensor_shape.Size());
...
Ort::Status Graph::InferAndVerifySubgraphTypes(const Node& node, Graph& subgraph,
  ...
  if (num_subgraph_inputs != input_types.size()) {
    ...
    if (num_required_subgraph_inputs != input_types.size()) {
      return ORT_MAKE_STATUS(ORT_FAIL, "Size mismatch validating subgraph inputs. Got ", input_types.size(),
  ...
  for (size_t i = 0; i < num_subgraph_inputs; ++i) {
    ...
    if (!status.IsOK()) {
      return ORT_MAKE_STATUS(ORT_FAIL, "Node:", node.Name(), " ", status.GetErrorMessage());
    }
  }
  ...
  for (const auto* implicit_node_arg : implicit_input_defs) {
    ...
    if (subgraph_nodearg->Type() == nullptr)
      return ORT_MAKE_STATUS(ORT_FAIL, "Subgraph input missing type.");
  }
...
Ort::Status Graph::UpdateShapeInference(Node& node) {
...
Ort::Status Graph::InferAndVerifyTypeMatch(Node& node, const OpSchema& op, const ResolveOptions& options) {
  auto& node_name = node.Name();
...

  {
    ...
    ORT_CATCH(const std::exception& ex) {
      ORT_HANDLE_EXCEPTION([&]() {
        status = ORT_MAKE_STATUS(ORT_FAIL, "Node (", node.Name(), ") Op (", node.OpType(), ") ", ex.what());
      });
    }
    ORT_RETURN_IF_ERROR(status);
  }
  ...
  for (auto& output_def : node.MutableDefinitions().output_defs) {
    ...
    if (utils::HasShape(onnx_inferred_type)) {
      if (output_def->Shape() == nullptr) {
        ...
        if (!status.IsOK()) {
          return ORT_MAKE_STATUS(ORT_FAIL, "Node:", node_name, " ", status.GetErrorMessage());
        }
...
Ort::Status Graph::TypeCheckInputsAndInitializers() {
...
Ort::Status Graph::VerifyNodeAndOpMatch(const ResolveOptions& options) {
  CheckerContext ctx;
...
Ort::Status Graph::CleanUpShapeValuesFromDataPropagation() {
  for (auto node_index : nodes_in_topological_order_) {
    auto& node = *GetNode(node_index);
...
Ort::Status Graph::VerifyInputAndInitializerNames() {
...
Ort::Status Graph::InitInputsInitializersOutputs() {
...
Ort::Status Graph::PerformTypeAndShapeInferencing(const ResolveOptions& options) {
...
Ort::Status Graph::Resolve(const ResolveOptions& options) {
  if (parent_graph_) {
...
Ort::Status Graph::ConvertInitializersIntoOrtValues() {
  std::vector<Graph*> all_subgraphs;
  FindAllSubgraphs(all_subgraphs);
  ...
  auto put_weights_maybe_in_memory_func = [&](Graph& graph) -> Ort::Status {
...
Ort::Status Graph::AddInitializedOrtValue(const ONNX_NAMESPACE::TensorProto& tensor_proto,
...
Ort::Status Graph::ForThisAndAllSubgraphs(const std::vector<Graph*>& subgraphs, std::function<Ort::Status(Graph&)> func) {
...
Ort::Status Graph::RemovedUnusedInitializersOrtFormat() {
  std::vector<Graph*> all_subgraphs;
  FindAllSubgraphs(all_subgraphs);
...
#if !defined(ORT_MINIMAL_BUILD)
Ort::Status Graph::ReplaceInitializedTensorImpl(ONNX_NAMESPACE::TensorProto new_initializer,
...
Ort::Status Graph::ReplaceInitializedTensor(const ONNX_NAMESPACE::TensorProto& new_initializer,
...
Ort::Status Graph::InjectExternalInitializedTensors(const InlinedHashMap<std::string, OrtValue>& external_initializers) {
...
Ort::Status Graph::InjectExternalInitializersFromFilesInMemory(
  ...
  for (const auto& [tensor_name, tensor_proto] : name_to_initial_tensor_) {
    if (utils::HasExternalDataInFile(*tensor_proto)) {
      ...
      ORT_RETURN_IF(user_provided_entry == external_initializer_files.end(),
                    "External file: ", std::filesystem::path{external_file}.string(),
                    " not found from the table user provided.");

      auto user_provided_length = user_provided_entry->second.second;

      ORT_RETURN_IF(file_offset < 0 || end_of_read > gsl::narrow<FileOffsetType>(user_provided_length),
...
Ort::Status Graph::LoadExternalInitializerAsOrtValue(const std::string& name, OrtValue& value) const {
...
Ort::Status Graph::SaveToOrtFormat(flatbuffers::FlatBufferBuilder& builder,
...
Ort::Status InlineOrCopyInitializer(const Graph& src_graph, const ONNX_NAMESPACE::TensorProto& initializer,
...
Ort::Status Graph::RegenerateInitializersAndReplaceInMemory(gsl::span<const InitializedTensorSet::const_iterator> iterators,
...
static Ort::Status GetSubgraphsWithMatchingGraphProtos(const GraphNodes& nodes,
...
Ort::Status Graph::ProcessSubgraphsInMemoryData(ONNX_NAMESPACE::GraphProto& output_graph_proto) const {
...
Ort::Status Graph::AddExternalInitializersToGraphProtoImpl(
  ...
  for (const auto& initializer : graph_proto_->initializer()) {
    ...
    if (IsSparseInitializer(initializer.name())) {
      ...
      external_offset = external_offset + tensor_bytes_size;
...
Ort::Status Graph::ToGraphProtoWithCustomInitializerHandlingImpl(
  ...
  for (gsl::not_null<const ONNX_NAMESPACE::TensorProto*> initializer : initializers) {
    ...
    if (IsSparseInitializer(initializer->name())) {
      ...
      Ort::Status status = ToStatusAndRelease(handle_initializer_func(state, initializer->name().c_str(),
                                                                 &ort_value,
...
Ort::Status Graph::ToGraphProtoWithCustomInitializerHandling(OrtGetInitializerLocationFunc handle_initializer_func,
                                                        void* state,
...
Ort::Status Graph::SetGraphInputsOutputs() {
  ...
  // inputs and outputs.
  if (is_loaded_from_model_file_) return STATUS_OK;
...
#if !defined(ORT_MINIMAL_BUILD) || defined(ORT_EXTENDED_MINIMAL_BUILD)
Ort::Status Graph::PopulateNodeArgToProducerConsumerLookupsFromNodes() {
...
Ort::Status Graph::AddConstantProtoAsInitializer(const ONNX_NAMESPACE::NodeProto& node_proto,
...
Ort::Status Graph::InlineIfSubgraph(bool condition_value, Node& if_node, const Ort::Logger& logger) {
...
Ort::Status Graph::InlineFunctionProto(const ONNX_NAMESPACE::FunctionProto& func_to_inline) {
...
Ort::Status Graph::InlineFunction(Node& callnode) {
...
Ort::Status Graph::LoadFromOrtFormat(const onnxruntime::fbs::Graph& fbs_graph,
...
#endif
                                const OrtFormatLoadOptions& load_options,
                                const Ort::Logger& logger, std::unique_ptr<Graph>& graph) {
...
Ort::Status Graph::LoadFromOrtFormat(const onnxruntime::fbs::Graph& fbs_graph,
  ...
  auto add_node_args = [&](const flatbuffers::Vector<flatbuffers::Offset<flatbuffers::String>>* fbs_node_args,
                           std::vector<const NodeArg*>& node_args) -> Ort::Status {
...
Ort::Status Graph::LoadFromModelEditorApiModel(const OrtGraph& api_graph, bool updating_existing_graph) {
  ArgNameToTypeMap name_to_type_map;
...
// static
Ort::Status Graph::LoadFromModelEditorApiModel(const OrtGraph& api_graph,
                                          const Model& owning_model,
                                          const std::unordered_map<std::string, int>& domain_to_version,
                                          IOnnxRuntimeOpSchemaCollectionPtr schema_registry,
                                          bool strict_shape_type_inference,
...
Ort::Status Graph::UpdateUsingModelEditorApiModel(const OrtModel& api_model) {
