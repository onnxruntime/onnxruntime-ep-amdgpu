...
Ort::Status OnnxRuntimeOpSchemaRegistry::SetBaselineAndOpsetVersionForDomain(
  ...
  if (domain_version_range_map_.end() != it) {
    return ORT_MAKE_STATUS(ORT_FAIL, "Domain already set in registry");
  }
...
Ort::Status OnnxRuntimeOpSchemaRegistry::RegisterOpSet(
...
Ort::Status OnnxRuntimeOpSchemaRegistry::RegisterOpSchema(ONNX_NAMESPACE::OpSchema&& op_schema) {
  return RegisterOpSchemaInternal(std::move(op_schema));
}

Ort::Status OnnxRuntimeOpSchemaRegistry::RegisterOpSchemaInternal(ONNX_NAMESPACE::OpSchema&& op_schema) {
  ...
  ORT_CATCH(const std::exception& e) {
    ORT_HANDLE_EXCEPTION([&]() {
      status = ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Schema error: ", std::string(e.what()));
    });
  }
  ...
  if (map_[op_name][op_domain].count(ver)) {
    ...
    return STATUS_OK;  // an op with the same name can be registered for multiple execution providers
  }
  ...
  if (ver_range_it == domain_version_range_map_.end()) {
    ...
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, ostream.str());
  }
