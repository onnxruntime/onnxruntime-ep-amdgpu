...
class CustomDataPropagationBase {
 public:
  ...
  virtual Ort::Status infer() = 0;

 protected:
  CustomDataPropagationBase(const Node& node,
                            NodeArg& output_def,
                            std::function<Ort::Status(const std::string&, TensorShapeVector&)> func,
  ...
  const Node& node_;
  NodeArg& output_def_;
  std::function<Ort::Status(const std::string&, TensorShapeVector&)> get_initialized_input_values_func_;
