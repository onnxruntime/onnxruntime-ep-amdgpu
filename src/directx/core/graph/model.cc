...
Model::Model(const ModelProto& model_proto, const PathString& model_path,
             const IOnnxRuntimeOpSchemaRegistryList* local_registries, const Ort::Logger& logger,
             const ModelOptions& options)
...
Model::Model(ModelProto&& model_proto, const PathString& model_path,
             const IOnnxRuntimeOpSchemaRegistryList* local_registries,
             const Ort::Logger& logger, const ModelOptions& options)
    : model_path_(model_path), check_load_cancellation_fn_(options.check_load_cancellation_fn) {
...
Ort::Status Model::ToGraphProtoWithCustomInitializerHandling(OrtGetInitializerLocationFunc handle_initializer_func,
                                                                void* state,
...
Ort::Status Model::Load(std::istream& model_istream, ModelProto* p_model_proto) {
  if (!model_istream.good()) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Invalid istream object.");
  }
  if (!p_model_proto) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Null model_proto ptr.");
  }

  google::protobuf::io::IstreamInputStream zero_copy_input(&model_istream);
  const bool result = p_model_proto->ParseFromZeroCopyStream(&zero_copy_input) && model_istream.eof();
  if (!result) {
    return ORT_MAKE_STATUS(ORT_INVALID_PROTOBUF, "Failed to load model because protobuf parsing failed.");
  }
  return STATUS_OK;
}

Ort::Status Model::Load(const ModelProto& model_proto,
...
Ort::Status Model::Load(const ModelProto& model_proto,
  ...
  if (!utils::HasGraph(model_proto)) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "No graph was found in the protobuf.");
  }
  ...
  ORT_CATCH(const OnnxRuntimeException& ex) {
    ORT_HANDLE_EXCEPTION([&]() {
      status = ORT_MAKE_STATUS(ex.ErrorCode(), ex.what());
    });
  }
  ORT_CATCH(const std::exception& ex) {
    ORT_HANDLE_EXCEPTION([&]() {
      status = ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Failed to load model with error: " + std::string(ex.what()));
    });
  }
...
Ort::Status Model::Load(ModelProto&& model_proto,
                   std::shared_ptr<Model>& model,
                   const IOnnxRuntimeOpSchemaRegistryList* local_registries,
                   const Ort::Logger& logger,
                   const ModelOptions& options) {
...
static Ort::Status LoadModelHelper(const T& file_path, Loader loader) {
  int fd;
  Ort::Status status = Env::Default().FileOpenRd(file_path, fd);
  if (!status.IsOK()) {
    switch (status.GetErrorCode()) {
      case ENOENT:
        return ORT_MAKE_STATUS(ORT_NO_SUCHFILE, "Load model ", ToUTF8String(file_path),
                               " failed. File doesn't exist");
      case EINVAL:
        return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Load model ", ToUTF8String(file_path), " failed");
      default:
        return ORT_MAKE_STATUS(ORT_FAIL, "system error number ", static_cast<int>(status.GetErrorCode()));
    }
  }
  ...
  ORT_CATCH(const std::exception& ex) {
    ORT_HANDLE_EXCEPTION([&]() {
      status = ORT_MAKE_STATUS(ORT_FAIL, ex.what());
    });
  }
...
template <typename T>
static Ort::Status LoadModel(const T& file_path, ONNX_NAMESPACE::ModelProto& model_proto) {
...
template <typename T>
static Ort::Status LoadModel(const T& file_path, std::shared_ptr<Model>& p_model,
                        const IOnnxRuntimeOpSchemaRegistryList* local_registries,
                        const Ort::Logger& logger, const ModelOptions& options) {
...
template <typename T>
static Ort::Status SaveModel(Model& model, const T& file_path) {
...
#else
  int fd;
  Ort::Status status = Env::Default().FileOpenWr(file_path, fd);
  ORT_RETURN_IF_ERROR(status);
...
Ort::Status Model::Save(Model& model, const PathString& file_path) {
  return SaveModel(model, file_path);
}

template <typename T>
static Ort::Status SaveModelWithExternalInitializers(Model& model,
                                                const T& file_path,
...
Ort::Status Model::Load(const PathString& file_path,
...
GSL_SUPPRESS(r .35)
Ort::Status Model::Load(const PathString& file_path, std::shared_ptr<Model>& p_model,
                   const IOnnxRuntimeOpSchemaRegistryList* local_registries,
                   const Ort::Logger& logger, const ModelOptions& options) {
  return LoadModel(file_path, p_model, local_registries, logger, options);
}

Ort::Status Model::SaveWithExternalInitializers(Model& model, const std::filesystem::path& file_path,
                                           const std::filesystem::path& external_file_name,
                                           const ModelSavingOptions& save_options) {
  return SaveModelWithExternalInitializers(model, file_path, external_file_name, save_options);
}

Ort::Status Model::LoadFromBytes(int count, const void* p_bytes, /*out*/ ONNX_NAMESPACE::ModelProto& model_proto) {
  const bool result = model_proto.ParseFromArray(p_bytes, count);
  if (!result) {
    return ORT_MAKE_STATUS(ORT_INVALID_PROTOBUF, "Protobuf parsing failed.");
  }

  return STATUS_OK;
}

Ort::Status Model::LoadFromBytes(int count, void* p_bytes, /*out*/ std::shared_ptr<Model>& p_model,
                            const IOnnxRuntimeOpSchemaRegistryList* local_registries, const Ort::Logger& logger,
                            const ModelOptions& options) {
  return LoadFromBytes(count, p_bytes, PathString{}, p_model, local_registries, logger, options);
}

Ort::Status Model::LoadFromBytes(int count, void* p_bytes, const PathString& model_path,
...
Ort::Status Model::Load(int fd, ONNX_NAMESPACE::ModelProto& model_proto) {
  if (fd < 0) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "<p_fd> less than 0.");
  }

#if GOOGLE_PROTOBUF_VERSION >= 3002000
  size_t file_size = 0;
  int block_size = -1;
  Ort::Status st = Env::Default().GetFileLength(fd, file_size);
  if (st.IsOK()) {
...
Ort::Status Model::Load(int fd, std::shared_ptr<Model>& p_model, const IOnnxRuntimeOpSchemaRegistryList* local_registries,
                   const Ort::Logger& logger, const ModelOptions& options) {
  return Load(fd, PathString{}, p_model, local_registries, logger, options);
}

Ort::Status Model::Load(int fd, const PathString& model_path, std::shared_ptr<Model>& p_model,
                   const IOnnxRuntimeOpSchemaRegistryList* local_registries, const Ort::Logger& logger,
                   const ModelOptions& options) {
  ModelProto model_proto;
...
// static
Ort::Status Model::LoadFromModelEditorApiModel(const OrtModel& model_editor_api_model,
                                                  const IOnnxRuntimeOpSchemaRegistryList* local_registries,
                                                  const ModelOptions& options,
                                                  const Ort::Logger& logger,
  ...
  if (auto onnx_iter = domain_to_version.find(kOnnxDomain); onnx_iter == domain_to_version.end()) {
    ...
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "The opset for the ONNX domain must be explicitly specified.");
  }
...
Ort::Status Model::Save(Model& model, int p_fd) {
  if (p_fd < 0) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "<p_fd> is less than 0.");
  }
  ...
  return ORT_MAKE_STATUS(ORT_INVALID_PROTOBUF, "Protobuf serialization failed.");
}

Ort::Status Model::SaveWithExternalInitializers(Model& model,
                                           int fd,
                                           const std::filesystem::path& file_path,
                                           const std::filesystem::path& external_file_name,
                                           const ModelSavingOptions& model_saving_options) {
  if (fd < 0) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "<fd> is less than 0.");
  }
...
Ort::Status Model::SaveToOrtFormat(flatbuffers::FlatBufferBuilder& builder,
...
Ort::Status Model::LoadFromOrtFormat(const fbs::Model& fbs_model,
#if !defined(ORT_MINIMAL_BUILD)
                                        const IOnnxRuntimeOpSchemaRegistryList* local_registries,
#endif
