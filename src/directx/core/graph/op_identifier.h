...
struct BasicOpIdentifier {
  ...
  static Ort::Status LoadFromString(std::string_view op_id_str, BasicOpIdentifier& op_id) {
...
template <typename T> struct fmt::formatter<onnxruntime::BasicOpIdentifier<T>> : formatter<std::string> {
    format_context::iterator format(const onnxruntime::BasicOpIdentifier<T> op_id, format_context& ctx) const {
        return formatter<std::string>::format(op_id.ToString(), ctx);
    }
};
