...
static Ort::Status CheckCopyDestination(std::string_view error_array_label, size_t src_size, gsl::span<DstElem const> dst) {
  ...
  if (dst.data() == nullptr && src_size > 0) {
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Buffer to store ", error_array_label, " is NULL.");
  }
...
EpNode::EpNode(const EpGraph* ep_graph, const Node& node, PrivateTag)
    : OrtNodePlugin(OrtGraphIrApi::kEpApi), ep_graph_(ep_graph), node_(node) {}

Ort::Status EpNode::Create(const Node& node, const EpGraph* ep_graph,
                      std::unordered_map<std::string, std::unique_ptr<EpValueInfo>>& value_infos_map,
...
Ort::Status EpNode::GetSinceVersion(int& since_version) const {
...
Ort::Status EpNode::GetInputs(gsl::span<const OrtValueInfo*> dst) const {
...
Ort::Status EpNode::GetOutputs(gsl::span<const OrtValueInfo*> dst) const {
...
Ort::Status EpNode::GetNumImplicitInputs(size_t& num_implicit_inputs) const {
  num_implicit_inputs = implicit_inputs_.size();
  return STATUS_OK;
}

Ort::Status EpNode::GetImplicitInputs(gsl::span<const OrtValueInfo*> dst) const {
...
Ort::Status EpNode::GetAttributes(gsl::span<const OrtOpAttr*> dst) const {
...
Ort::Status EpNode::GetNumSubgraphs(size_t& num_subgraphs) const {
  num_subgraphs = subgraphs_.size();
  return STATUS_OK;
}

Ort::Status EpNode::GetSubgraphs(gsl::span<const OrtGraph*> subgraphs,
...
Ort::Status EpNode::GetGraph(const OrtGraph*& parent_graph) const {
...
static Ort::Status GetInputIndices(const EpNode& consumer_node,
...
static Ort::Status GetOutputIndex(const EpNode& producer_node,
                             const std::string& value_info_name,
                             /*out*/ size_t& index) {
  bool found = false;
...
Ort::Status EpValueInfo::GetProducerInfo(OrtValueInfo::ProducerInfo& producer_info) const {
#if !defined(ORT_MINIMAL_BUILD) || defined(ORT_EXTENDED_MINIMAL_BUILD)
  producer_info.producer_node = nullptr;
  producer_info.output_index = 0;

  if (graph_ == nullptr) {
    return ORT_MAKE_STATUS(ORT_NOT_FOUND, "Unable to get producer node for OrtValueInfo '", name_,
  ...
  producer_info.producer_node = ep_node->ToExternal();
...
Ort::Status EpValueInfo::GetConsumerInfos(std::vector<OrtValueInfo::ConsumerInfo>& result) const {
#if !defined(ORT_MINIMAL_BUILD) || defined(ORT_EXTENDED_MINIMAL_BUILD)
  if (graph_ == nullptr) {
    return ORT_MAKE_STATUS(ORT_FAIL, "Unable to get uses of OrtValueInfo '", name_,
                           "' that is not owned by a OrtGraph.");
  }
...
Ort::Status EpValueInfo::GetNumConsumerInfos(size_t& num_consumers) const {
#if !defined(ORT_MINIMAL_BUILD) || defined(ORT_EXTENDED_MINIMAL_BUILD)
  num_consumers = 0;

  if (graph_ == nullptr) {
    return ORT_MAKE_STATUS(ORT_FAIL, "Unable to get number of uses of OrtValueInfo '", name_,
                           "' that is not owned by a OrtGraph.");
  }
...
Ort::Status EpValueInfo::GetInitializerValue(const OrtValue*& result) const {
...
Ort::Status EpValueInfo::GetExternalInitializerInfo(std::unique_ptr<ExternalDataInfo>& result) const {
  if (!IsFlagSet(kIsConstantInitializer) && !IsFlagSet(kIsOptionalGraphInput)) {
    result = nullptr;
    return STATUS_OK;
  }
...
Ort::Status EpValueInfo::IsRequiredGraphInput(bool& is_required_graph_input) const {
  is_required_graph_input = IsFlagSet(Flags::kIsRequiredGraphInput);
  return STATUS_OK;
}

Ort::Status EpValueInfo::IsOptionalGraphInput(bool& is_optional_graph_input) const {
  is_optional_graph_input = IsFlagSet(Flags::kIsOptionalGraphInput);
  return STATUS_OK;
}

Ort::Status EpValueInfo::IsGraphOutput(bool& is_graph_output) const {
  is_graph_output = IsFlagSet(Flags::kIsGraphOutput);
  return STATUS_OK;
}

Ort::Status EpValueInfo::IsConstantInitializer(bool& is_const_initializer) const {
  is_const_initializer = IsFlagSet(Flags::kIsConstantInitializer);
  return STATUS_OK;
}

Ort::Status EpValueInfo::IsFromOuterScope(bool& is_outer_scope) const {
...
Ort::Status EpGraph::Create(const GraphViewer& graph_viewer, /*out*/ std::unique_ptr<EpGraph>& result, bool create_parent_node) {
...
// Static class function to create a std::unique_ptr<EpGraph>.
Ort::Status EpGraph::Create(std::unique_ptr<GraphViewer> src_graph_viewer,
...
Ort::Status EpGraph::CreateImpl(std::unique_ptr<EpGraph> ep_graph, const GraphViewer& graph_viewer,
...
Ort::Status EpGraph::GetNumOperatorSets(size_t& num_operator_sets) const {
  num_operator_sets = graph_viewer_.DomainToVersionMap().size();
  return STATUS_OK;
}

Ort::Status EpGraph::GetOperatorSets(gsl::span<const char*> domains,
...
Ort::Status EpGraph::GetInputs(gsl::span<const OrtValueInfo*> dst) const {
  const size_t num_inputs = inputs_.size();
...
Ort::Status EpGraph::GetOutputs(gsl::span<const OrtValueInfo*> dst) const {
  const size_t num_outputs = outputs_.size();
...
Ort::Status EpGraph::GetInitializers(gsl::span<const OrtValueInfo*> dst) const {
...
Ort::Status EpGraph::GetNodes(gsl::span<const OrtNodePlugin*> dst) const {
  const size_t num_nodes = nodes_.size();
  ORT_RETURN_IF_ERROR((CheckCopyDestination<const OrtNodePlugin*>("graph nodes", num_nodes, dst)));
...
Ort::Status EpGraph::GetParentNode(const OrtNodePlugin*& result) const {
...
Ort::Status EpGraph::GetInitializerValue(std::string_view name, const OrtValue*& result) const {
  auto ensure_ort_value_loaded = [&](const std::unique_ptr<OrtValue>& ort_value) -> Ort::Status {
