...
class Model {
 public:
  ...
  static Ort::Status Save(Model& model, const PathString& file_path);

  static Ort::Status Save(Model& model, int fd);

  // Save the model to file using an external file for initializers larger than the given threshold (in bytes).
  // Initializer offset could be page aligned and allocation granularity aligned for mmap support.
  static Ort::Status SaveWithExternalInitializers(Model& model,
                                                     const std::filesystem::path& file_path,
  ...
  static Ort::Status Load(std::istream& model_istream, ONNX_NAMESPACE::ModelProto* p_model_proto);

  static Ort::Status Load(const PathString& file_path,
  ...
  static Ort::Status Load(int fd, /*out*/ ONNX_NAMESPACE::ModelProto& model_proto);

  static Ort::Status Load(int fd, /*out*/ std::shared_ptr<Model>& p_model,
                             const IOnnxRuntimeOpSchemaRegistryList* local_registries,
                             const Ort::Logger& logger,
                             const ModelOptions& options = {});

  static Ort::Status Load(int fd,
                             const PathString& model_path,
                             /*out*/ std::shared_ptr<Model>& p_model,
                             const IOnnxRuntimeOpSchemaRegistryList* local_registries,
                             const Ort::Logger& logger,
  ...
  static Ort::Status LoadFromBytes(int count, const void* pBytes,
                                      /*out*/ ONNX_NAMESPACE::ModelProto& model_proto);

  // 'int' rather than 'size_t' because of a protobuf design choice; let callers handle type checks
  static Ort::Status LoadFromBytes(int count, void* pBytes, /*out*/ std::shared_ptr<Model>& p_model,
                                      const IOnnxRuntimeOpSchemaRegistryList* local_registries,
                                      const Ort::Logger& logger,
                                      const ModelOptions& options = {});

  // 'int' rather than 'size_t' because of a protobuf design choice; let callers handle type checks
  static Ort::Status LoadFromBytes(int count, void* pBytes,
                                      const PathString& model_path,
                                      /*out*/ std::shared_ptr<Model>& p_model,
                                      const IOnnxRuntimeOpSchemaRegistryList* local_registries,
                                      const Ort::Logger& logger,
  ...
  static Ort::Status Load(const ONNX_NAMESPACE::ModelProto& model_proto, /*out*/ std::shared_ptr<Model>& p_model,
                             const IOnnxRuntimeOpSchemaRegistryList* local_registries,
                             const Ort::Logger& logger,
                             const ModelOptions& options = {});

  static Ort::Status Load(const ONNX_NAMESPACE::ModelProto& model_proto,
                             const PathString& model_path,
                             /*out*/ std::shared_ptr<Model>& p_model,
                             const IOnnxRuntimeOpSchemaRegistryList* local_registries,
                             const Ort::Logger& logger,
  ...
  static Ort::Status Load(ONNX_NAMESPACE::ModelProto&& model_proto,
                             /*out*/ std::shared_ptr<Model>& p_model,
                             const IOnnxRuntimeOpSchemaRegistryList* local_registries,
                             const Ort::Logger& logger,
                             const ModelOptions& options = {});
  ...
  static Ort::Status LoadFromModelEditorApiModel(const OrtModel& graph_api_model,
                                                    const IOnnxRuntimeOpSchemaRegistryList* local_registries,
                                                    const ModelOptions& options,
                                                    const Ort::Logger& logger,
  ...
  static Ort::Status LoadFromOrtFormat(const onnxruntime::fbs::Model& fbs_model,
#if !defined(ORT_MINIMAL_BUILD)
                                          const IOnnxRuntimeOpSchemaRegistryList* local_registries,
#endif
...
template <> struct fmt::formatter<ONNX_NAMESPACE::Version> : formatter<std::string_view> {
    format_context::iterator format(ONNX_NAMESPACE::Version format, format_context& ctx) const {
        std::string_view name{"invalid"};
        switch (format) {
        case ONNX_NAMESPACE::Version::_START_VERSION:
            name = "START-VERSION";
            break;
        case ONNX_NAMESPACE::Version::IR_VERSION_2017_10_10:
            name = "2017-10-10";
            break;
        case ONNX_NAMESPACE::Version::IR_VERSION_2017_10_30:
            name = "2017-10-30";
            break;
        case ONNX_NAMESPACE::Version::IR_VERSION_2017_11_3:
            name = "2017-11-3";
            break;
        case ONNX_NAMESPACE::Version::IR_VERSION_2019_1_22:
            name = "2019-11-22";
            break;
        case ONNX_NAMESPACE::Version::IR_VERSION_2019_3_18:
            name = "2019-11-18";
            break;
        case ONNX_NAMESPACE::Version::IR_VERSION_2019_9_19:
            name = "2019-09-19";
            break;
        case ONNX_NAMESPACE::Version::IR_VERSION_2020_5_8:
            name = "2020-5-8";
            break;
        case ONNX_NAMESPACE::Version::IR_VERSION_2021_7_30:
            name = "2021-7-30";
            break;
        case ONNX_NAMESPACE::Version::IR_VERSION_2023_5_5:
            name = "2023-5-5";
            break;
        case ONNX_NAMESPACE::Version::IR_VERSION_2024_3_25:
            name = "2024-3-25";
            break;
        case ONNX_NAMESPACE::Version::IR_VERSION_2025_05_12:
            name = "2025-05-12";
            break;
        case ONNX_NAMESPACE::Version::IR_VERSION_2025_08_26:
            name = "2025-08-26";
            break;
        case ONNX_NAMESPACE::Version::IR_VERSION:
            break;
        }
