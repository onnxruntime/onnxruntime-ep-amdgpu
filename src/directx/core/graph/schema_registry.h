...
class OnnxRuntimeOpSchemaRegistry : public IOnnxRuntimeOpSchemaCollection {
 public:
  ...
  Ort::Status SetBaselineAndOpsetVersionForDomain(
      const std::string& domain,
      int baseline_opset_version,
      int opset_version);
 ...
 private:
  Ort::Status RegisterOpSchema(ONNX_NAMESPACE::OpSchema&& op_schema);

  Ort::Status RegisterOpSchemaInternal(ONNX_NAMESPACE::OpSchema&& op_schema);
