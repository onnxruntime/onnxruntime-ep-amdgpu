...
Ort::Status MoveInputOutputImpl(Graph& graph, const ValueMoveInfo& move_info, Node& src, Node& dest,
  ...
  auto process = [&](int src_idx) {
    ...
    if (!valid_index) {
      return ORT_MAKE_STATUS(ORT_FAIL, "Index out of range");
    }
...
Ort::Status MoveInputOutput(Graph& graph, Node& src, Node& dest, const ValueMoveInfo& move_info,
                       bool only_update_dest_definitions) {
  return MoveInputOutputImpl(graph, move_info, src, dest, only_update_dest_definitions);
}

Ort::Status MoveInputOutput(Graph& graph, const NodesToOptimize& selected_nodes, Node& dest,
