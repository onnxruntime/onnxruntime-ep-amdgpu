...
Ort::Status DuplicateDQForOutputEdge(const graph_utils::GraphEdge& original_dq_output_edge, Graph& graph) {
  // DQ
...
Ort::Status EnsureUniqueDQForEachExplicitOutputEdge(const Node& node, Graph& graph, bool& modified) {
  ...
  const auto dq_output_edges_to_explicit_inputs_end = std::remove_if(
      ...
      [&const_graph = std::as_const(graph)](const graph_utils::GraphEdge& dq_output_edge) {
        ...
        return gsl::narrow<size_t>(dq_output_edge.dst_arg_index) >= consumer_explicit_input_defs_count;
      });
...
Ort::Status EnsureUniqueDQForNodeUnit::ApplyImpl(Graph& graph,
                                            bool& modified,
                                            int graph_level,
