...
Ort::Status WhereDummyDq::InsertDummyDQ(Node& node, Graph& graph, bool& modified, const Ort::Logger& logger) const {
  const auto& where_inputs = node.InputDefs();
  const Node* parent_node_1 = graph.GetProducerNode(where_inputs[1]->Name());
  const Node* parent_node_2 = graph.GetProducerNode(where_inputs[2]->Name());
...
Ort::Status WhereDummyDq::ApplyImpl(Graph& graph, bool& modified, int graph_level, const Ort::Logger& logger) const {
