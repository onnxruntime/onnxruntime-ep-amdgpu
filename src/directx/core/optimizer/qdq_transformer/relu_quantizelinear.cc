...
bool ReluQuantFusion::SatisfyCondition(const Graph& graph, const Node& node, const Ort::Logger& /*logger*/) const {
...
Ort::Status ReluQuantFusion::Apply(Graph& graph, Node& node, RewriteRuleEffect& rule_effect, const Ort::Logger&) const {
  ...
  Initializer zero_point(graph, *zp_tensor_proto, graph.ModelPath());
