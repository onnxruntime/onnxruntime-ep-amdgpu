...
bool ClipQuantFusion::SatisfyCondition(const Graph& graph, const Node& node, const Ort::Logger& logger) const {
...
Ort::Status ClipQuantFusion::Apply(Graph& graph, Node& node, RewriteRuleEffect& rule_effect, const Ort::Logger&) const {
  float min, max;
