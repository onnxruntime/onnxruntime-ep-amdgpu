...
Ort::Status GemmTransposeFusion::Apply(Graph& graph, Node& node, RewriteRuleEffect& modified, const Ort::Logger&) const {
  auto& gemm_node = node;
...
bool GemmTransposeFusion::SatisfyCondition(const Graph& graph, const Node& node, const Ort::Logger&) const {
