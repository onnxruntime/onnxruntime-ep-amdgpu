...
static bool CheckInput(NodeArg* input, const Ort::Logger& logger) {
...
The embeddings before conversion:
  (input_ids) -------->  Gather ---------+       (segment_ids)
    ...
    +---(optional graph)                      Add
                                       ...
                                       LayerNormalization
*/
Ort::Status EmbedLayerNormFusion::ApplyImpl(Graph& graph, bool& modified, int graph_level, const Ort::Logger& logger) const {
