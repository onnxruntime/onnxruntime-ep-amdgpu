...
class RuleBasedGraphTransformer : public GraphTransformer {
 public:
  ...
  Ort::Status Register(std::unique_ptr<RewriteRule> rule);
 ...
 protected:
  ...
  Ort::Status ApplyRulesOnNode(Graph& graph, Node& node,
                                  gsl::span<const std::reference_wrapper<const RewriteRule>> rules,
                                  RewriteRule::RewriteRuleEffect& rule_effect, const Ort::Logger& logger) const;
