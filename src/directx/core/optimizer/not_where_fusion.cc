...
Condition -> Where ->
      ...
      |----> Where ->
          ...
          v0----|
 */
bool NotWhereFusion::SatisfyCondition(const Graph& graph, const Node& node, const Ort::Logger& logger) const {
...
Ort::Status NotWhereFusion::Apply(Graph& graph, Node& node, RewriteRuleEffect& rule_effect, const Ort::Logger&) const {
  const Node* p_not_node = graph_utils::GetInputNode(node, 0);
