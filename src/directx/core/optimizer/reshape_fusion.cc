...
Ort::Status ReshapeFusion::ApplyImpl(Graph& graph, bool& modified, int graph_level, const Ort::Logger& logger) const {
...
*/
static bool Match_Linear_Subgraph_1(Graph& graph, const Node& concat, const Node& root, const Ort::Logger& logger) {
...
static bool Match_Shape(Graph& graph, const Node& concat, const Node& shape, const NodeArg& root_input, const Ort::Logger& logger) {
...
bool ReshapeFusion::Match_One_Element_Output_Subgraph_2(Graph& graph, const NodeArg& root_input, const Node& cur_node,
                                                        int index, const Ort::Logger& logger) {
  std::vector<graph_utils::EdgeEndToMatch> parent_path{
...
bool ReshapeFusion::Is_One_Element_Output_Subgraph(Graph& graph, const NodeArg& root_input, const Node& concat,
                                                   int index, gsl::span<const int64_t> shape_value, const Ort::Logger& logger) {
    ...
    [Sub-graph Root]   (Constant Initializer)
                  \         [0, a, 0, b]
                   \        /
                    Reshape
*/
bool ReshapeFusion::Fuse_Subgraph(Node& reshape, Graph& graph, const Ort::Logger& logger) {
