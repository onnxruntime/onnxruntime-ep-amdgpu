...
Ort::Status RuleBasedGraphTransformer::Register(std::unique_ptr<RewriteRule> rule) {
...
Ort::Status RuleBasedGraphTransformer::ApplyRulesOnNode(Graph& graph, Node& node,
                                                   gsl::span<const std::reference_wrapper<const RewriteRule>> rules,
                                                   RuleEffect& rule_effect, const Ort::Logger& logger) const {
...
Ort::Status RuleBasedGraphTransformer::ApplyImpl(Graph& graph, bool& modified, int graph_level, const Ort::Logger& logger) const {
