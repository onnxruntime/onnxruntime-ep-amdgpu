...
*/
Ort::Status GatherSliceToSplitFusion::ApplyImpl(Graph& graph, bool& modified, int graph_level,
  ...
  if (onnx_opset_version < 13) return STATUS_OK;
...
*/
Ort::Status GatherToSliceFusion::ApplyImpl(Graph& graph, bool& modified, int graph_level,
                                      const Ort::Logger& logger) const {
  GraphViewer graph_viewer(graph);
  const auto& node_topology_list = graph_viewer.GetNodesInTopologicalOrder();
