...
struct ExtractScalarAsFloatDispatchTarget {
  Ort::Status operator()(const ONNX_NAMESPACE::TensorProto& tensor_proto, const std::filesystem::path& model_path,
                    float& scalar_float) {
    T scalar;
...
std::optional<float> GetScalarConstantInitializer(const Graph& graph, const NodeArg& node_arg, size_t max_rank) {
  ...
  ORT_THROW_IF_ERROR(
      (dispatcher.InvokeRet<Ort::Status, ExtractScalarAsFloatDispatchTarget>(*initializer, graph.ModelPath(), scalar)));
...
Ort::Status MatMulScaleFusion::ApplyImpl(Graph& graph, bool& modified, int graph_level, const Ort::Logger& logger)
    const {
