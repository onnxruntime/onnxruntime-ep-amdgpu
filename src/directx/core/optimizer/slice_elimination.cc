...
Ort::Status EliminateSlice::Apply(Graph& graph, Node& node, RewriteRuleEffect& rule_effect, const Ort::Logger&) const {
...
bool EliminateSlice::SatisfyCondition(const Graph& graph, const Node& node, const Ort::Logger& logger) const {
