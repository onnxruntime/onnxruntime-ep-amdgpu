...
ReshapeInfo UpStreamReshapeGraphTransformer::PropagateReshapeForInput(
    ...
    ReshapeInfo& info, std::vector<DimCompare>& dim_compare_rets, const Ort::Logger& logger) const {
...
Ort::Status UpStreamReshapeGraphTransformer::RemoveOriginalReshapeNode(
    Graph& graph, Node& reshape_node, Node& current_node, const Ort::Logger& logger, ReshapeInfo& info) const {
...
std::optional<ReshapeInfo> UpStreamReshapeGraphTransformer::IsSupportedForUpstream(
    Graph& graph, Node& node, const Ort::Logger& logger) const {
