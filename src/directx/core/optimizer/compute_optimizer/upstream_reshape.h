...
class UpStreamReshapeGraphTransformer
 ...
 public:
  ...
  std::optional<optimizer::compute_optimizer::ReshapeInfo> IsSupportedForUpstream(
      Graph& graph, Node& node, const Ort::Logger& logger) const override;
 ...
 private:
   ...
   * @return
   */
  Ort::Status RemoveOriginalReshapeNode(Graph& graph, Node& reshape_node, Node& current_node,
                                   const Ort::Logger& logger, optimizer::compute_optimizer::ReshapeInfo& info) const;
};
