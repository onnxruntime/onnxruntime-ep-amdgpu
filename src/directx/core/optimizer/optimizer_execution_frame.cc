...
OptimizerExecutionFrame::Info::Info(const std::vector<const Node*>& nodes,
  ...
  auto initialize_maps = [this, &initialized_tensor_set, &model_path](const NodeArg& arg, size_t /*index*/) -> Ort::Status {
...
OptimizerExecutionFrame::Info::Info(const std::vector<const Node*>& nodes,
  ...
  // Create MLValues related maps
  auto initialize_maps = [this, &initialized_tensor_set](const NodeArg& arg, size_t /*index*/) -> Ort::Status {
    int idx = ort_value_name_idx_map_.Add(arg.Name());
    ort_value_idx_nodearg_map_.insert_or_assign(idx, &arg);
...
Ort::Status OptimizerExecutionFrame::Info::TryFindKernel(const Node* node, const KernelCreateInfo** out) const {
...
static Ort::Status TryCreateKernel(const Node& node,
...
Ort::Status OptimizerExecutionFrame::CopyTensor(const Tensor& src, Tensor& dest) const {
...
Ort::Status OptimizerExecutionFrame::CreateNodeOutputMLValueImpl(OrtValue& ort_value, int ort_value_idx, const TensorShape* shape) {
  ...
  if (ml_type->IsSparseTensorType()) {
    ...
    return STATUS_OK;
#else
    return ORT_MAKE_STATUS(ORT_INVALID_ARGUMENT, "Sparse tensor is not supported in this build");
#endif
  }
