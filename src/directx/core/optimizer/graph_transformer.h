...
class GraphTransformer {
  ...
  */
  Ort::Status Apply(Graph& graph, bool& modified, const Ort::Logger& logger) const;

  virtual bool ShouldOnlyApplyOnce() const { return false; }

 protected:
  /** Helper method to call ApplyImpl on any subgraphs in the Node. */
  Ort::Status Recurse(Node& node, bool& modified, int graph_level, const Ort::Logger& logger) const {
 ...
 private:
  ...
  // should suffice.
  virtual Ort::Status ApplyImpl(Graph& graph, bool& modified, int graph_level, const Ort::Logger& logger) const = 0;
