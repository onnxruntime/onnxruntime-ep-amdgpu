...
class RewriteRule {
  ...
  */
  ...
  /** Checks if the condition of the rule is satisfied, and if so applies the body of the rule.
      ...
      @returns Ort::Status indicating success or providing error information */
  Ort::Status CheckConditionAndApply(Graph& graph, Node& node, RewriteRuleEffect& rule_effect, const Ort::Logger& logger) const {
    return SatisfyCondition(graph, node, logger) ? Apply(graph, node, rule_effect, logger) :STATUS_OK;
  }
 ...
 private:
  ...
  /** Checks if the Node of the given Graph satisfies the conditions of this rule. The body of the rule will be
      ...
      of the nodes. */
  virtual bool SatisfyCondition(const Graph& graph, const Node& node, const Ort::Logger& logger) const = 0;
  ...
  virtual Ort::Status Apply(Graph& graph, Node& node, RewriteRuleEffect& rule_effect, const Ort::Logger& logger) const = 0;
};
}  // namespace onnxruntime
