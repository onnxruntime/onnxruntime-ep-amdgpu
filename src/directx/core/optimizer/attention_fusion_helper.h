...
bool CheckSliceParameters(const Graph& graph, const Node& slice, const std::vector<int>& input_indices,
                          const std::vector<int64_t>& expected_values, const Ort::Logger& logger) {
...
bool ValidateGemmInitializer(const Graph& graph, const Node& gemm, int64_t hidden_size, bool is_before_split, const Ort::Logger& logger) {
...
bool ValidateUnidirMask(const Graph& graph, const NodeArg& mask, bool& is_unidirectional, const Ort::Logger& logger) {
...
*/
bool MatchUnidirMaskSubgraph(const Graph& graph, const Node& add_node, MatchUnidirMaskResult& result, bool use_shared_node, const Ort::Logger& logger) {
...
*/
bool MatchInputMaskSubgraph(const Graph& graph, const Node& qkv_matmul, AttentionMaskNodes& result, const Ort::Logger& logger, bool is_input_mask_optional) {
...
bool MatchInputMaskSubgraph(const Graph& graph, const Node& layer_norm, const Node& qkv_matmul,
                            AttentionMaskNodesDistilBert& result, NodeIndex& record_node_idx, const Ort::Logger& logger) {
...
bool MatchPastSubgraph(Graph& graph, const Node& k_concat, const Node& v_concat, bool transpose_optimized_pattern,
                       MatchPastResult& result, const Ort::Logger& logger) {
...
bool CheckDistilBertReshapeShape(const Graph& graph, const Node& reshape, int64_t hidden_size, NodeIndex& record_node_idx, const Ort::Logger& logger) {
...
bool CheckNodesInPathV(const Graph& graph, const Node& reshape, const Node& transpose, const Node& qkv_matmul, const Node& v_transpose, const Node& v_reshape,
                       int64_t& num_heads, int64_t& head_size, int64_t hidden_size, NodeIndex& record_node_idx, const Ort::Logger& logger) {
...
bool CheckNodesInPathV(const Graph& graph, const Node& reshape, const Node& transpose, const Node& qkv_matmul, const Node& v_transpose, const Node& v_reshape,
                       int64_t& num_heads, int64_t& head_size, int64_t hidden_size, const Ort::Logger& logger) {
...
bool CheckNodesInPathQ(const Graph& graph, const Node& qk_div, const Node& q_reshape, const Node& q_transpose, int64_t num_heads, int64_t head_size, const Ort::Logger& logger) {
...
bool CheckNodesInPathK(const Graph& graph, const Node& k_reshape, const Node& k_transpose, int64_t num_heads,
                       int64_t head_size, bool tranpose_optimized_pattern, const Ort::Logger& logger) {
    InlinedVector<int64_t> perm;
...
*/
bool FuseGptAttention(Node& layer_norm, Graph& graph, int64_t hidden_size, std::map<std::string, NodeArg*>& mask_int32_map, bool use_shared_node, const Ort::Logger& logger) {
