...
Ort::Status FuseReluClip::Apply(Graph& graph, Node& node, RewriteRuleEffect& rule_effect, const Ort::Logger&) const {
  ...
  if (graph_utils::RemoveNode(graph, node)) {
    if (replace_min) {
      ...
      if (min_max_are_attributes) {
        ...
        auto* replacement_min_nodearg = &graph_utils::AddInitializerWithOrtValue(graph, replacement_min);
...
bool FuseReluClip::SatisfyCondition(const Graph& graph, const Node& node, const Ort::Logger& logger) const {
