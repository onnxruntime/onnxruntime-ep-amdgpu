...
Ort::Status ConvMulFusion::Apply(Graph& graph, Node& node, RewriteRuleEffect& rule_effect, const Ort::Logger&) const {
  auto& conv_node = node;
...
bool ConvMulFusion::SatisfyCondition(const Graph& graph, const Node& node, const Ort::Logger&) const {
