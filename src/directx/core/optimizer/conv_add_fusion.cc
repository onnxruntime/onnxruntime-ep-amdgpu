...
Ort::Status ConvAddFusion::Apply(Graph& graph, Node& node, RewriteRuleEffect& modified, const Ort::Logger&) const {
  auto& conv_node = node;
...
bool ConvAddFusion::SatisfyCondition(const Graph& graph, const Node& node, const Ort::Logger&) const {
