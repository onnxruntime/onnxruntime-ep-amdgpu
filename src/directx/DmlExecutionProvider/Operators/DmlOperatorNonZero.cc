...
public:
    ...
    {
        PluginDmlExecutionProviderImpl* executionProvider = static_cast<PluginDmlExecutionProviderImpl*>(m_executionProvider.Get());

        // Wrap OrtAllocator as IAllocator for use with onnxruntime::Tensor constructors
        auto gpuAllocator = WrapOrtAllocatorAsIAllocator(executionProvider->GetGpuAllocator());
        auto cpuInputAllocator = WrapOrtAllocatorAsIAllocator(executionProvider->GetCpuInputAllocator());
