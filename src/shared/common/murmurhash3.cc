...
FORCE_INLINE uint32_t getblock32(const uint32_t* p, ptrdiff_t i) {
    if constexpr (endian::native == endian::little) {
        return p[i];
    } else {
        const auto c = reinterpret_cast<const uint8_t*>(&p[i]);
...
FORCE_INLINE uint64_t getblock64(const uint64_t* p, ptrdiff_t i) {
    if constexpr (endian::native == endian::little) {
        return p[i];
    } else {
        const auto c{reinterpret_cast<const uint8_t*>(&p[i])};
        return static_cast<uint64_t>(c[0]) |
               static_cast<uint64_t>(c[1]) << 8 |
               static_cast<uint64_t>(c[2]) << 16 |
               static_cast<uint64_t>(c[3]) << 24 |
               static_cast<uint64_t>(c[4]) << 32 |
               static_cast<uint64_t>(c[5]) << 40 |
               static_cast<uint64_t>(c[6]) << 48 |
               static_cast<uint64_t>(c[7]) << 56;
    }
}
